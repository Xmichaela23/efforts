...
## Table: `planned_workouts` (Planned Workout Management)

### Purpose
The `planned_workouts` table stores workout plans created by users or generated by training plan algorithms. This table is separate from completed workouts to maintain clean data separation and enable planned vs. actual performance comparison.

### Core Workout Fields
| Column | Type | Nullable | Default | Description |
|--------|------|----------|---------|-------------|
| `id` | uuid | NO | gen_random_uuid() | Primary key |
| `user_id` | uuid | NO | null | User reference (foreign key to users) |
| `name` | text | NO | null | Workout name |
| `type` | text | NO | null | Workout type: run, ride, swim, strength |
| `date` | date | NO | null | Scheduled workout date |
| `description` | text | YES | null | Workout description |

### Workout Structure
| Column | Type | Nullable | Default | Description |
|--------|------|----------|---------|-------------|
| `intervals` | jsonb | YES | '[]' | Interval workout data (run/ride/swim) |
| `strength_exercises` | jsonb | YES | '[]' | Strength exercise data |

### Status and Source
| Column | Type | Nullable | Default | Description |
|--------|------|----------|---------|-------------|
| `workout_status` | text | YES | 'planned' | Status: planned, in_progress, completed |
| `source` | text | YES | 'manual' | Source: manual, plan_template, training_plan |

### Training Plan Integration
| Column | Type | Nullable | Default | Description |
|--------|------|----------|---------|-------------|
| `training_plan_id` | uuid | YES | null | Reference to training_plans table |
| `week_number` | integer | YES | null | Week number in training plan |
| `day_number` | integer | YES | null | Day number in training plan |
...
#### `intervals` (Run/Ride/Swim Intervals)
```json
[
  {
    "id": "interval-1",
  ...
  },
  {
    "id": "interval-2",
    "time": "20:00",
    "effortLabel": "Threshold",
    "repeatCount": 5,
    "segments": [
      {"time": "3:00", "effortLabel": "Hard"},
      {"time": "1:00", "effortLabel": "Easy"}
    ]
  }
]
```

#### `strength_exercises` (Strength Workouts)
```json
[
  {
    "id": "exercise-1",
...
```
Manual Builder → planned_workouts table (source: 'manual')
Algorithm Templates → planned_workouts table (source: 'plan_template')
Training Plans → planned_workouts table (source: 'training_plan')
```

#### Workout Execution Flow
```
Planned Workout → User completes → Moves to completed workout system
Planned vs Actual → Summary comparison in UI
```

### Key Benefits
- **Clean separation** from completed workout data
- **Supports all workout types** (run, ride, swim, strength)
- **Training plan integration** (week/day organization)
- **Source tracking** (manual vs algorithm-generated)
- **Status management** (planned → in_progress → completed)

### RLS Policies
- Users can only view, create, update, and delete their own planned workouts
- Full CRUD operations protected by user_id authentication

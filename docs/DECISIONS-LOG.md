...
- **Decision (Michael ratified):**
  ...
  4. **Formula A retired** (Gate 2). compute-snapshot's calendar-DECOUPLED model (`weekTotal / mean(4 prior weeks)`, chronic EXCLUDING the current week) is replaced by the shared coupled-rolling model (chronic CONTAINS acute; standard Gabbett), same source/window/floor as coach → **persisted `athlete_snapshot.acwr` == what coach computes live**. **Expected shift profile: ramps and post-taper weeks read LOWER** (coupling dampens the ratio; golden fixture: a ramp week reads 1.33 coupled vs 1.50 decoupled). A thin chronic base now also nulls the persisted ACWR (previously it returned a number). **Acceptance gate:** compute-snapshot's response carries a one-time `acwr_convergence` readout (`old_decoupled` vs `new_coupled`, on real data) for Michael's eyeball before this is considered closed.
...
- **Part C (RPE-signal dedup) — pending, gate scoped:** the State BODY section renders the same avg-RPE delta up to three times ("How hard it feels" via `weekly.ts:rpeFeelVerdict`, "Cross-training" via `readiness-receipts.ts:buildCrossTrainingReceipt`, and the LEGS LOADED "why" via `loaded-legs.ts`), all reading `endurance.rpe.current_avg/baseline_avg`, none via `buildBodyResponse`. Rule: when RPE is the SOLE fired factor, defer/roll-up the Cross-training row into "How hard it feels" and fold the LEGS LOADED echo into the same rule. The removed Cross-training string is listed old-vs-new for Michael's review BEFORE ship.

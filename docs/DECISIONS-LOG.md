...
**Addendum (2026-06-11) — header restructured to two rows.** The Deload pill was butting against the date field. First pass added a gap + truncated the title (one row, `justify-between`), but at 380px the date (~137px) + Pick-planned (~110px) + pill squeezed the title to ~1 char ("L…"). Final fix (per user): **two-row header** — outer `flex flex-col gap-2`; **row 1 = title + Deload pill** (the pill describes the workout, so it belongs with the name — it gets the full row width, ~284px for the title, no longer competing with the controls); **row 2 = date + Pick planned** (their own row, full room). Verified at 380px: rows stack cleanly, neither row wraps, Pick-planned ends ~86px before the edge (ample room), and a normal name ("Log: Upper Body") shows in full with the pill; only an extreme name (e.g. "Log: Tri Performance — Hypertrophy Deload (Upper)") truncates, still showing most of it + the pill. The header is taller by one row — fine inside the sheet.

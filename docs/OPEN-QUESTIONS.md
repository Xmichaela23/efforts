...
- **Status:** filed 2026-06-29 · **PROBE-CONFIRMED then FIXED (both locks) 2026-06-29, NOT DEPLOYED.** Probe (E3b-probe analog, real engine code, no DB — `scratchpad/q093-probe.ts`) showed a non-race `five_by_five` goal shipped **durability** (band face pulls, single-leg calf raises, …). Fix proven: post-fix the same path emits real **5×5 (Back Squat, Bench, Row, OHP, Deadlift)**; guard test confirms byte-identical for `neural_speed`/`durability`/`upper_aesthetics`/the freq-4 lanes (only `five_by_five` moved). **Owed:** deploy + verify the live `runRetestBody` request shape (fidelity note below).

...
- **the COACH — STILL UNGUARDED (the remaining Q-129 work).** The coach is a 4th narrative generator with no honesty chokepoint. This IS the "shared spine" deliverable: rather than a 4th point-fix, define the `_shared` honesty pass (fact contract + assertion set, both rule classes above) that the analyzers AND the coach route through, generalizing `execution-honesty.ts` from one rule to the family. Needs Michael sign-off on shape (§ Spec direction). The three point-fixes above are what "each point fix teaches the interface" was waiting on — the interface is now well-informed by three worked surfaces.

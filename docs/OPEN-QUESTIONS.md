...
- **Status:** filed 2026-06-29 · **PROBE-CONFIRMED → FIXED (both locks) → DEPLOYED 2026-06-29** (`generate-run-plan` v144, `create-goal-and-materialize-plan` v224). The live prod durability-downgrade is fixed. Probe (E3b-probe analog — `scratchpad/q093-probe.ts`) showed a non-race `five_by_five` goal shipped **durability**; post-fix the overlay emits real **5×5 (Back Squat, Bench, Row, OHP, Deadlift)**; guard test confirms byte-identical for `neural_speed`/`durability`/`upper_aesthetics`/the freq-4 lanes. **STILL OWED — live e2e confirm:** the fix is deployed + proven in isolation, but NOT yet confirmed on a live round-trip. Can't self-serve (create-goal needs a user JWT; `create`-mode preview still inserts a goal; no barbell test account on hand → a bodyweight account correctly returns durability and proves nothing). Confirm by eyeballing a real plan: a non-race "get stronger" run goal on a **barbell/commercial-gym** account → expect 5×5, not calf raises. Until then, **don't log "verified live."**
...
- **The fix (both locks — APPLIED, not deployed):** (1) `create-goal…index.ts` `(b)-run` block now resolves equipment (narrow `user_baselines` fetch, mirrors the event path) → threads `strength_tier`/`strength_intent`/`equipment_type` into `runRetestBody`: barbell-capable → `strength_power` (developer honored), bodyweight → `injury_prevention` (durability, correct + byte-identical) — clears Lock 1. (2) `five_by_five` added to `RUN_CENTRIC_STRENGTH_PROTOCOL_IDS` (`selector.ts`) — clears Lock 2. Guard test `strength-focus-freq4.test.ts` asserts byte-identical for `neural_speed`/`durability`/`upper_aesthetics`/the freq-4 lanes; 124 deno tests green.

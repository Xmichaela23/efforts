...
### Q-145 — Intensity ANCHORS (LTHR / FTP / future swim CSS) need a calibration health-check: a stale anchor silently shifts every session's bin while each computation looks locally correct (2026-07-08, FILED — D-242-class, one level up)

Live exhibit (D-263 bs3 receipt): the primary user's easy runs come in at ~135–138 bpm against a learned **LTHR of 150** (`0.90` ratio → IF `0.88` → **hard**), so a genuinely-easy Sunday run landed in `hard_cardio`. The HR data is clean (`hr_quality: 'ok'`) — the **anchor** is wrong (LTHR-150 is low-confidence, 95th-percentile-derived). This is a class, not a one-off: intensity binning (Item 2), TRIMP (Item 1), and every %-of-threshold read normalise to an anchor — LTHR for run HR, FTP for bike power, CSS for swim pace (which doesn't even exist yet, Q-139/amendment-2). **A stale or miscalibrated anchor silently shifts EVERY session's bin, and each individual computation still looks locally correct** — the exact D-242 failure mode (absence/garbage rendered as a confident value), but one level up: it's not a missing input, it's a wrong *reference*. **Needs a calibration health-check:** flag when observed data contradicts the anchor (e.g. easy-tagged runs persistently at ≥88% LTHR ⇒ LTHR likely low; best-effort paces implying a CSS that doesn't match the stored one), and either re-learn or mark the anchor low-confidence so downstream bins can widen/soften. Log only — informs Item 1 (TRIMP threshold) and Item 2 (bin boundaries); the anchor-quality signal is itself a candidate provenance flag for Item 4.

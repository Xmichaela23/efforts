...
- **Status:** filed 2026-06-27 · **SCOPED + BUILT 2026-06-29 (D-220), NOT REALIZED END-TO-END.** Engine + module + producer built and **proven in isolation** (12 deno tests, byte-identical ≤3); routing **confirmed run-path** ((b)-run fork, not combined). **Blocked end-to-end on Q-093** (`runRetestBody` omits `strength_tier` → developer drops to durability). Owed: Q-093 fix → deploy. See ENGINE-STATE Q-088 entry. The concurrent-matrix audit is DONE (`AUDIT-strength-frequency-concurrent-matrix-2026-06-29.md`) and shape/scope/lanes are decided. **Shape:** U/L/U/L 4-day container (structure only). **Scope:** endurance-posture gate — `develop` → ≤3 (tri 2), `maintenance`/`parked` → may reach 4 — a strength-focus plan MODE, not a standalone carve-out. **Lanes:** build (`five_by_five` lineage, net-new split content) + power (`performance_neural` lineage, rebalanced to even U/L/U/L). **Run-path first; no optimizer 4th-day placer this cut.** Implementation (audit Tier 1.3 builders) NOT started. Original framing below kept for history.
...
## Q-093 — Non-race run path (`runRetestBody`) omits `strength_tier`/`strength_intent` → ALL non-race run strength developers resolve to `durability` (pre-existing prod bug)

- **Status:** filed 2026-06-29 · **PRE-EXISTING prod bug, broader than Q-088.** The blocker that stops Q-088 freq-4 from materializing end-to-end — but it downgrades EVERY non-race run developer, not just the freq-4 lanes. **The clean first move next session.**
- **The bug:** the non-race run path builds `runRetestBody` (`create-goal-and-materialize-plan/index.ts:2378`) sending `strength_protocol` + `strength_frequency` but **NOT `strength_tier` or `strength_intent`**. `generate-run-plan` honors a chosen protocol ONLY at `tier === 'strength_power'` (`generate-run-plan/index.ts:271`: `protocolId = (tier === 'strength_power' && request.strength_protocol) ? … : undefined`). No tier sent → `tier` defaults to `injury_prevention` → `protocolId = undefined` → the overlay resolver (`resolveStrengthProtocolForGoal`, sport `run`, no intent) falls to **`durability`**. So a non-race "get stronger" goal seeded with `five_by_five`/`neural_speed` **silently ships durability** — the chosen developer is dropped. **(Code-traced this session, NOT device-verified — confirm on a live/preview run before treating as fact.)**
- **Why it's bigger than Q-088:** the fix threads `strength_tier`/`strength_intent` (+ equipment) into `runRetestBody`, which needs the equipment resolver + `baseline` — **not in scope at the `(b)-run` block** (`baseline` is fetched ~`:2626`, after the block). It corrects the downgrade for ALL developers; Q-088's freq-4 lane is just one beneficiary. **Do NOT conflate it into the freq-cap feature** — different bug.
- **Cross-ref:** `create-goal-and-materialize-plan/index.ts:2378` (runRetestBody), `generate-run-plan/index.ts:266-273` (the tier gate), `resolveStrengthProtocolForGoal` (`shared/strength-system/protocols/selector.ts`), Q-088 (downstream — blocked on this), D-220, D-218 (the `(b)-run` fork).

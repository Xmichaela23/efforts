...
| 31 | **[SPEC]** 🔴 The Get Stronger spec's `[T1 Wilson 2012]` modality claim is downgraded to match the repo's own science doc | **THE SPEC CONTRADICTS `SCIENCE-concurrent-training-interference.md` §2, which is already in this repo.** That doc marks the running-vs-cycling split **CONTESTED**: "Schumann 2022, with more studies, found NO modality moderation… treat as a plausible mechanism with split meta-analytic support, not a settled rule." The spec cites it **T1** twice, in user-facing copy — *"Why easy stays and hard goes"* and *"Why cyclists have more room."* By the spec's own tier rules that is a T1 that should be T2/contested. **I made the same error in conversation**, calling the modality split the strongest-supported part of the hypothesis, without reading the repo's doc first. |
| 32 | **[SPEC]** The spec's endurance volume cut uses the SOURCED figure, not an invented one | Spec says **~66% of baseline [T3 — invented]**. `SCIENCE-5x5-linear-progression.md` §Rønnestad carries a **sourced ~20-30% reduction**. Either adopt the sourced number or state why the block needs a deeper cut. A T3 sitting next to an available T1 is a choice that should be deliberate. |
| 33 | **[BLOCKS-SPEC]** "Endurance maintain" actually means easy/zone-2 with quality parked | **RAISED AND NEVER CHECKED.** `seedFromGoal` sets endurance to `maintain` when strength is `develop` — but whether `maintain` caps INTENSITY or only trims volume was never traced. The whole interference argument is about intensity, so if maintain leaves quality sessions in, the block's core premise isn't enforced. Trace it before trusting the spec's Part 2. |
| 34 | **[ENGINE]** There is ONE implementation of "get stronger with barbells", not three | **Currently three.** `five_by_five` (full-body A/B 2×/wk, 70→85%, the pickable default), `strength_focus_build`/`_power` (4-day U/L/U/L, *identical* load curve, unreachable, self-described as "CONVENTION" not methodology), and `strength_primary` (4-day U/L/U/L, 72→94% block, unreachable via the registry yet it built the live plan through `config.source`). `strength-focus-split.ts` isn't even in the `getProtocol` switch. **Was banner-only until now** — banners get superseded, so it is a ledger line. |

...
**Contrast:** the race generators (`generate-run-plan` / `generate-triathlon-plan` / older `generate-combined-plan` race path) DO emit rich run tokens (`interval_6x800m_5kpace_r90s`, `longrun_90min_easypace`, `run_easy_50min`, …). The non-race path (`generate-combined-plan/session-factory.ts`, the Easy Run / Long run / Combo sessions) does not.

**Fix direction (deploy-gated, NOT started — needs sign-off):** have the non-race run builder attach `steps_preset` tokens (at minimum `run_easy_<Nmin>` / `longrun_<Nmin>_easypace`) the same way the race path does, so run load reflects the prescription. **Guard rail:** this path also materializes the strength spine — the change must be scoped to run-session content only and verified against a combined-plan fixture so it can't perturb strength placement/frequency (the D-215 contract). Verify: activate a fresh non-race plan on a deno/throwaway fixture, assert run rows now carry tokens AND strength rows are byte-identical to pre-change.

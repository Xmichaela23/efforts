...
- **Consolidated design inputs (2026-07-02 State walkthrough) — the round works WITHIN the extended D-232 (row receipt → "open for more" breakdown → provenance) and must address all of these ONCE, coherently:**
  ...
  6. **The cross-training-strain double-count (detection bug)** — `stressSignals` (`coach/index.ts:5056`) counts `bodyConcerned` (`assessment.signals_concerning > 0`) alongside the specific signals (`rpeRising`/`driftWorsening`/…), but `bodyConcerned` **overlaps** them (a declining RPE *is* the concerning signal). So a single declining signal hits `stressSignals ≥ 2` and fires "strain **across disciplines**" off ONE signal. The 2026-07-03 receipt work made the *display* honest (single distinct signal → "Perceived effort up (5.3 vs 4.4)", no "across disciplines"), but the underlying detection still over-fires the `warning` tone — fix the gate here (count distinct signals, or exclude `bodyConcerned` from the tally).

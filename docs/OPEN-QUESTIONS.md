...
## Q-015 — §6.3 "never repeat across consecutive sessions" relies on rotation salts, not recent-pick memory — RESOLVED 2026-05-25 (D-044 item 6 + D-045 harvest fix)

- **Status:** RESOLVED 2026-05-25 (D-045). D-044 item 6 shipped the threading (`prevWeekDrillTokens` opt → 7 swim creators → `pickSwimDrillInset` filter at `swim-drill-tokens.ts:521-538`), but the orchestrator harvest in `generate-combined-plan/index.ts:286-301` walked `week.days[].sessions[]` while `buildWeek` actually returns flat `week.sessions[]` (see `computeWeekMetrics` at `week-builder.ts:593`). The wrong-shape walk silently produced an empty Set every week, so the filter never fired in production. Surfaced by a real-plan audit (W2 Fri Catch-Up → W3 Mon Catch-Up). Fix (D-045) extracted the harvest to `generate-combined-plan/drill-token-harvest.ts` (pure helper) and pinned the N→N+1 contract with 6 tests in `drill-token-harvest.test.ts`.

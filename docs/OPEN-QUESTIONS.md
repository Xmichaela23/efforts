...
**Where the vocabulary already lives (traced 2026-07-05):** `session-factory.ts` ALREADY defines the correct run tokens — `easyRun` (`session-factory.ts:344`) → `run_easy_${miles}mi`, `longRun` (:319) → `longrun_${miles}mi_easypace`. The 109 historical tokenized run rows carry exactly these. So Q-126 is NOT inventing a vocabulary — it's routing the existing one into the ONE builder that skips it. The non-race combined path does NOT use `session-factory.easyRun/longRun`; its run-content sites (in `generate-combined-plan/week-builder.ts`) emit `steps_preset: []`. Tell (live rows named "Easy Run" / "Combo 1 of 2 — Long ru…" — NOT session-factory's "Easy Run — N mi" shape). NB: the `steps_preset: []` at `week-builder.ts:2540/2564/2587` are the ASSESSMENT-WEEK time-trials (`buildAssessmentWeekSessions`) — those are correctly instructions-only (materialize skips token expansion on empty presets) and MUST stay empty; the regular easy/long-run builder site is elsewhere (see next-step trace below).

**LOCKED ROUTING — Option B (duration-native token helper).** Deploy-gated, NOT started, needs sign-off.
- **Seam:** the single point where the non-race builder emits `type:'run'` content with empty `steps_preset` (in `week-builder.ts`). NOT `activate-plan` (its `estimatePlannedWorkload` is correct post-Gap-B), NOT the strength builders (`runStrength`:2549 → `st_main_squat_3x8…`), NOT `session-factory` (already correct).
- **Helper:** a pure `runIntensityToken(kind, durationMin)` → `run_easy_${durMin}min` / `longrun_${durMin}min_easypace`, set as `steps_preset: [token]`. Duration-native (week-builder is minutes-based, has no miles). Vocabulary unifies at the MATCHER, not the string: Gap B matches the `run_easy` / `easypace` substring, so `_60min` and `_5mi` both resolve to 0.65 — parity with the race path + `materialize-plan` token-parser without reusing session-factory's miles-based `PlannedSession` shape.

**Three guarantees the strength spine stays untouched:**
1. **Type gate** — only `type === 'run'` sessions get a token; strength rows never in scope; edit lives entirely in the run branch.
2. **The combo is the ONE co-mingle risk** — "Combo 1 of 2 — Long run" is the D-225 hybrid (long run + fatigued-legs stations). Live data shows it as a `type:'run'` row → already split into a run row + a separate strength row → the type gate isolates it for free. **CONFIRM at implementation** whether it's one session or two; if ever a single co-mingled session, token the run component ONLY and leave the station tokens intact, exactly like `activate-plan:449–450`'s brick split filters bike/run by prefix.
3. **No scheduling delta** — tokens carry intensity only; day placement is the week-optimizer's job, independent of `steps_preset`; duration unchanged → the only observable change is `workload_planned` dropping from the 0.75 default to the honest 0.65/0.70.

**Isolation proof (deno fixture, per verify-on-fixtures rule) — the permanent regression gate:** activate a fresh non-race combined plan on a throwaway user BEFORE/AFTER; assert (a) run rows now carry `run_easy_*`/`longrun_*` tokens and `workload_planned` reflects 0.65/0.70; (b) the `type='strength'` subset is **byte-identical** across both runs (same days, counts, `steps_preset`, durations) — a zero-diff on strength is the spine-safety gate. Keep it as a permanent regression.

**Scope discipline — do NOT fold in:** (1) no quality tokens on non-race easy runs (they're genuinely easy; `run_easy` is honest; the Get Strong shape prescribes no run quality). (2) Leave the long-run 0.65-vs-0.70 question alone — `longrun_easypace:0.70` is currently dead (bare `easypace` matches first), so long runs compute at 0.65; nudging to 0.70 is a separate refinement, NOT Gap A. (3) Go-forward only — no retro-fix of historical rows (decided 2026-07-05: pre-launch, no live surface reads them).

**IMMEDIATE NEXT STEP (when greenlit):** ~5-min trace to pin the exact regular easy/long-run builder site in `week-builder.ts` (the `steps_preset: []` for real training runs, distinct from the `buildAssessmentWeekSessions` time-trials). That's the only discovery debt left; everything else above is locked.

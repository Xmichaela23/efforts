...
### Q-136 — `plan_phase` null on ALL snapshot rows → Gate 2 (build-band load-status leniency) is INERT in production until phase labeling is populated upstream (2026-07-07, FILED, trace-and-fix owed)

The `09-db-schema.md` audit (§4) confirmed `plan_phase` is `null` on **every** `athlete_snapshot` row, even where `plan_id` + `plan_week_number` are fully populated — the column exists, the writer never fires. Consequence for D-259: **Gate 2** (the build/baseline plan-phase tolerance that would read WK1 ACWR 1.40 as `on_target`) fires ONLY when `weekIntent ∈ {build, baseline}`, so on the LIVE path — where phase resolves to `'unknown'` — Gate 2 does nothing. This is BY DESIGN (fail-safe: unknown keeps the strict bands, never over-softens), and **Gate 1 alone still fixes the reported symptom** (clears the false running-`'high'` → `'elevated'`). But the "reads `optimal` in a build week" benefit is unrealized until phase labeling works. **Owed (separate scope, NOT D-259):** trace why `weekIntent`/`plan_phase` isn't resolving from the plan into the snapshot (is it never written by `compute-snapshot`, or written null, or the plan's week-intent isn't threaded to the snapshot?), and fix it. When fixed, the existing D-259 fixtures (`build` cases) become the live path with no code change. Verification is cheap: re-run the `_shared/load-status-reconcile.test.ts` `build` regression against a snapshot whose `weekIntent` is populated.

...
## Q-061 — Equipment/drill pace-contamination exclusion from the swim trend — BOTH directions (data captured, filtering not built)

- **Status:** filed 2026-06-15 · **scope WIDENED 2026-06-16** (fins-only → any non-unaided-swimming set, both directions) · capture done (D-162), filtering deferred
...
- **Partial since 2026-06-16 (D-183 — NARRATIVE half only):** `analyze-swim-workout` now **reads** the captured equipment (first consumer outside the client) and the swim narrative **honestly flags** fin-assisted pace — direction-only, never quantified ("your average pace … is flattered by fin-assisted sets, so your true unaided swim speed reads faster"). This makes the **displayed prose** honest. It does **NOT** touch the **trend substrate** (`compute-facts` `pace_per_100m` is still fin-blended and equipment-blind) — so the core of Q-061 (the trend/fitness-signal exclusion) remains **unbuilt**. D-183 = honest read; Q-061 = honest signal. **Directional gap (post-widening):** the shipped D-183 narrative flags only the **fins/optimistic** direction ("reads faster"); the **kick/drill pessimistic** direction ("reads slower") is NOT yet flagged in prose — part of the widened scope, a small D-183 narrative follow-up alongside the trend-substrate build.

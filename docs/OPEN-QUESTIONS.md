...
- **Status (2026-07-02) — PARTIALLY BUILT: the highest-value signal is now wired.** Michael's call: *"why have them"* — the check-in was collecting soreness/sleep and discarding them, when **the evidence says subjective soreness/readiness OUTPERFORM objective markers for overreaching detection (BJSM systematic review)** — i.e. the app was collecting its best data and throwing it away. So the dropped sliders are now read as an **advisory longitudinal signal** (Q-049 Phase 1a):
  ...
  - **Two-stage cold-start design (deliberate):** **v1 (now)** = ABSOLUTE floor (soreness ≥6/10 on ≥4 of last 6 check-ins) — works from day one, resists a single noisy day. **v2 (when ≥3–4 weeks of check-ins exist)** = rising-trend-vs-own-baseline (median ± individual SD, the consensus gold standard / AthleteMonitoring / runner-overreaching study), mirroring the pace/HR-drift signals. v1 is structured so **v2 is a predicate swap, not a rewrite** (fetch + emit stay identical).
...
- **Status:** filed 2026-07-02 · **audited, not built — the roadmap to finish the athlete-continuity migration.** Full evidence: `AUDIT-spine-conformance-2026-07-02.md` (+ synthesis `AUDIT-app-synthesis-2026-07-02.md`).
...
- **Status:** filed 2026-07-02 · **audited, not built.** Full catalog: `AUDIT-state-screen-2026-07-02.md` (H1–H4, M1–M4, L1–L3, ranked).
- **Top items:** H1 bench "125→115 · back off" is RIR-only + baseline-blind (fix with Q-106's resolver); H2 a strong test raises nothing (unbuilt test excluded; `previous_e1rm` hardcoded null; 0-RIR test *falsely* triggers back-off); H3 two contradictory strength rows ("needs data" client vs "125→115" coach — the PERFORMANCE section is self-labeled "NOT YET SHIPPED" yet rendered live); H4 RACE "Marathon — 0w out" + "add a race target" contradiction (`weeks_out` defaults to 0; renders on `has_active_plan` alone). **Quick wins:** H4, swim-rest provisional tag (M1), delete the `×1.1`/phantom `peak1RM` dead code (L1/L2). **Verify:** "feels 1.2 harder" (M4) — trace to narrative vs RPE-ratio; if LLM narrative it violates the coach's own no-"felt"-language guard.
- **Cross-ref:** Q-106, D-230.
...
- **Design inputs folded in from the 2026-07-02 State walkthrough (address the fork ONCE, coherently):**
  - **H3 — the two contradictory STRENGTH rows** (top row spine `state_trends_v1` = "needs data · N unplanned" vs bottom row coach `per_lift` = confident "Working ~125 vs 150 baseline"). Same screen, two engines, opposite confidence. Q-111 must reconcile which strength verdict the athlete sees (or how they compose) — not leave two.
  - **The glass-box standard (D-232)** is the language contract every Q-111 verdict string must meet: plain verdict + receipt, plus the plan/history context ("expected during the marathon block, Week N of 12").
- **Cross-ref:** Q-107 (State H1 "125→115 RIR-only + baseline-blind" — fixed; H3 folded here), D-231, D-232 (glass-box standard it designs against), D-225 (Get Stronger add-ons/blocks), Q-097; `AUDIT-state-screen-2026-07-02.md` H1/H3.

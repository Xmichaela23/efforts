...
| **Retest** (one row per sport, 2026-09-05) | Strength: Lower / Upper lifts open the logger's test session now. Run: the threshold test goes on the calendar three days out; a run logged on that day is read as the test and the result is proposed. Bike: the 20-min and 5-min FTP tests go on the calendar two days out; the learner fits the power curve, and a 5-min test prices only alongside a 20-min effort on file. A scheduled test shows its day and can be removed. | Viada p210 (run), p212 (20-min FTP), course Module 3 (5-min all-out) |

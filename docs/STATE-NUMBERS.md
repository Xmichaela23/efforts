...
| **Retest** (one row per sport, 2026-09-05) | Strength: Lower / Upper lifts write today's retest session to the calendar (tagged like week one's test, linked to the plan) and open it in the logger; the rebuild reads the latest tested session per lift, any week. Run: the threshold test goes on the calendar three days out; a run logged within a day of it attaches by tag and is read as the test; the result is proposed. Bike: the 20-min and 5-min FTP tests go on the calendar two days out; the learner fits the power curve, and a 5-min test prices only alongside a 20-min effort on file. Every "your number" pill carries an auto segment: auto clears the manual choice through the same save Baselines uses and re-prices. | Viada p210 (run), p212 (20-min FTP), p215 (lift retest), course Module 3 (5-min all-out) |

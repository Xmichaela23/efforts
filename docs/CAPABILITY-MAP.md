...
### Added 2026-08-24 — THE STANDING PLAN (the strength-leading block, live)

| what it does for an athlete | where | status |
|---|---|---|
| **"Build me a strength block around the endurance I keep — my sports, my days, my picks."** The whole strength-leading plan: Viada's Strength+5K frame, week-1 test off p215, per-slot sport + variant choices, swim add-on, muscle floors, ME set ladder, rotating named plyos. | wizard `NonRaceBuilder.tsx` (strength path) → fork in `generate-strength-plan/index.ts` → `_shared/standing-plan/` (`compose.ts` the composer; `frames.ts` p246 as data; `working-number.ts` p215; `sport-slots.ts`; `plyo.ts`; `me-history.ts`; `restate.ts`; `day-map.ts`) → restated on save by `rematerialize-standing-block` | **BUILT + DEPLOYED.** Wizard verified end-to-end (Michael, 2026-08-24); the endurance-week screen re-verified 2026-08-25. **The accessory step is the Dial (D-450), NOT the Wendler push/pull/single-leg-core picker** — `_shared/standing-plan/accessory-picks.ts`, eight picks incl. two day-scoped pairs. Hard endurance sessions are opt-in, max 2, default 0 (D-451). Unverified: the test-save → weeks-fill chain, and the built week's accessory rows. D-442 · D-450 · D-451. |
| **"Price every session off MY numbers, and re-price the block when I test."** Viada working number (96%, two formulas averaged) in its own config key — never `training_max`. | `_shared/standing-plan/working-number.ts` · stored via `plan-row.ts` | BUILT. Same-word collision with Wendler's 85% documented in both files. |
| **"…and climb it as I get stronger."** Reactive progression — reps advance on logged performance, weight follows. | `_shared/standing-plan/progression.ts` (`progressionVerdict:174`, `advanceStep:209`, `meSetsFromHistory:397`) | ⚫ **BUILT AND ORPHANED — the plan climbs on a CALENDAR only.** Verified 2026-08-26: `progressionVerdict` and `meSetsFromHistory` have TEST-ONLY call sites, `advanceStep` has **none at all**, and there is no `advanceOrHold` in the repo. What ships is `prescribedLoad:91` = `workingNumber × (1 + scheduledRise) × haircut × pct`, where `scheduledRise` is `RATE_ANCHOR[frame].perWeek × (week − 1)` (1%/3wk, p247, `frames.ts:258`) — no logged set is read. The printed rep range is static (`compose.ts:942`). Measured over 12 weeks at 85% with 5 lb rounding: a 225 squat gets **3 distinct weights**, a 135 press gets **2, with an 8-week identical stretch**. ⛔ Michael ruled out fractional plates (2026-08-26) — 5 lb rounding stays; the gap is the missing caller, not the increment. **This is the next session's job.** |
| **The three stage libraries** — endurance sessions (13 families × 3 levels, page-cited bands), the strength grid (pattern × category × intent over our own catalogue), accessory dosing (sets per muscle, floors, 14-set ceiling). | `_shared/endurance-library/` · `_shared/strength-grid/` · `_shared/accessory-dosing/` | BUILT, client-reachable, mutation-tested. The composer is their first consumer. **2026-08-26 (D-455):** the grid's 211-movement pool is now EQUIPMENT-GATED — 147 movements tagged in `src/lib/strength-gear.ts` (previously 160 of 211 were untagged and 148 of those reached a declared home gym anyway), and ten needing kit `GearKey` cannot express are dropped from prescribing via `PRESCRIPTION_EXCLUDED` (`strength-grid/taxonomy.ts:374`, filtered at `:415`) while staying in the library. **DEPLOYED 2026-08-26 18:50:23 UTC** (`generate-strength-plan` v158, `rematerialize-standing-block` v33, `create-goal-and-materialize-plan` v369 — all three had to move, since `strength-gear.ts` and `taxonomy.ts` ride the `_shared` bundle). ⚠️ Not device-verified. |
| **The old plan builders** — marathon, tri, combined. | `generate-run-plan` / `generate-triathlon-plan` / `generate-combined-plan` | ⛔ **CLOSED FOR REPAIRS** (banners on the files, 2026-08-22). Repairs land through the Standing Plan work, not patches. |
...
4. **A run plan generator.** There are **six DEAD generator classes** in `generate-run-plan/generators/` (corrected from "five", census 2026-08-07), and **two of them collide by class name with a live file** — editing the wrong one is a silent no-op:
   - ✅ **DELETED — this cleanup is DONE (verified 2026-08-19: the file is not on disk).** Kept as a record of the duplicate-generator trap, not as an open task.
   - `supabase/functions/generate-run-plan/generators/simple-completion.ts:89` exports **`SustainableGenerator`** — live twin `…/generators/sustainable.ts:38`. Ruin is **457 lines** (Mar 2); live is 841 (Aug 6). All twelve of the ruin's methods also exist on the live class.
   - ✅ **DELETED — this cleanup is DONE (verified 2026-08-19: the file is not on disk).** Kept as a record of the duplicate-generator trap, not as an open task.
   - `supabase/functions/generate-run-plan/generators/balanced-build.ts:66` exports **`PerformanceBuildGenerator`** — live twin `…/generators/performance-build.ts:75`. Ruin is **1,249 lines** of plausible marathon periodization; this is the worse trap of the two. ⚠️ It is a `PerformanceBuildGenerator`, **not** a `SustainableGenerator` — don't conflate the two collisions.

   The other four ruins: `cumulative-load.ts` · `hybrid-athlete.ts` · `time-efficient.ts` · `volume-progression.ts`. ⚠️ **`base-generator.ts` and `assign-days.ts` are LIVE**, not ruins. Only `sustainable` and `performance_build` are switched on (`generate-run-plan/index.ts:280`, cases `:282`/`:300`). Full census: `docs/AUDIT-plan-generators-2026-08-07.md`.
...
13. **A plan "baker".** ⛔ **GONE — and this entry is the reason to check before rebuilding, not the reason to.** It read *"`src/services/plans/tools/plan_bake_and_compute.ts:948` (`augmentPlan`) exists, works offline (`npm run bake`)"*. None of it exists as of 2026-08-19: no file, no script, no references. ⚠️ **The anti-rebuild index listing a capability that is gone is worse than not listing it** — a session reads "already built, just disabled" and stops looking.
...
15. **A marathon race-entry screen.** `GoalsScreen.tsx:2433` (`renderEventForm`) already takes name, date, distance, fitness, strength protocol + frequency and plan start date, and builds a plan. Marathon is in `DISTANCE_OPTIONS:115`. Do not build a second race form — the missing pieces are `extract-races` wiring and the hold cards, not the screen. ⟨2026-08-03⟩ ⛔ **STALE 2026-08-06 — RUN WAS ROUTED OUT OF THAT FORM.** `renderEventForm` no longer offers Run (its sport select drops it and defaults to Ride); a run race routes to the Focus → Race card (`NonRaceBuilder`), which is the ONLY marathon builder now. The form keeps ride/swim/tri. `DISTANCE_OPTIONS.run` stays — an already-built run event reads its distances back through that map. [D-397]
...
| **threshold_pace** | ✅ `resolveCurrentRunThresholdPace` (`src/lib/resolve-current-run-pace.ts`, D-300, 2026-07-18) | coach + race-projections + snapshot spine (`extractRun`/`resolveLiveRun`) + infer-training-fitness | 🟢 **SINGLE-SOURCE.** 3 units normalized to sec/mi (+ sec/km carried); learned-first, Q-174 choice honored. Left deliberately (not fractures): arc `buildRunPaceForCoach` (Law-3 provenance view), create-goal gates, ~~generate-run-plan VDOT (circular)~~ **CLOSED 2026-08-06 [D-395]: the sustainable path's VDOT is now DERIVED from `resolveCurrentRunEasyPace`, so the selected pace anchors it rather than the other way round**, race-readiness (race target), course-* display (learned-first + fiveK). |
| **run threshold pace** | ✅ `src/lib/resolve-current-run-pace.ts` `resolveCurrentRunThresholdPace` (D-436); MEASURED by `src/lib/run-critical-speed.ts` (D-437) | spine · coach · race-readiness · marathon builder · Strong Focus · materialize | 🟢 **SINGLE-SOURCE + BOUNDED.** Five states named by `describeThresholdBasis`. An inferred value is held to `easy ÷ 1.19` ±4%; measurements and typed values never are. ⚠️ The best-effort fit needs 2 hard efforts in different duration bands on different days — it abstains until then, by design. |
| **run easy pace** | ✅ `resolveCurrentRunEasyPace` (D-287/Q-174) | 13 surfaces | 🟢 SINGLE-SOURCE. ⚠️ Its BAND is anchored on threshold HR, and a non-detection can no longer anchor it (D-440). |
| **LTHR (run + ride)** | ✅ `src/lib/resolve-current-lthr.ts`, `{ sport }` option (D-439) | analysis · workload · easy bands · coach · baselines · marathon builder | 🟢 **PER SPORT since 2026-08-20.** The bike had no owner and three private chains. ⚠️ HR **zones** are per-sport too — one shared array was priority 1 for every discipline and binned rides against running zones. |
| **anchor discipline** | ✅ `_shared/anchor-resolver-lint.test.ts` (D-438) | the build | 🟢 **ENFORCED.** 49 raw readers frozen → **32** (26 legitimate, 6 swim). The ledger MAY ONLY SHRINK. |
...
| **run easy pace** | ✅ `src/lib/resolve-current-run-pace.ts:164` (`resolveCurrentRunEasyPace`) | ⟨A31⟩ server ✅ universal (D-287) | 🟡 client stragglers: `AllPlansInterface.tsx:664/791`, `StructuredPlannedView.tsx:352`, ~~`PlanWizard.tsx:470`~~ (retired, [D-448]), `ArcSetupWizard.tsx:1693` |
...
| Today's narrative for a hard run or ride | `_shared/planned-narrative.ts` → `materialize-plan` stamps `computed.narrative` beside `step_lines` → `get-week` passes it → `src/lib/today-lines.ts enduranceLinesFor` prints it above the purpose line | BUILT 2026-09-20 | Today's card only; the session sheet, week view and device sends keep the step list. Same grouping as the list (`groupAt` / `setBlockAt`). No narrative when an effort carries no pace or watts. Words approved by Michael 2026-09-20 |
...
| Hard aerobic session, strength-primary block | `strength-primary-plan.ts` `hardRunSession` / `hillSession` / `flatSession` (+ `bikeQualitySession`); terrain menu `NonRaceBuilder.tsx` (Hard day = Run) | **BUILT — four run options + bike inferred [D-391]** | ⛔ **REWRITTEN 2026-08-19 — THE TERRAIN MENU IS GONE FROM THE WIZARD.** The intake asks ONE ground question and only on a run holding the top-end slot: **Speed focus** (flat, `goal:'speed'`) vs **VO2 max focus** (incline, `goal:'vo2'`). Which flat surface, and hill-vs-treadmill, are named in the SESSION DESCRIPTION and answered on the day — they changed a sentence, not a token. Bike = `4×4` Helgerud / threshold blocks, and the athlete now ALLOCATES which sport holds the top end ([D-429]). The 48h flat-sprint clearance keys on `goal:'speed'`, not on `terrain:'flat'` — it missed the sprint session entirely until 2026-08-18. **[Q-260] CLOSED.** ⚠️ **NONE of the rebuilt intake is device-verified** — see the AWAITING block in `POLISH-PUNCH-LIST.md` |
...
| capability | entry point | status | note |
|---|---|---|---|
| **THE wrapper** — create a goal, build + activate a plan | `create-goal-and-materialize-plan/index.ts:2115` (`Deno.serve`) | BUILT | ⟨A31⟩ every plan path goes through here — ⚠️ the `PlanWizard` exception this row named is retired ([D-448]). **Survivor census 2026-08-07: `docs/AUDIT-plan-generators-2026-08-07.md`** — read it before editing anything under `generate-*/generators/` |
| Season / combined (multi-sport) | `generate-combined-plan/index.ts:60` | 🟢 **LIVE** | the most active engine; the only one wired to `week-optimizer`. Reached from `create-goal…:1756` (inside `buildCombinedPlan`, defined `:1187`) on all three combine gates (`:2826` non-race tri-shaped, `:2913` tri, `:3530` run). **No ruins inside it** — every non-test module has a live importer (census §3.2) |
| Run race plan | `generate-run-plan/index.ts:51` (`Deno.serve`); approach switch `:280` | 🟢 **LIVE — 3 callers** | only `sustainable` (`:282`) + `performance_build` (`:300`) are switched on; `default` 400s (`:305`). ⛔ **This — not `generate-combined-plan` — is what a SINGLE marathon goal actually builds** (`create-goal…:3786`). Also reached by the non-race run retest head (`create-goal…:2801`). ⛔ **THE CLIENT'S THIRD DOOR IS GONE (retired `b0fecb8f`, recorded as [D-448]).** This row described `src/components/PlanWizard.tsx:858` + route `App.tsx:55` as live, writing a plan with **no goal row and no `activate-plan`** — a named file, a named line and a real-sounding hazard, all pointing at a file that does not exist. A session auditing plan lifecycle would have chased it for hours. **`NonRaceBuilder.tsx` is the only client front door**; `ArcSetupWizard.tsx` is live and NOT a rival (multi-race season planning). Its own phase structure (`generators/base-generator.ts:274`), own recovery weeks (`:411`, −30% volume `:640`), and it does NOT route through `week-optimizer` (audited 2026-08-03) | ⛔ **REBUILT 2026-08-06 — the marathon block is now a PRESCRIPTION with a computed prerequisite; read `ENGINE-STATE.md` → "HOW A MARATHON BLOCK IS BUILT, END TO END" before touching any number in it.** Long-run arc `src/lib/run-volume-tables.ts` (`buildLongRunArc`, `marathonPrerequisiteFor`, `longRunPeakWeek`) — shared with the intake, so both quote one arc. [D-392]…[D-398]
| **`generate-run-plan/generators/` — the survivors** | LIVE: `sustainable.ts:38` · `performance-build.ts:75` · `base-generator.ts:20` (superclass) · `assign-days.ts:102` (pin-reading placement, imported `base-generator.ts:14`) | 🟢 **LIVE — 4 of 10 files** | ⛔ **RUINS (zero importers, all frozen at Mar 2):** `balanced-build.ts` · `simple-completion.ts` · `cumulative-load.ts` · `hybrid-athlete.ts` · `time-efficient.ts` · `volume-progression.ts`. ⚠️ `base-generator.ts` and `assign-days.ts` are **NOT** ruins — deleting either kills every run plan. Census §3.1 / full do-not-edit path list §3.6 |
| **✅ THE TWO COLLISIONS — RESOLVED, both ruins DELETED (verified 2026-08-19)** | see note | ✅ **NO LONGER A HAZARD — kept as the record of the trap.** Only the LIVE files below remain on disk; the RUIN paths are gone. ⚠️ Read this row as history, not as a warning to act on. | **`sustainable`:** LIVE `supabase/functions/generate-run-plan/generators/sustainable.ts:38` (841 ln, Aug 6, instantiated `index.ts:282`) — RUIN `…/generators/simple-completion.ts:89` (**457 ln**, Mar 2, zero importers). **`performance_build`:** LIVE `…/generators/performance-build.ts:75` (2,337 ln, Aug 6, instantiated `index.ts:300`) — RUIN `…/generators/balanced-build.ts:66` (**1,249 ln**, Mar 2, zero importers). ⚠️ The 1,249-line corpse is the **`PerformanceBuildGenerator`** twin, *not* a `SustainableGenerator` — the two ruins are different files with different class names. Census §3.5 |
| **THE LIVE MARATHON PATH (completion)** | `create-goal…:2115` → approach picked `:3596` → invoke `:3786` → `generate-run-plan/index.ts:51` → switch `:280` → **`new SustainableGenerator` `:282`** → `generators/sustainable.ts:38` | 🟢 **LIVE — traced end to end 2026-08-07** | week built by **`sustainable.ts:137`** (`generateWeekSessions`) using `getRunningDaysForWeek` (`base-generator.ts:687`), `calculateWeeklyMileage` (`sustainable.ts:421`), `getLongRunMiles` (`:557`) off `buildLongRunArc` (`src/lib/run-volume-tables.ts:286`). Days placed by `assignDaysToSessions` (`base-generator.ts:710`) → **`assign-days.ts:102`**; rest day derived as the day before the long run (`:136`). ⛔ **Race week takes a different route:** `generateRaceWeekSessions` (**`sustainable.ts:251`**, entered `:165`) places anchors itself and **bypasses `assignDaysToSessions`** (`:363`); **race day = `createCompletionRaceDay` (`sustainable.ts:398`)**, pushed at `:286`, distance-token not duration-token. The time-goal path has its own race day (`performance-build.ts:833`). Full trace: census §3.7 |
| `generate-plan` (the 4th generator) | `generate-plan/index.ts` | ⛔ **RUIN — zero callers (re-verified 2026-08-07)** | 71 lines, dated 2024, `serve()` from `std@0.168.0`. Validates five input fields and returns `success:true`; generates nothing. Repo-wide grep outside its own dir returns only stale comments (`_shared/week-optimizer.ts:131` + docs). Listed here so it stops being mistaken for a generator |
| Run non-race (capacity, retest head) | gate `create-goal…:2394` → `:2724`; invoke `create-goal…:2801` | 🟢 **LIVE** | `goal_type` capacity/maintenance + `sport === 'run'` + not Get-Strong → `generate-run-plan` with `approach: 'sustainable'`, `terminalShape: 'retest'` |
| **Get Stronger (strength-primary)** | `generate-strength-plan/index.ts:26` (`Deno.serve`) → `shared/strength-system/strength-primary-plan.ts`; gate `create-goal…:2443`, invoke `create-goal…:2660` | 🟢 **LIVE — PARTIAL** | entry condition: non-race goal + `per_discipline_posture.strength === 'develop'` + **no** endurance discipline on `develop`. One composer only (`composeStrengthPrimaryPlan`); below it `place-week.ts`, `loading/wendler-531.ts`, `loading/cycle-verdicts.ts` all live. All 9 protocol IDs in `protocols/selector.ts:277-293` reach a real module — no orphaned protocol (census §3.4). ⚠️ `strength-arc.ts` belongs to the **run** overlay, not this path. ⚠️ **No equipment gate — deliberately removed 2026-07-25** (`create-goal…:2440-2452`, "Do not reinstate an equipment gate"). Entry is gated on having all FOUR main-lift 1RMs, with a message. Still true: **no plan pin** (`generate-strength-plan` never calls `buildAthleteSnapshot`). ⟨A31⟩ |
...
| **Assistance slots** — the three accessory movements. ⛔ **GET STRONGER / Wendler PATH ONLY.** The Standing Plan does NOT use this picker — its accessory screen is the Dial (D-450, `_shared/standing-plan/accessory-picks.ts`). The two screens are separate `StepLayout` blocks and share no table. | `src/lib/assistance-menu.ts` (`resolveAssistance`, `ASSISTANCE_MENU`) | **BUILT — D-404 IN WORKING TREE, not pushed, not deployed** | ⛔ **ONE SOURCE** — the intake dropdown and the composer both read it. Day-dependent: a pick that collides with the day's main lift is substituted. **Upper days are push · pull · ARM, lower days pull · single-leg · core (D-404 supersedes D-385).** ⚠️ The plane-rebalance (chins→rows on press day, D-328) is now **OFF by default** — gated behind `AssistanceTemplate`, `concurrent` only, no production caller. **Never priced — the engine prescribes no weight for assistance, ever** |
...
| Triathlon — standalone legacy | `generate-triathlon-plan/index.ts:31` (`Deno.serve`) → `generators/tri-generator.ts:187` (`TriathlonGenerator`) | 🟢 **LIVE (legacy) — PARTIAL** | ⚠️ **legacy ≠ dead.** Reached whenever `sport === 'triathlon'` AND `combine === false` — i.e. a **single** tri event goal, since `combine = eventGoals.length >= 2` (`src/lib/arc-setup-persistence.ts:465`). Invoke `create-goal…:3108`. One class, no orphaned sub-generators; `base_first` / `race_peak` are branches inside it (`tri-generator.ts:406/:772/:871/:956/:1017`). **bypasses `week-optimizer`** — its day placement can disagree with combined |
...
| Library plans | ⛔ **RETIRED — the whole chain is gone (checked 2026-08-19).** `LibraryPlans.ts`, `plan_bake_and_compute.ts` and the `npm run bake` / `plan:validate` scripts have **zero references anywhere in the repo**; the commands were still listed in `CLAUDE.md` too. | GONE | This row read PARTIAL with a live file:line. ⚠️ If library plans are ever wanted again that is a BUILD, not a re-enable — do not read this row as a disabled feature waiting for a flag. |
...
⚠️ Add (census 2026-08-07): **`generate-overall-context`** — ⚠️ its caller `src/hooks/useOverallContext.ts` has since been DELETED (checked 2026-08-19) — the row's conclusion was right and its path is now a ghost; the hook had zero importers (it appears in exactly one file, the one defining it). Nothing mounts it, so the function never fires. Not a plan builder; listed so the hook is not mistaken for a live surface.

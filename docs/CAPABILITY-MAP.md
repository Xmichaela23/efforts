...
|---|---|---|---|
| **HR zones (Friel)** | `src/lib/friel-zones.ts:36` (Z2 = 85–89% LTHR → **128–134** @ LTHR 151) | ❌ **NO** | 🟡 **LATENT — corrected 2026-07-13 by looking at the app.** The live account is **CORRECT**: stored bins are Z2 128-135 / Z3 135-143 (half-open) = the canon. The analyzer's **Priority 1 is `configured_hr_zones`** — deliberately, *"so debrief zone references match exactly what every other surface shows"* — and those are Friel 0.89. **But two divergent tables exist and are one condition away:** `_shared/endurance/hr-zones.ts:18` Z2 ceiling **0.90** → **136** (used by `generate-run-plan`; dormant only because the current plan prescribes PACE, not HR) · `analyze-running-workout:1030-1033` a **non-Friel** 0.75/0.85/0.92/0.98 model, Z2 tops at **128** and **threshold caps at 148 — below a real LTHR of 151** (fires only when `configured_hr_zones` is **missing** → **a brand-new user**). **D-286 fixed three copies of this seam. There were five.** |

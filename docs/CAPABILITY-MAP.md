...
| **"Place my lifting around my long run, my club night and my rest days."** Takes endurance pins + day/kind/label/`canSplitDay`, returns placed lifts, rest days and named compromises. Heavy legs first, 48h clear of long days, 24h of quality, stacking only where the athlete SAID they can split (Robineau 2016). | `shared/strength-system/place-week.ts` (**12 tests**) | **ZERO IMPORTERS.** `strength-primary-plan.ts` still lays out a hardcoded Mon/Tue/Thu/Fri grid — **that grid is the outlier, not this file.** ⚠️ Its rules are re-expressed as **D-325 §5's directional penalty pairs** (amended 2026-07-26); the capability is unchanged and still unreachable. ⛔ **It does NOT have hard walls and it already never returns empty** — it emits `compromises[]` (`:123-127`). D-325's original "walls" rationale is **RETRACTED**; keep the `compromises[]` contract. It shares its clearance law with the optimizer (`:25`), so §0.6's "three placement authorities" reads better as **two placers on one law, plus a grid that reads none.** |
...
| **The three words (how a set felt)** | `StrengthLogger` → `strength_exercises[].sets[].difficulty` → `compute-facts` → `workout_facts.strength_facts[].difficulty` | ✅ WIRED 2026-07-30 (D-338). Replaces RIR on 5/3/1 — one tap, heaviest set, optional. ⛔ The FACT exists; **State does not render its consequence yet** (a block of Grinds at week-1 % = training max too high) |
| **The all-out set (the measurement)** | `set_plan[].amrap` → logger (blank reps + "all out") → `strength_facts.amrap_reps` / `.measured` | ✅ CAPTURED 2026-07-30 (D-338). ⛔ **NOTHING ACTS ON IT.** `verdictFrom95Set` is written and called by nothing; the bar still climbs by calendar. This is the next job — see the ENGINE-STATE banner |
...
| **Which protocol a plan is on** | `config.strength_protocol` (run/tri) **OR** `config.source === 'strength_primary'` | ⛔ **TWO ANSWERS, AND THE COACH KNOWS ONLY ONE.** `materialize-plan` handles both; `coach` does not, so State cannot identify a Strength Focus block. Audit F3 / **Q-230 Part A** — fix the WRITE side, one resolver, unknown means silent |
| **Does State know the GOAL type?** | — | ⛔ **NO.** `goal_type` exists on the goal and never reaches the payload's plan slice, so chasing speed and chasing distance both arrive as "run: develop". **Q-230 Part B** — do not ship the field without a reader |
| **Block identity card** | `_shared/block-identity.ts` → `coach` payload `plan.block` + `session_detail_v1.block` | ✅ SHIPPED 2026-07-30 (D-340). Which protocol / goal / week / cycle / deload / measurement week / how effort is read. ⛔ Read-only — nothing that reads it may move a session or change a weight |
...
| **A run's intent (steady vs hard)** | planned row → `compute-facts` `classifyRunIntent` → `run_facts.workout_type` | ✅ SHIPPED 2026-07-30 (D-345). ⛔ Was NEVER WRITTEN, so every run was excluded from the efficiency/decoupling read and the trend sat 16 days stale |

...
| **FTP** | ✅ `src/lib/resolve-current-ftp.ts:62` | 4 stragglers routed 2026-07-18; `normalizer.ts` deferred | 🟢 **routed:** `athlete-snapshot/identity.ts` (the LLM prompt — coach no longer voices a different FTP), `get-week` (week watts), `course-strategy` (bike leg), `PlanSelect`. ⏸️ **`normalizer.ts:308/898` (plan-watts display) DEFERRED** — needs `learned_fitness` threaded through the `Baselines` type + its 5 callers (own pass, low stakes: display only). |

...
| capability | entry point | status | note |
|---|---|---|---|
| **THE wrapper** — create a goal, build + activate a plan | `create-goal-and-materialize-plan/index.ts:2115` (`Deno.serve`) | BUILT | ⟨A31⟩ every plan path goes through here except `PlanWizard`. **Survivor census 2026-08-07: `docs/AUDIT-plan-generators-2026-08-07.md`** — read it before editing anything under `generate-*/generators/` |
...
| Run race plan | `generate-run-plan/index.ts:51` (`Deno.serve`); approach switch `:280` | 🟢 **LIVE — 3 callers** | only `sustainable` (`:282`) + `performance_build` (`:300`) are switched on; `default` 400s (`:305`). ⛔ **This — not `generate-combined-plan` — is what a SINGLE marathon goal actually builds** (`create-goal…:3786`). Also reached by the non-race run retest head (`create-goal…:2801`) and **directly by the client** (`src/components/PlanWizard.tsx:858`, route `App.tsx:55`) — that third door writes a plan with **no goal row and no `activate-plan`**. Its own phase structure (`generators/base-generator.ts:274`), own recovery weeks (`:411`, −30% volume `:640`), and it does NOT route through `week-optimizer` (audited 2026-08-03) | ⛔ **REBUILT 2026-08-06 — the marathon block is now a PRESCRIPTION with a computed prerequisite; read `ENGINE-STATE.md` → "HOW A MARATHON BLOCK IS BUILT, END TO END" before touching any number in it.** Long-run arc `src/lib/run-volume-tables.ts` (`buildLongRunArc`, `marathonPrerequisiteFor`, `longRunPeakWeek`) — shared with the intake, so both quote one arc. [D-392]…[D-398]
...
⚠️ Add (census 2026-08-07): **`generate-overall-context`** — technically it *has* a caller, `src/hooks/useOverallContext.ts:80`, but **`useOverallContext` has zero importers** (it appears in exactly one file, the one defining it). Nothing mounts it, so the function never fires. Not a plan builder; listed so the hook is not mistaken for a live surface.

# OPEN-QUESTIONS — ARCHIVE (Q-001 → Q-129)

Split out of `OPEN-QUESTIONS.md` on **2026-07-13** (the live file had grown to 423KB).

**These still matter.** The point of this doc is to stop a session from "fixing" a behaviour that someone already considered and chose to leave. Grep here before changing anything that looks intentional.

⚠️ **A Q is a LEAD, not a verified bug report.** And ⚠️ **several entries here are STALE** — they were fixed and never closed (Q-100 hyrox, Q-136, Q-138 among them; each is now marked at the top of its entry).

**Live questions (Q-130 →) are in [`../OPEN-QUESTIONS.md`](../OPEN-QUESTIONS.md).**
...
> ⚠️ **STALE — "wanted direction, not built" IS FALSE. THE HYROX AND GLUTE BIASES ARE SHIPPED AND USER-SELECTABLE.**
> Add a goal → Get Stronger → strength posture `develop` → pick **Hyrox** or **Glutes** (`src/components/NonRaceBuilder.tsx:395`). Engine: `shared/strength-system/strength-primary-plan.ts:193` (station rotation: sled push / farmers carry / sandbag lunge / sled pull), `:402` + `:495` (the fatigued-legs station combo after the long run), `:459-472` (+1 accessory on Upper A). Shipped as **D-225, 101/101 tests**. Explainer card at `GoalsScreen.tsx:1652`.
>
> **Superseded by Q-103 the very next day** — which correctly records what shipped (the Hyrox *movement-familiarity* bias, honestly labelled) and what did NOT (a real Hyrox **program**: the bias misses high-intensity endurance, power/RFD, and equipment-gated stations. *"Hyrox is a real PROGRAM, not a +1 accessory delta."*). **Q-103 is the live entry. This one is history.**
>
> This entry told a 2026-07-13 session that hyrox did not exist, and `CAPABILITY-MAP.md` had no hyrox row at all — a shipped feature was invisible to both. *(Back-annotated 2026-07-13.)*
...
- **Status: SPLIT (corrected 2026-07-04) —**
  ...
  - **(2) The ~5.5% aerobic-gain READING: RESOLVED 2026-07-11 — it was an ARTIFACT, now fixed (D-275-bike).** Verified against Michael's real rides (SQL pull, all 27 completed rides): the "improving" was NOT fitness. The efficiency substrate ate **every ride type** — so (a) a May CLIMBING block (144–153 bpm in-band, hard rides where in-band time is incidental) inflated the mid-series, and (b) one "endurance" ride RIDDEN at threshold (May 30, 165W ≈ 94% FTP, 145 bpm via cardiac lag) faked the rest. Gating to steady-aerobic rides (type ∈ {endurance, endurance_long, recovery} + ≥600s in-band dwell + best-20 below the Coggan Z4 floor / 90% FTP) → the read on his data flips **"improving −5.5%" → "holding −1.1%"** (the truth). Fix in `assemble.ts` (efficiency substrate gate) + `bike-fitness.ts` (`bikeEfficiencyRideEligible` / `bikeRideIntensityAerobic`) + `in_band_s`/`band_hi` threaded through `compute-snapshot` + `useStateTrends`. The **second engine** — the coach's BIKE 7d HR-drift row — now shares the SAME intensity gate (coach v79) so a hard-ridden ride can't pollute it either. Fixtures: `bike-efficiency-hr-filter.test.ts` (incl. the May-30 contaminant repro). Deployed + pushed.
...
- **the COACH — BUILT + DEPLOYED 2026-07-11 (commit `77392eb4`, coach v77).** The premise "coach unguarded" was STALE: the week headline already ran through `runGuardedNarrative` (rules 6/7/5/8/10 vs the spine). Two real holes found by code trace + closed: (1) `atypicalSignals` was hardcoded `[]` at `coach/index.ts:~4682` → **rule 2 (don't call the week "comfortable/steady/cruising" while a signal is atypical) could never fire.** Now fed the CONCERNING spine verdicts (any discipline `sliding`, e.g. the AERO "durability gap" row) — derived from the SAME verdicts the rows render, so the one headline can't contradict its own screen. (2) **Cold-start bypassed the guard entirely** (`spineVerdicts.length ? guarded : generate(null)`) → a data-less athlete got an unguarded narrative; now ALWAYS runs guarded. Also extended the shared `ACK_ATYPICAL` lexicon (`narrative-core/validate.ts`) with weekly decline words (sliding/slipping/declining/fading/worth-watching) so an honest "run is slipping" headline that also says "easy" isn't falsely flagged (the old list only knew per-session drift words). **Fixtures:** 2 coach-net cases (cruising-while-sliding fails rule 2; plain/acknowledged headline doesn't over-fire); 29/29 narrative-core green (workout surface unaffected). **Live:** the WK headline recomputed clean (not dropped, reads human) with AERO "durability gap" on screen — the honest-headline no-over-fire case, confirmed on device (1 of the ≥3 recompute discipline; 2 more owed to fully close). Deployed coach + the 3 analyzers bundling narrative-core.
- **Scope note (confirmed correct):** the coach's only AI-written surface on State is **ONE sentence** (the WK headline); everything else is deterministic/glass-box. Design posture right vs incumbents (Whoop sweet spot, better decomposed). Do NOT add more prose to State. **The foundation is now honest → building the State coaching OUT later is safe** (Michael's sequencing intent, met).
- **STILL OPEN (the generalized version, not blocking):** a fully `_shared` honesty pass every generator *formally* subscribes to (one fact-contract + assertion-set object) remains a future consolidation — today wired the coach into the existing guard + populated its signals, which achieves the honesty outcome without the formal shared-interface refactor.

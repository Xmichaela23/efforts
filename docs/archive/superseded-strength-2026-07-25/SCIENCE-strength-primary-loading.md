...
## The RETEST — AMRAP, one reusable tool (D-224)
The block **must end with a higher 1RM than it started** — that is the entire promise; the wk12 retest enforces it. It is an **AMRAP** test: warm up, then ONE all-out set at a **fixed ~88%** with **open reps**.
- **Why AMRAP, not a prescribed rep-max (the D-223 bug):** the old retest prescribed a fixed `88% × 3` and computed off the OLD max → Epley `0.88 × 1.10 = 0.968` → it back-projected ~97% of the entered 1RM **every time**, mathematically incapable of showing a gain (it logged a guaranteed ~3% loss). AMRAP holds the weight fixed and **opens the reps** — getting stronger shows up as *more reps* → higher e1RM. Cannot force a loss.
- **The estimate — cluster Epley + Brzycki, ≤10 reps (CITED [6]):** `Epley = w×(1+r/30)`, `Brzycki = w/(1.0278−0.0278r)`, averaged; reps **capped at 10** (accuracy degrades above ~10; Brzycki's denominator collapses). Best at 3–6 reps → the ~88% test weight targets that zone. **All equations underestimate the deadlift**, so the deadlift retest copy flags "reads conservative — a flat number isn't necessarily a flat lift."
- **Accuracy-critical framing:** the estimate assumes a set taken **to/near failure**, so the copy lands on **"stop at ~RPE 9"** (≈1 rep in reserve) — near enough to hold accuracy, safe enough to do solo. Not "stop early."
- **ONE tool, two jobs:** the SAME guided AMRAP session **establishes** baselines (entry / no-1RM → athlete picks a ~5-rep weight; standalone "Baseline Test: Lower/Upper/Full Body" in Strength Baselines) and **re-measures** them (exit / wk12 → ratchet-UP-only write-back → next block compounds). Closes the no-1RM entry gap and block-to-block compounding on one surface. The write-back may only **raise** a stored 1RM, never lower it (D-223 guard).

[6] **LeSuer, D.A., McCormick, J.H., Mayhew, J.L., Wasserstein, R.L., & Arnold, M.D. (1997).** "The Accuracy of Prediction Equations for Estimating 1-RM Performance in the Bench Press, Squat, and Deadlift." *J Strength Cond Res* **11(4):211–213.** Error within ~3% for 2–10 reps; Epley/Brzycki best at low reps; all equations underestimate the deadlift. *(⚠ The commonly-passed "PMID 9355611" is a different, unrelated paper — cite by journal/vol/pages.)*

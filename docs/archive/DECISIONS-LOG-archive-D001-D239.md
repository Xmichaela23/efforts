# DECISIONS-LOG — ARCHIVE (D-001 → D-239)

Split out of `DECISIONS-LOG.md` on **2026-07-13** (the live file had grown to 830KB and no session could read it).

**These are still authoritative.** A decision here is still in force unless a later `D-NNN` supersedes it. Grep this file before reversing anything.

⚠️ **Known rot:** entries here were never back-annotated when later work overturned them. If an entry looks live, verify it against code before trusting it. (The clearest case: **D-275** — reversed by D-283 — sat for two days presenting a dead decision with its full justification intact. It has since been marked.)

**Live decisions (D-240 →) are in [`../DECISIONS-LOG.md`](../DECISIONS-LOG.md).**
...
- **Date:** 2026-05-22 (spec `30e88008`; implementation `ad4102f8`; Phase 0 of the feedback-loop closure work order documented in `docs/FEEDBACK-LOOP-WORKORDER.md` + spec doc `docs/archive/PHASE-0-ARC-CHANNEL-SPEC.md`. Prerequisite for Phases 1-4 — D-033 through D-036.)
...
- **Date:** 2026-05-23 (spec `docs/archive/UNLINKED-WORKOUT-INTERPRETATION-SPEC.md`; implementation this commit. Bundled all three analyzers in one ship.)
...
- **Decision:** D-035 (Run + cycling + swim analyzers ship together. Spec `docs/archive/UNLINKED-WORKOUT-INTERPRETATION-SPEC.md` approved 2026-05-23. 9 new Deno tests, 400 _shared + 19 cycling tests all green, zero regressions. No client changes. Redeploys: `analyze-running-workout`, `analyze-cycling-workout`, `analyze-swim-workout`, `workout-detail`, `recompute-workout`, `bulk-reanalyze-workouts`, `ingest-activity`.)
...
Spec: docs/archive/PERF-COMPARISON-POOL-SPEC.md
...
Alternatives rejected: see archive/PERF-COMPARISON-POOL-SPEC.md §3. Key: pace_delta
...
Per archive/SESSION-CONTEXT.md §3 cosmetic-deferred note + Q-010: cycling
...
- **Decision — two specs are now architecture of record (committed, not drafts):**
  ...
  - **`docs/archive/BUILD-SEQUENCE-spine-foundation.md`** — the build order: P0 shared-primitive relocation (client→server, one impl) → P1 truth-reconciliation audit (read-only) → P2a bike-fitness signals (terrain-binned power + HR-at-power) → P2b per-ride HR@power read → P2c narrative→spine (Part B/C/D) → P2d load/readiness fold-in → **P3 close-loop-to-adjustment (GATED, separate sign-off — autoregulation)**.
...
- **Date:** 2026-06-16
- **What:** built `_shared/narrative-core/` — the shared 7-rule SCAFFOLD + the shared VALIDATOR SUITE, single-sourced and parameterized by per-discipline ADAPTERS (no `if discipline ==` monolith). Migrated RUN onto it (first of four). This is the same invariant as D-185/D-186 applied to *reasoning* instead of *numbers*. Standard: `SPEC-universal-narrative-inference.md`; plan: `archive/WORK-ORDER-narrative-core.md`.
- **The core:**
...
- **Cross-ref:** `archive/SPEC-strength-island-phase1.md`, `archive/ISLAND-PROPOSAL.md`, `archive/STRENGTH-SCOUT-REPORT.md`, `archive/STATE-OF-BOARD.md`; tests `retest-behavior.test.ts`, `retest-tail.test.ts`.
...
- **Closes:** open thread **T-3** in `archive/STATE-OF-BOARD.md`.
- **Cross-ref:** `ISLANDS-ORIENTATION.md`, `archive/SPEC-non-race-run-retest.md`, D-213/D-214 (annotated superseded-in-part), the stash (`F-9 provisional combined single-sport cut` — step-1 for bike).
...
- **Cross-ref:** `archive/SPEC-e3b-bottom-up-volume.md`, `ISLANDS-ORIENTATION.md`, E3a zones (`94f1c58f`), the spine (`_shared/endurance/`). Deferred: faders supply the budget; completion-race volume move (SPEC §7); the day-count lever; bike consumes `rideHrs`. See OPEN-QUESTIONS Q-091.
...
- **Cross-ref:** `archive/AUDIT-app-synthesis-2026-07-02.md`, `archive/AUDIT-spine-conformance-2026-07-02.md`, `AUDIT-state-screen-2026-07-02.md`, `SCREEN-CONNECTIVITY.md`, D-149/D-150/D-151/D-213, Q-106/Q-107/Q-108.
...
- **Cross-ref:** D-234, `readiness-scale.ts`, `archive/DESIGN-soreness-input.md`.

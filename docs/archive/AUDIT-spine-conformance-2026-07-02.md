...
| `body_response` (load_status, weekly_trends) | **rebuilt** via `buildBodyResponse` from the raw ledger (`coach/index.ts:3277`) for the main load verdict, instead of reading the persisted `body_response` column — note this column IS NOT in the §3 SELECT and IS read elsewhere, but only for a UI load-bar fallback (`coach/index.ts:3568-3591`, `4781`). So: **rebuild-instead-of-read, not fetch-and-ignore.** | ❌ PARALLEL |
...
**The most actionable finding:** the coach doesn't lack the data — it **shadows** it. It recomputes its own ACWR while `snapshot.acwr` sits fetched-and-unused; recomputes strength trend while `strength_volume_trend`/`strength_top_lifts` sit unused. (`body_response` is a *different* case — **rebuilt** via `buildBodyResponse` at `coach/index.ts:3277` rather than fetched-and-ignored; it's not in the §3 SELECT, and the persisted column IS read for a UI load-bar fallback at `3568-3591`/`4781` — the fix there is "read the persisted column instead of rebuilding," a call-site retirement, not a one-line column read.) For the genuinely-shadowed columns (ACWR, the trend scalars) a large slice of the fix is literally *"read the columns you already fetch, and delete the parallel derivation"* — exactly the D-151 move.
...
2. **Move coach verdicts onto the spine, axis by axis — start with "read the columns you already fetch."** The coach fetches `acwr`, `strength_volume_trend`, `strength_top_lifts`, `rpe_trend`, `run_easy_pace_at_hr_trend` and ignores them (SELECTed at `coach/index.ts:2715`, never read). For each: read the cached column, delete the parallel derivation (the D-151 move). **`body_response` is a separate, larger case** — it is *rebuilt* via `buildBodyResponse` (`coach:3277`), not fetched-and-ignored; the fix is to read the persisted column and retire the rebuild call site (reconciling with the load-bar reads at `3568-3591`/`4781`). Order by athlete-visibility: readiness/FATIGUED and the strength verdict first (they're the ones misrepresenting today).

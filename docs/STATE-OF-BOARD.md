...
- **T-3 — routing tension D-213/D-214 vs the shipped path. ✅ RESOLVED 2026-06-28 by D-218.** D-218 supersedes D-213/D-214 (in part) for single-sport run non-race: it routes through `generate-run-plan` (the b-run fork), not the combined engine, because combined can't produce a single-sport week. D-213/D-214 are annotated superseded-in-part; both still govern non-race tri. Institutional record now matches the shipped architecture.
  - **Re-confirmed 2026-06-29** by an independent read-only trace (Q-088 producer work): run-shaped non-race returns from the `(b)-run` block (`create-goal…index.ts:2369-2421`) **before** the combine block ever runs. **Residual cleanup owed (code, not record):** the inline comment at `create-goal…index.ts:2178` still reads "non-race goals MUST route through the combined engine" — stale per D-218; the (b)-run fork overrides it. The record is correct; only the code comment lags. Fix the comment when next in that file.

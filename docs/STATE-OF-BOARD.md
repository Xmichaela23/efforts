...
- **T-3 — routing tension D-213/D-214 vs the shipped path.** D-213/D-214 say non-race routes through the combined "one engine"; the live run non-race path is `generate-run-plan` with a retest head (the deliberate (b)-run fork). ENGINE-STATE flags generate-run-plan as a "standing exception." This deserves an explicit superseding decision (a new D-NNN) so the institutional record matches the shipped architecture.

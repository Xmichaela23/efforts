...
>
> ✅ **DONE 2026-08-15 — [D-434].** The field now carries `reason: 'reset' | 'bump'`, the training maxes either side of the change, and an `undone_at` stamp; `reason: 'ceiling'` is gone with the ceiling. ⛔ **And it gained a second job this entry did not anticipate: it is the UNDO record.** The rematerializer is a pure function of the stored max and the logged sets, so an undo that only restored rows would be recomputed away on the next save — `undone_at` is what makes the athlete's refusal outlive the recompute. Type and helpers: `shared/strength-system/loading/calibration.ts`.
>

...
- **Refresh = honored-swim-gated nudge** (STATE screen only — NOT Performance; Michael 2026-06-18: it's a fitness-marker insight, belongs with the markers, not in the baseline editor) — BUILT 2026-06-18 (`efd72f8a`): helper `src/lib/swimBaselineNudge.ts` + `useSwimBaselineNudge` + State card + `swimPace100_updated_at` stamp. Markers now also honor the popup uncheck (`swim_facts.swam_as_planned`). Dormant until a 4-week honored window accrues:

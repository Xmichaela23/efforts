...
- **D-180 → D-195, 2026-06-17:** swim rest-fraction norm model (in/below/above-band read in the narrative). Planned feature; noted for traceability. On-device confirm pending.
- **D-196, 2026-06-17:** swim delivery — (1) FORM Goggles rest-per-rep fix **shipped to main**; (2) Send to Apple Watch via WorkoutKit **built, Xcode-compile-pending** (on branch `feat/d196-swim-delivery`); (3) breakout equipment chip built (web-verified). Item 2's on-device path intentionally does NOT inherit the `send-workout-to-garmin` client-`userId` trust flag (§3 item 11). Planned features; noted for traceability.

**New discrepancy surfaced (2026-06-17, during D-195) — for human review:**
- **`planned_workouts.session_type` and `hardness` are NULL on all swims** (`intensity` is an empty `{}`). Session intention for swims lives only in `tags` (`technique_swim`, `easy`/`aerobic`, `css_aerobic`/`quality`, …) and free-text `description`. D-195 derived intent from tags as a result, but the underlying gap is real: the plan generator does not populate the structured `session_type`/`hardness` columns for swim sessions (likely other disciplines too — unverified). Anything keying off `planned_workouts.session_type` for swims gets null. **Flag only; not acted on beyond D-195's tag-based workaround.**

...
| S3 | **CLOSED 2026-09-07** — `public.jobs` + `claim_jobs()` (migration `20260907070000`), `run-jobs` on pg_cron every minute (migration `20260907090000`), `ingest-activity` enqueues `recompute-workout` + `adapt-plan` (`enqueueOrCall`, falls back to the direct call if the insert fails); 1 / 5 / 25 min backoff, 3 attempts, then `failed` + alarm. Verified: `scripts/_plumbing-verify-2026-09-07.mjs` t1–t3 after the paste. ~~**Ingest fan-out: no queue, no retry, no dead-letter.**~~ ~8 fire-and-forget `fetch` calls; failures only `console.error`; a dropped analyze leaves `analysis_status:'pending'` forever. Thundering herd + partial-failure accumulator at scale. | **Serious (scale)** | `ingest-activity/index.ts:1433-1680`; `compute-facts/index.ts:1794-1821` | **NEW — file** |
...
| B4 | **CLOSED 2026-09-07** — `_shared/alarm.ts` `raise()` → one Resend email per kind per 15 min (`ALARM_TO_EMAIL`) + every alarm in `public.alarms`; `withAlarm()` wraps the four analysers, `compute-workout-analysis`, `recompute-workout`; the two webhooks and `run-jobs` call `raise()` on a 5xx / final job failure. ~~**No error sink.**~~ Zero Sentry/structured logging; 455 `console.error` into ephemeral edge logs; no error table in 97 migrations. If a user's compute breaks you never know — this turns every other bug into a *silent* bug. Reuse the `notify-admin-signup` Resend wiring for a `notify-admin-error`. | **BLOCKER** | no error table; `notify-admin-signup` exists, no error analog | **NEW — file** (only "considered", DECISIONS-LOG:2120) |
...
| B3 | **CLOSED 2026-09-07** — `health` / `last_error` / `last_ok_at` on both connection tables (migration `20260907080000`), written by `_shared/connection-health.ts` from `strava-webhook`, `garmin-webhook-activities`, `send-workout-to-garmin`, `import-strava-history`, `import-garmin-history`, `swift-task`, the shared Strava refresh; reset on reconnect (`bright-service`, `strava-token-exchange`). Connections shows "Reconnect ›"; Home shows one line under Today. The `import-strava-history` rotation bug below is still open. ~~**Sync dies silently; no connection-health state.**~~ No `status`/`needs_reauth`/`last_error` column on `user_connections`/`device_connections`. A dead token looks healthy; cardio sync just stops. Plus a rotation bug: `import-strava-history` overwrites `connection_data` with stats, dropping the rotated refresh_token → next server refresh breaks permanently. | Serious | `strava-refresh/index.ts:58-64`; `import-strava-history/index.ts:880-891` | **NEW — file** |
...
| B7 | **CLOSED 2026-09-07** — `get-week` + `workout-detail` return `analysis_status` / `analysis_error` / `analysis_updated_at` (trigger-stamped); `src/lib/analysis-state.ts` turns them into "Analysis failed at <step>: <reason>." / "Analysis did not finish." (analyzing > 10 min) with "Try again" on the Performance card, and a dot on the Home row + week chip. `athlete_snapshot` still has no status column. ~~**Failure illegible to the user.**~~ Analyzers write `analysis_status='failed'`/`analysis_error` but the client reads them NOWHERE; `athlete_snapshot` has no status column. User can't tell "computing" from "failed" from "genuinely low week." | Serious | write at `compute-workout-analysis:2018`; zero client reads | **NEW — file** |
...
| B10 | **`weekly_workload` table has `user_id` but no RLS.** **CLOSED 2026-09-07** — `supabase/migrations/20260907010000_weekly_workload_rls.sql` (owner-only policies; Michael pastes it). Latent cross-user read if ever queried with the anon client (server-only today). The one gap in an otherwise consistent RLS picture. | Cleanup (latent) | migration `20250115000002` (no `ENABLE ROW LEVEL SECURITY`) | **NEW — file** |
...
| B14 | **Weather licence — Open-Meteo's free API is non-commercial.** Every weather reading in the app comes from Open-Meteo's free endpoints (`api.open-meteo.com/v1/forecast`, `archive-api.open-meteo.com/v1/archive`), whose terms forbid commercial use. Before launch: subscribe to Open-Meteo's commercial plan (API key, same endpoints) or switch provider. The licence also requires an on-screen credit: `Weather by Open-Meteo` is on Today's header block (added 2026-09-10); the session Details weather (`WeatherDisplay.tsx`) carries none yet. (Michael, 2026-09-10.) | **LAUNCH GATE** | `get-weather/index.ts:496, 507`; `_shared/fetch-race-weather-archive.ts` | **[WORKORDER-garmin-strava-attribution-2026-09-09 §7]** |

...
| FTP determination | ✗ Multi-source drift risk | Two live storage paths (manual + learned), no enforced precedence, different consumers pick differently, one **dead fallback path** still in code. |
...
| Plan-generation FTP reads | ✗ Drift risk | Plan generators read `performance_numbers.ftp` live, not snapshot. Athlete updates FTP mid-plan → described workouts use old value, delivered targets use new value. |
...
| Legacy code surfaces | ✗ One dead path + dormant fields | `compute-facts:1115` reads `learned_fitness.cycling.ftp` which is never written. FIT-file and Garmin device FTPs extracted but never persisted. |
...
1. **(blocker, single-source)** Establish FTP precedence rule. Pick one canonical "current FTP" — probably `learned_fitness.ride_ftp_estimated` when confidence ≥ medium, else `performance_numbers.ftp`. Centralize the resolver (~1 helper file). Replace every consumer's ad-hoc fallback with the helper. Same architectural pattern as `useStrengthOrderingPreference` consolidation. Affected consumers: 8 sites enumerated above.
2. **(drift risk)** Implement `bike.ftp_w` snapshot pin in `athlete-snapshot.ts:163`. Match the running/strength pattern. Pin at plan-creation time so plan descriptions and delivered targets stay in sync even if baselines change. Bonus: re-materialization trigger if pinned FTP drifts >X% from current.
3. **(legacy cleanup, low risk)** Remove the dead `learned_fitness.cycling.ftp` fallback at `compute-facts:1115`. Trivial.
...
What's NOT tested:
- The dual-source FTP fallback chains. No test asserts which value wins when both manual and learned are present with different values.

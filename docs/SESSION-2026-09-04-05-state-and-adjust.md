...
## Easy pace is a readout from the athlete's runs (2026-09-05)

Michael: the 10:32 "from threshold" was wrong for him (his easy runs are 12:40 at 134 bpm, all in 75–82°F). Research: Daniels easy = 1.21–1.30 × threshold, Friel Z2 = 1.14–1.29; our 1.19 is a table's fast edge. The products prescribe easy by heart rate and show pace as a readout. Now: the learner's easy pace is the median of the FIVE MOST RECENT easy runs; the resolver returns it first (source `learned`, not an estimate) and falls back to threshold × 1.19 below five; Adjust shows "12:40/mi · from runs" with the note "Your last five easy runs. Heat and hills move it."; the easy steps' reference pace follows the same resolver. No accept step. Heat is deliberately not factored (D-283). Also this pass: the step list prints the heart-rate range first on easy steps with the book's talk-test line once per session; the MLSS row text is the page's prescription in substance with no effort number; Rebuild restates run/ride rows.

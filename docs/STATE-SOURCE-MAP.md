...
|---|---|---|---|
| **RUN · aerobic base** (durability / decoupling) | `workouts.workout_analysis.heart_rate_summary.decouplingPct`, **seeded from `workouts.workout_analysis`** (`compute-snapshot:686`) — ⚠️ **NOT** `route_progress_metrics`; that moved 2026-07-21 and this row said otherwise until 2026-07-31 | ⛔ **"non-steady type" NEVER FIRED** — `workout_type` is `steady_state` on all 25 runs, so hill sessions counted. Real gates: `basis='raw'` · <20 min · implausible % | **Staleness yes** ("last steady run Nd ago"). **Exclusions no.** |
...
⚠️ **What remains true, and it is worse than "not read":** `effort_adjusted_pace_sec_per_km` — the one column the routes table actually owns — is populated on **8 of 164 rows**. And `temp_f` is null on **49 of 164**, disproportionately the recent ones, because `compute-facts` writes the route row from `workouts.weather_data` while the weather is fetched by `analyze-running-workout`, which runs *afterwards* and fire-and-forget. **The heat de-confound regression is starved by a race, not by a missing feature** (measured 2026-07-31).

...
- **2026-07-03 (session close) — START HERE NEXT: Q-111** (Axis 3 plan/history-aware strength verdicts — design doc `DESIGN-Q111-plan-history-aware-verdicts.md` drafted, awaiting Michael's review; note §6 cross-training double-count is now CLOSED by D-236 Part C) **and Q-112** (Axis 2 narrative claim-grounding audit). Also open/deferred: Q-116 (EWMA ACWR option, post-nothing-blocking), the Q-111 §8 STRENGTH-label cosmetic reconciliation (low priority), and Axis 1's real-data live positive (data-gated on Michael logging soreness/RPE). Nothing else in flight. Read this map → the target Q in `OPEN-QUESTIONS.md` → `DECISIONS-LOG.md` D-236.
- **2026-07-04 — Axis 3 grounding HARDENED (no new axis built); track PARKED during a substrate/input-honesty arc.** The strength-verdict grounding is now **end-to-end confirmed**: Q-107 **H1 fixed** — the verdict cites the typed **150 anchor** all the way to the render (`resolveStrengthCapacity` `coach:2346` → `per_lift.anchor_1rm` `:2358` → `response-model/weekly.ts` verdict `:230` + suggested-weight `:249` both consult it → `StateTab:1526`; `COACH_PAYLOAD_VERSION` v47 cache-bump; fixture `weekly-strength-verdict.test.ts`). This firms up Axis 3's grounding but does NOT advance the *active* tone reasoning (still Q-111). **No axis reached 'built' this session** — the self-awareness track was parked while a substrate/input-honesty arc ran: **D-237** (estimate-provenance guard — no fabricated fallback may flow into a user-facing verdict undeclared) + **D-238** (output-first cardio load ladder; TRIMP/resting-HR retired, deployed forward-only), on top of D-234/235/236. That arc hardens the load/soreness INPUTS the axes reason from; it adds no new axis. **START HERE NEXT unchanged: Q-111 → Q-112.** Axis 1's live positive still data-gated on Michael logging soreness/RPE.

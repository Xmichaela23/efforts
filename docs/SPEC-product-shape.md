...
2. **Then scope the CONDUCTOR** — the phase→protocol sequencing layer that runs the arc (base→power→sharpen) instead of one flat protocol. Shape-only for review; the protocols all exist — this is **orchestration**, not invention. (Audit gap #3 + #4: the strength 1RM-retest terminal.)

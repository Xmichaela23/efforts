...
- **Five post-deploy end-to-end checks** (against deployed code, **throwaway test user only — never real user `45d122e7`**):
  ...
  5. **builder-path materialization** (Cut B): navigate `/goals/build`, submit the minimal non-race goal (length + confirm) → a plan **materializes** (the shell → `complete()` → server short-circuit → `buildCombinedPlan` chain works end-to-end against deployed code; no local wrapper oracle). Confirms `target_weeks` reached the goal row (B3 persist + B1 forward) and the non-race route fired.

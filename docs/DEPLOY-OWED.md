...
- **Five post-deploy end-to-end checks** (against deployed code, **throwaway test user only — never real user `45d122e7`**):
  ...
  5. **builder-path materialization + seed** (Cuts B+C): navigate `/goals/build`, pick a goal (e.g. **"Get stronger"**) → length → confirm → a plan **materializes** AND reflects the **seeded posture** (swim:out → 0 swim sessions; strength:develop → `upper_aesthetics` 5×5/hypertrophy sessions, not durability) (the shell → `seedFromGoal` → `complete()` → server short-circuit → `buildCombinedPlan` chain, end-to-end against deployed code; no local wrapper oracle). Confirms `target_weeks` reached the row (B3+B1), the non-race route fired, and the Cut C seeds + Cut A posture wiring took effect. Cross-check a runner-only profile once real `athleteDisciplines` is wired (Cut D): no swim/bike sessions.

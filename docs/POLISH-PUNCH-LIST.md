...
- [ ] **⛔ AUDIT the State posture-aware reads (Michael asked explicitly; next fresh session).** Client display reframe on top of the engine's verdicts. Check: (1) e1RM / decoupling verdicts underneath are sound, not noise; (2) the strength read (getting stronger / on plan / gains flat / easing off) matches the field (RTS/RIR, plateau=3–4wk); (3) thresholds grounded — `planWeek≥4`, 28-day active window; (4) recompute vs Michael's real data (LoadBar audit held clean earlier). Details: ENGINE-STATE banner + D-302 + this session's transcript.
- [ ] **RIR / deload fatigue layer for the strength read (owed — D-302 slice 2).** Today the strength read only sees the e1RM plateau; "on plan" can hide grinding. Wire `strength_rir_below/above_prescription` (`longitudinal-signals.ts`) so RIR dropping / weight dropped → "grinding, deload may be due" (the autoregulation concern, separate trigger from the e1RM plateau).

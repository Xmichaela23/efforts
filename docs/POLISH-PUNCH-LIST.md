...
- [ ] **Merge the branch to main** (fast-forward from `291b3147`), then deploy the 33 functions listed in the
      ENGINE-STATE banner (transitive importers of the changed `_shared` files), then run `backfill-strength-load`
      so stored strength points move to Friel's scale.
- [ ] **Look for on State:** LOAD line reads `fitness N · fatigue N · form ±N zone` (no "balanced", no ACWR);
      ...
      composition strip says "last 7 days". Coach: no "ACWR", no "balanced", no "improving / sliding" anywhere
      in its copy; the week title is the Form zone word.

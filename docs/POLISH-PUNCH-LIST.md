...
- [ ] **D-325 — Session Cost Ledger + Penalty Scheduler.** Full contract in DECISIONS-LOG. Build order: the ledger + scorer as a PURE module with tests first (cost table, ceilings, reconciler deltas, penalty scoring, breach output) — nothing touches the composer until that is green. Then it takes over placement and `place-week.ts`'s hard rules come out. ⚠️ Blocked-adjacent: **Q-205** (two of the three emphasis ceilings are unvalidated).

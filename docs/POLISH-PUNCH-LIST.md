...
- [ ] **Performance tab — interval / variable-effort detection in INSIGHTS narrative + TREND filter (run + cycling).** When a session shows high pace variance (≳90s/mi range across segments, observed example: 7 segments with paces 7:45 → 10:07 /mi and HRs 140-167 bpm on a 3.5mi / 33min run), the INSIGHTS narrative currently treats it as a steady aerobic effort and compares average HR to recent same-route sessions — *"Your HR ran 16 bpm higher than your recent similar efforts on this route ... a sign of elevated cardiovascular demand"* — when the actual structure was an interval workout. The PACING row in the SAME view DOES correctly detect the interval structure (*"Work intervals sped up 63s/mi (10:02 → 8:59)"*), so the signal exists; the bug is INSIGHTS not consuming it. TREND should also filter or flag sessions with significantly different effort profiles on the same route (interval vs easy run) to avoid mixing apples and oranges. Applies symmetrically to cycling — variable-effort rides currently get compared as steady aerobic. Fix shape: a variance gate (pace-range OR HR-range OR `classified_type` mismatch across the comparison set) that either (a) tags the comparison set as mixed-effort or (b) excludes high-variance sessions from the trend baseline. **Queue after run arc closes** — run arc Phases 0-3 are now closed (D-023, 2026-05-21), so this is ready to pick up. Touch surfaces: `_shared/session-detail/build.ts` (INSIGHTS narrative builder), `analyze-running-workout` / `analyze-cycling-workout` (TREND series + same-route comparison), `SessionNarrative.tsx` (chart filter on the client side).

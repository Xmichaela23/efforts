...
- [ ] **Deload work sets prescribe below the empty bar** (seen on device — week-4 OHP 30×5, 40×5 on a 45 lb bar, with plate chips). The bar floor at `strength-primary-plan.ts` `mainLiftRow` (~line 701) applies to warmups only, on the stated reasoning that a work set below the bar implies a near-empty-bar TM the athlete would notice. Deloads break that assumption: 40/50/60% of TM with no warmup ramp, so any TM under ~112 lb authors unliftable sets. Wendler's own deload guidance is that the exact weight doesn't matter — likely fix is flooring deload sets at the bar too (or dumbbells), but that's a generator change: decide, then sweep which cycles/lifts it touches.

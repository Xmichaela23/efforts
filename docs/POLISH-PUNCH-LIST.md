...
- [ ] **`aerobic_direction` unwired into workout INSIGHTS (Q-023).** Computed in compute-snapshot, labelled in longitudinal-signals, reaches coach surface and plan generation — never routed into run analyzer signals block. Fix-shape: new field on fact-packet display format + mixed-effort-aware prompt rule. Pick up after `run_easy_hr_trend` rename (naming first, then wire).
...
- [ ] **Tri-generator swim sessions don't respect beginner rotation (D-025 §10.3 substitution).** `generate-triathlon-plan` is the legacy path (`CLAUDE.md` Topology — "Plan generation is fragmented"); it uses `raceTwoSwimRotationSlotMeta` directly without the `athleteFitness === 'beginner'` dispatch that `generate-combined-plan/_shared/swim-program-templates.ts: getSwimSlotTemplates` adds via `raceTwoSwimRotationSlotMetaForBeginner`. So a learning swimmer who happens to land on the tri-generator path gets `[threshold, race_specific_aerobic]` instead of the beginner-substituted `[css_aerobic, technique_aerobic]`. **Deliberately deferred in D-020** (swim-arc note: "Tri-generator's swim drill picks unaffected by tier biasing — legacy path"). Now ready to queue. Fix: thread `swimFitness` (athleteState.swim_fitness or derived) into the tri-generator's swim-rotation call site(s), dispatch on `'beginner'` to the beginner meta the same way `getSwimSlotTemplates` does. The 6 `pickSwimDrillInset` call sites in tri-generator already got `athleteFitness` threading for drill picking in D-020 Slice 3d — same pattern, extended to rotation dispatch.
- [ ] **Q-015 — drill repeat-pick memory** (rotation salts can collide across weeks; foundation-drill picks may repeat on consecutive sessions for beginners since `pickSwimDrillInset` has no memory of prior selections). Fix-shape locked in Q-015: in-memory-per-build posture (no persistence; tracked across a single `generateAllWeeks` pass via opts). Cross-ref: `docs/OPEN-QUESTIONS.md` Q-015.
...
- [ ] **Coverage gap — `analyze-running-workout` has no direct test file.** D-034's variance gate logic (`_varGate` predicate at `analyze-running-workout/index.ts:~2040`, `buildRowsFromBreakdown` at `:~4040`, `'Overall session'` literal removal at three sites `:4014/4209/4277`) is covered transitively via `_shared/session-detail/variance-gate.test.ts` (humanize labels, GAP-aware resolver, pool filter, isMixedEffortRow) — but the analyzer's end-to-end gate computation has no pinned coverage. `analyze-cycling-workout/index.test.ts` exists (19 tests) and demonstrates the pattern. Not blocking — the components ARE pinned, just not the integration. Fix-shape: add `analyze-running-workout/index.test.ts` with at least: (a) linked-plan interval session → `is_mixed_effort=true`, `variance_signal='interval_execution'`; (b) hilly easy run with GAP available → `is_mixed_effort=false`, `pace_cv_basis='gap'`; (c) flat fartlek without plan link → `is_mixed_effort=true`, `variance_signal='detected_intervals'` OR `'pace_cv'`; (d) linked-plan easy that trips variance → `classified_type='easy'` + `classified_type_variance_override=true`; (e) unplanned 6-interval session → mode `'interval_compare_ready'`, 6 rows, no `'Overall session'` literal. Filed 2026-05-23 alongside D-034.
...
- [ ] **Segment label formatting bug** — steady-state single-segment sessions render raw pace-range string ("81:00 @ 10:56-11:…" / "10:56-11:22/mi") instead of a clean label. Fartlek/interval sessions show clean "Interval 1-7" labels — different code path. Client-side display bug, no server changes needed.

...
- [ ] **Coverage gap — `analyze-running-workout` has no direct test file.** D-034's variance gate logic (`_varGate` predicate at `analyze-running-workout/index.ts:~2040`, `buildRowsFromBreakdown` at `:~4040`, `'Overall session'` literal removal at three sites `:4014/4209/4277`) is covered transitively via `_shared/session-detail/variance-gate.test.ts` (humanize labels, GAP-aware resolver, pool filter, isMixedEffortRow) — but the analyzer's end-to-end gate computation has no pinned coverage. `analyze-cycling-workout/index.test.ts` exists (19 tests) and demonstrates the pattern, but the cycling tests target a single exported helper (`generateCyclingAdherenceSummary`), not the full analyzer pipeline. **D-043 item 7 attempt 2026-05-25 — blocked on refactor:** `_varGate` is an inline IIFE inside the request handler, not exported. Writing the 5 pinned scenarios (linked-plan interval / hilly easy with GAP / flat fartlek unplanned / linked easy that trips variance / unplanned 6-interval) requires extracting `computeVarianceGate({analysis, fact_packet_v1, isLinkedPlanSession, classifiedTypeKey, intervalsToAnalyze, plannedWorkout})` as an exported pure function first. Extraction is ~75 lines of analyzer hot path; touches a load-bearing surface. Punted from the autonomous batch — needs deliberate refactor work with its own pin tests for the pre-extraction behavior so the extraction is verifiably no-op.

...
export function validateClaimsGrounded(
  ...
  // T1 — trailing FITNESS-STATE claims that dodge the direction list above (a verdict said with a
  // different word). Overloaded: "held the 150-167 W target" is EXECUTION, not fitness — so require
  // a fitness/efficiency SUBJECT within a short span; only then is holding/responding/consolidating
  // a fitness claim that must trace to the spine verdict.
  const fitStateRe = /\b(?:aerobic efficiency|aerobic control|efficiency|fitness)\b[^.]{0,45}\b(?:holding|responding|consolidat\w+)\b/i;
  const m = String(summary || '').match(dirRe) || String(summary || '').match(fitStateRe);
...
- ANSWER "SO WHAT?" — don't just state findings, explain them. After normalized power / the trend: name what drove it (climbing, intervals, pacing, group ride). After the intensity read: say whether that was the right intensity for the ride type ("appropriate for a climbing day" vs "harder than your endurance target"). After the heart-rate-vs-power read: describe the HR–power relationship for THIS ride in plain words ("heart rate stayed controlled as the power held"); name a fitness DIRECTION only when cross_workout.trend is present — do NOT infer one ("efficiency is holding", "fitness is responding") from a single ride.
- NO SPECULATION beyond the packet — this leaks in the CLOSING/trailing clause, so police it there. (a) PHASE: do NOT guess the training phase or race framing ("suggests you're in a base-building phase rather than a taper") — name the phase ONLY from plan.week_number / plan_phase in the packet, never infer it from one ride. (b) PROJECTION: do NOT assert future outcomes ("will pay dividends", "will unlock faster returns", "move the needle", "the foundation you need"). (c) ALARM: do NOT escalate ("risks injury", "your body is signaling rest is the priority"). (d) FITNESS DIRECTION: do NOT claim one ("efficiency is holding", "fitness is responding/consolidating") unless cross_workout.trend is present. The trailing clause may add ONLY packet-grounded context — phase from the packet, a fatigue note in plain words — never a projection, diagnosis, or guessed phase. Describe THIS ride; stop where the data stops.
- LIMITER FRAMING — when cross_workout.limiter is present you may STATE the limiter as a fact ("bike fitness is a limiter for the 70.3 distance"), but do NOT prescribe or project how to address it ("sustained FTP and long-ride work will unlock the most return", "move the needle", "pays dividends"). Naming the limiter is grounded; projecting its fix is NOT — that's the plan's job, not this ride's summary. End on the limiter statement, not a training recommendation.

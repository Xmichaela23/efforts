...
Deno.test('run + bike both holding → holding, 2 contributors', () => {
  const r = rollupHrResponse(v1({ run: { verdict: 'holding', newestAgeDays: 3 }, bike: { verdict: 'holding', newestAgeDays: 5 } }));
  assertEquals(r.verdict, 'holding');
  assertEquals(r.contributors.map((c) => c.discipline), ['run', 'bike']);
  assertEquals(r.newestAgeDays, 3); // freshest contributor
});
...
Deno.test('bike-only (run needs_data) → reads the bike verdict', () => {
  ...
  assertEquals(r.newestAgeDays, 2);
});

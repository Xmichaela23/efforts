...
function buildActivePlanSummary(
  ...
  // D-039 Fix 3: fallback to config.phases when plan_contract_v1.phase_by_week
  // is missing. Plans generated by generate-combined-plan write the phase
  // structure to `config.phases` as `[{ name, start_week, ... }]` covering
  // the whole plan. Pre-D-039 only plan_contract_v1.phase_by_week was checked,
  // so plans without that field resolved phase=null → mode='unstructured_read'
  // even on a clearly-active build week (8cbfa389 / plan 1c3bfb92 symptom).
  if (phase == null && weekIndex != null && Array.isArray(config.phases)) {
    const phases = config.phases as Array<{ name?: unknown; start_week?: unknown }>;
    const sorted = phases
    ...
    // Find the last phase whose start_week ≤ weekIndex.
    let matched: string | null = null;
    for (const p of sorted) {
      if (p.start_week <= weekIndex) matched = p.name;
      else break;
    }
    if (matched) phase = sanitizeUserFacingPhaseLabel(matched);
  }

...
const namesIn = (w: ReturnType<typeof week>) =>
  w.sessions
    ...
    .flatMap((s) => (s.strength_exercises ?? []).flatMap((e) => [String(e.name), String((e as { execution_name?: string }).execution_name ?? '')]))
    .map((n) => n.toLowerCase())
    .filter(Boolean);

/** The movements the screen showed, and the class they belong to. */
const NEEDS_A_STATION_OR_INCLINE = /preacher|spider|incline|chest[- ]supported|rear delt machine|pullover machine/;
...
Deno.test('⛔ THE DEVICE FINDING: dumbbells and a flat bench are not handed a preacher curl, a spider curl or an incline movement', () => {
  for (const wk of [1, 2, 3, 4]) {
    const names = namesIn(week(FLAT, wk));
    const hits = names.filter((n) => NEEDS_A_STATION_OR_INCLINE.test(n));
    assertEquals(hits, [], `week ${wk} prescribed a movement the kit cannot do: ${hits.join(', ')}`);
  }
});

Deno.test('the swap sheet for the arm cells offers only what the kit reaches', () => {
  for (const key of ['iso_pull_a', 'iso_pull_b'] as const) {
    const offered = pickOptions(key, FLAT, null, null).map((o) => o.name.toLowerCase());
    assert(offered.length > 0, `${key} offered nothing on dumbbells + flat bench`);
    const hits = offered.filter((n) => NEEDS_A_STATION_OR_INCLINE.test(n));
    assertEquals(hits, [], `${key} offered ${hits.join(', ')} on dumbbells + flat bench`);
    for (const n of offered) assert(canPerform(n, FLAT), `${key} offered "${n}", which the kit cannot perform`);
  }
});

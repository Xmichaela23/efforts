...
/** One pacing group as course-strategy-build produces it and course_segments stores it. */
export type DisplayGroup = {
  ...
  /** Null when the athlete has neither configured zones nor a max heart rate. */
  target_hr_low: number | null;
  target_hr_high: number | null;
  coaching_cue: string;
};

export type ParsedStrategy = { display_groups: DisplayGroup[] };

const EFFORT = new Set(['conservative', 'cruise', 'caution', 'push']);

/** Self-check on the built groups: full segment coverage, ordered bounds, lengths. */
export function validateDisplayGroups(
  parsed: unknown,
  segmentCount: number,
  maxHr: number | null,
): { ok: true; data: ParsedStrategy } | { ok: false; error: string } {
  ...
  for (const g of dg) {
    ...
    if (o.target_hr_low != null || o.target_hr_high != null) {
  ...
  return { ok: true, data: parsed as ParsedStrategy };
}
...
export function materializeSegmentRows(
  ...
  return geometry.map((geo) => {
    ...
    return {
      ...
      target_hr_low: g.target_hr_low == null ? null : Math.round(g.target_hr_low),
      target_hr_high: g.target_hr_high == null ? null : Math.round(g.target_hr_high),

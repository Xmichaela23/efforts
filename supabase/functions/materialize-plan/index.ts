 ...
 *   · HARD steps carry their PACE and nothing else — the page's percentage of threshold (p229–235).
 *
 * ⛔ NO EFFORT NUMBER ON ANY RUN OR RIDE STEP (Michael, 2026-09-17). The run and ride pages prescribe a
 * percentage of threshold and nothing else, so a planned effort band had no page behind it: first an OURS
 * 5–6 / 8–10 (ledger row 201), then Foster's CR-10 (7264c3e9) — a field scale, not the book's. Both are gone.
 * The book's own effort words go on the two sessions that print them, as whole lines under the steps
 * (`_shared/planned-step-lines.ts`): the talk test on VT1 and LSD (p235), "all-out" on Sprint / Power (p229–231).
 *
 * ⚠️ OURS: which token shapes count as easy. Labelled below. Strides carry no zone — they are
 * neuromuscular, not an easy run.
...
export function toV3Step(st: any, row?: any): any {
   ...
   * times (`load_basis`, `slot_intent`, …). `stampRunPrescription` put `prescription` and `hr_range`
   * on the expanded step, and this function rebuilt the step without them, so the

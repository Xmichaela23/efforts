...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // SWIM = FACTS ONLY (2026-07-19, D-304 principle). Watch/goggle swim data is too unreliable to
    // interpret honestly (equipment, lap detection, set type), so the read is the numbers: distance,
    // pace, intervals — the fixed sentences in `insights` below. No model has ever written this block;
    // the model path that used to sit here was gated off and is deleted (2026-09-07, no-AI work order).

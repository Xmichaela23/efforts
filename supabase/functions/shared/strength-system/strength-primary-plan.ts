...
export function composeStrengthPrimaryPlan(args: StrengthPrimaryArgs): {
  ...
  return {
    ...
    description:
      `${weeks} weeks of Wendler's 5/3/1, four lifting days: bench Monday, squat Tuesday, ` +
      `overhead press Thursday, deadlift Friday. ${leaders} building cycle${leaders === 1 ? '' : 's'} ` +
      `then one measuring cycle from week ${anchorStart}, each ending in a deload week. ` +
      // The "say once" line (SPEC §4). Flat, once, never repeated and never apologised for.
      `Weights come off 85% of your max, and that buffer is what makes the last set of every ` +
      `third week worth measuring. Week one sits well inside you by design.${enduranceNote}`,

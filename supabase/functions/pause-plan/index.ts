...
/**
 * pause-plan edge function
 * 
 * Pauses a training plan by:
 * 1. Setting plan status to 'paused'
 * 2. Recording when it was paused (for resume date calculation)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Update plan status to 'paused' and record when it was paused
    const { error: updateErr } = await supabase
      .from('plans')
      .update({ 
        status: 'paused',
        paused_at: today.toISOString()
      })
      .eq('id', planId)
    ...
    return new Response(
      JSON.stringify({ 
        ...
        paused_at: today.toISOString(),
        message: `Plan paused. Removed ${count || 0} future planned workouts.`
      }), 
      { headers: { ...cors, 'Content-Type':'application/json' } }
    )
  } catch (e) {
    console.error('Error pausing plan:', e)
    return new Response(

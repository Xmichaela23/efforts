...
// The Sep 3 shape. ⚠️ NO CLIMB HERE: the climb is NOT a fact-table field any more — it reads off
// `workouts.elevation_gain`, the one column both sports write (2026-09-03). `ride_facts` never had it,
// which is why reading the copy gave a ride nothing and, before the workout-id key, gave it the run's.
const RUN_ID = '11111111-1111-1111-1111-111111111111';
const RIDE_ID = 'b33cd7b4-5953-44f7-955d-288b8c542248';
const sep3Run = {
  workout_id: RUN_ID, date: '2026-09-03', discipline: 'run',
  run_facts: { efficiency_index: 1.41, hr_avg: 151, hr_drift_pct: 3.2 },
  ride_facts: null,
};
const sep3Ride = {
  workout_id: RIDE_ID, date: '2026-09-03', discipline: 'ride',
  run_facts: null,
  ride_facts: { efficiency_factor: 1.11, avg_hr: 134, hr_drift_pct: -7.2 },
};
...
Deno.test('same date, ride then run: order of rows cannot change the answer', () => {
  ...
  assertEquals(idx.get(RIDE_ID)!.efficiency, 1.11);
  assertEquals(idx.get(RIDE_ID)!.hr, 134);

...
${narrativeFacts.join('\n')}`;
    ...
    const weekExecutionV1: NonNullable<CoachWeekContextResponseV1['weekly_state_v1']>['week_execution_v1'] = (() => {
      ...
      const doneArr = (Array.isArray(weekWorkouts) ? weekWorkouts : []).filter(isDone);
      ...
      const plannedByToday = (disc: string) =>
      ...
      const shortfalls = counts
        ...
        .map((c) => ({ discipline: c.discipline, gap: plannedByToday(c.discipline) - c.done }))
        .filter((c) => c.gap >= 1) // behind what was DUE BY NOW, not merely early in the week

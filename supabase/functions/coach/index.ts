...
const COACH_PAYLOAD_VERSION = 179; // 179: THE LIFTING CARD READS THE PLAN'S WEEK (2026-09-01, approved by Michael — "is this a rolling week?"). `state_trends_v1.display.viadaWeek` is cut on `weekStartOf` + the plan's start-day — the SAME boundary as the planned-vs-actual bar — instead of a rolling as-of-minus-six; `since` is now the plan week's first day. `weekChange` gains `basis` / `from` / `to` and compares only CLOSED plan weeks (last week vs the week before while the week is open; this week vs last on its final day). ⛔ COMPUTATION CHANGE: a cached row carries rolling-window numbers under a "this week" label and passes the `cachedVer >= COACH_PAYLOAD_VERSION` gate — the trap v175–v178 each hit on this object. Without this bump the window does not move.

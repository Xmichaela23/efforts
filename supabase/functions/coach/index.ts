...
${narrativeFacts.join('\n')}`;
    ...
    const weekExecutionV1: NonNullable<CoachWeekContextResponseV1['weekly_state_v1']>['week_execution_v1'] = (() => {
      ...
      const accent: WeekAccent | null = composeWeekAccent([
        ...
        leverCandidate(),
        anchorDescent,
        // upkeepCandidates REMOVED from the accent (2026-07-21) — the COACH'S EYE row now owns the
        // maintain-under-target FLOOR read (with the same number), so keeping it here would be TWO
        // running voices a few inches apart (Michael's "one story, one place"). The upkeep computation
        // still lives above; its FACT (run under target) now feeds composeCoachEye's floor flag instead.

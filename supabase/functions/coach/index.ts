...
import { loadComposition7d, formZoneRows, formHeadline, formKicker, recoveryKicker, isRecoveryIntent, loadChartCaptions } from './load-composition.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const form_by_day: Record<string, { form: number; label: string | null }> = {};
    ...
    try {
      for (let i = 1; i <= DAY_SERIES_DAYS; i++) {
        ...
        if (ff.form != null && Number.isFinite(ff.form)) form_by_day[d] = { form: Math.round(ff.form), label: formZone(ff.form) };
      }

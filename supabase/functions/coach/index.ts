  ...
  try {
    ...
    // can't contradict the PERFORMANCE run/bike reads.
    {
      ...
      if (hrResp.verdict !== 'needs_data' && hrResp.contributors.length > 0) {
        ...
        const parts = hrResp.contributors.map((c) => `${discLabel(c.discipline)} ${partVerdict(c.verdict)}${c.provisional ? ' (limited data)' : ''}`);
        ...
        const asOf = hrResp.newestAgeDays != null ? addDaysISO(asOfDate, -hrResp.newestAgeDays) : null;

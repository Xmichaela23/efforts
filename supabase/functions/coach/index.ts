...
const COACH_PAYLOAD_VERSION = 177; // 177: §B5's CHANGE RULE REACHES THE CARD (2026-09-01, Round 3 addendum item 4, approved by Michael). `state_trends_v1.display.viadaWeek` gains `weekChange` — this rolling seven-day window's lifting buckets against the seven days before it, only the buckets that moved by MORE than 10% (`WEEK_CHANGE_FLAG_PCT`, `accessory-dosing/dose.ts`), resolved on the spine; the card prints the list. A stated `comparable` flag separates "no prior work to measure against" from "nothing moved". ⛔ COMPUTATION CHANGE: a cached row serves a `display.viadaWeek` with no `weekChange` and passes the `cachedVer >= COACH_PAYLOAD_VERSION` gate — the same trap v175 hit on `perLift` and v176 on `patternBandApplies`. Without this bump the field lands nowhere.

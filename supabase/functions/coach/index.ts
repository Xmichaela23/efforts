  ...
  try {
    ...
    // can't contradict the PERFORMANCE run/bike reads.
    {
      ...
      if (hrResp.verdict !== 'needs_data' && hrResp.contributors.length > 0) {
        ...
        const bikeOpp = bikeStale ? ' A steady ride with power refreshes the bike side.' : '';
        ...
        const provenance = `From the spine: ${parts.join(' · ')} — run = aerobic decoupling, bike = HR-at-power. Swim excluded (in-water HR isn't reliable).${excludedRun ? ` ${excludedRun}` : ''}${bikeOpp}${baseSlip}`;

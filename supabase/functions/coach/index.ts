  ...
  try {
    ...
    // can't contradict the PERFORMANCE run/bike reads.
    {
      ...
      if (hrResp.verdict !== 'needs_data' && hrResp.contributors.length > 0) {
        ...
        const detail = hrResp.verdict === 'improving' ? 'settling — lower HR at the same effort' // D-318 copy: was "HR easing…"; "easing" collided with the efficiency verdict word "easing off" (opposite valence, adjacent on State)

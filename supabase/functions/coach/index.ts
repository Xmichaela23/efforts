  ...
  try {
    ...
    // can't contradict the PERFORMANCE run/bike reads.
    {
      ...
      if (hrResp.verdict !== 'needs_data' && hrResp.contributors.length > 0) {
        ...
        // NO SILENT DROP: a run present but below the 8-run trend floor is excluded from the rollup —
        // name it so the read doesn't look stale/lagging while fresh runs sit just under the bar (F-hr).
        const excludedRun = hrResponseExcludedRunNote(latestSnapshot?.state_trends_v1, hrResp.contributors);
        const provenance = `From the spine: ${parts.join(' · ')} — run = aerobic decoupling, bike = HR-at-power. Swim excluded (in-water HR isn't reliable).${excludedRun ? ` ${excludedRun}` : ''}`;

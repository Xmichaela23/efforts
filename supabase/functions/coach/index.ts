...
import { mondayOfToday } from '../_shared/parse-local-date.ts';
import {
  getAcwrRiskFlag,
  getAcwrStatus,
...
import { computeFitnessFatigue, formZone, formZoneText } from '../_shared/fitness-fatigue.ts';
// The LOAD chart's window and its two-point minimum — the same 12-week window as every State chart (2026-09-25).
import { TREND_CHART_MIN_POINTS, TREND_FIT_MAX_WEEKS } from '../_shared/state-trend/trend-fit.ts';
// Audit 2026-09-10 (item 17): the load plate's shares, zone table and headline, and the logged-sets split.
import { loadComposition7d, formZoneRows, formHeadline, formKicker, recoveryKicker, isRecoveryIntent, loadChartCaptions, formKeyText } from './load-composition.ts';
import { buildStrengthLoggedSets } from './strength-logged-sets.ts';
// Audit 2026-09-10 (H-B10): the finish after race day, off complete-race's race-day pick.
import { buildPostRaceUnofficial, postRaceCandidate } from './post-race-unofficial.ts';
...
// overload verdict instead of a soft signal count.
...
// Q-254 slice 1 — the all-out (AMRAP) top set, read by the SAME function the Performance screen's
// ALL-OUT SET card calls, so State and Performance cannot disagree about one session.
import {
  type LastAllOutByLift,
  lastAllOutByLift,
...
/** The app's floor (`src/lib/coach-contract.ts`) is this same number, from `_shared/coach-payload-version.ts`. */
...
/** v161 [D-373]: COACHING LANGUAGE IS MAIN-LIFTS-ONLY. `computeLiftVerdict` now gates on
 *  `isMainBarbellLift` and returns an EMPTY label for anything else, so a hard-feeling accessory
 *  (Hip Thrust, Barbell Row) no longer prints "back off weight" — a command Strong and Hevy never
...
// ⛔ COACH_PAYLOAD_VERSION (now 219) LIVES IN `_shared/coach-payload-version.ts` — bump it there; the app's floor reads the same number. History: // 219 (2026-09-26): the LOAD key's words come from the coach (`load.form_key`, approved by Michael) and every form zone prints ONE name (`load.label_text`, `form_by_day[].label_text`, the table's `meaning`, `form_headline`, the loaded-legs Why — `FORM_ZONE_TEXT`); a 218 row carries Friel's words and no key. // 218 (2026-09-25): `load.fitness_fatigue.series` (each day of the 12-week chart window — fitness, fatigue and form off the same whole-history walk as the card's three numbers), `series_caption` (each line's actual first and last day, through the card's own rounders, `loadChartCaptions`) and `draw_series` — State's LOAD card opens to TrainingPeaks' Performance Management Chart. A cached 217 row has none of the three and the card has no chart to open. // 217 (2026-09-18): `state_trends_v1.display.viadaWeek.perSession[].display` — each lifting day spelled out in the book's terms ("Dynamic Effort day, upper body"), which State's weekly card prints; a 216 row has none and the card falls back to the raw day name. // 216 (2026-09-18): `load.week_time_line` (the plan week so far, time per sport) — State's load card; the planned-vs-done bars are off it. A cached row has no line and the card prints none. // 215 (2026-09-18): `trends.applied_run_threshold` / `_word` / `_hr` and `trends.run_easy_target` — State's run row heads with the threshold in use, its word, its heart rate, and the easy row carries the plan's easy range. A cached row has none and the run row prints the easy row alone. // 214 (2026-09-17): `load.form_by_day` and `load.body_line_by_day` — a past day's own form and BODY line for the Today card, 84 days back. A cached row has neither and a past day's space stays empty. // 213 (2026-09-17): BODY is numbers only — no verdict word or colour on effort / soreness, `soreness_flag` and `readiness_rpe_driver` no longer sent; `load.body_today_line` is BODY's one line on Today. A cached row still carries the words and has no Today line. // 212 (2026-09-17): `trends.applied_ftp_word` — where the FTP came from, by Adjust's rule (`ftpSourceWord`); State's bike row prints it and the phone's tested/estimated pick is deleted. A cached row has no word and the row prints the number alone. // 210 (2026-09-15): `trends.applied_ftp` — the FTP the app RUNS ON (`resolveCurrentFtp`), so State's bike row prints the typed number when there is one instead of the learned estimate the anchor carries (§8.0 #22). A cached 209 row has no field and the row falls back to the anchor, which is the bug. // 209 (2026-09-10): BODY carries its window — `response_model.body_window_label` ("last 7 days"), "usual" says its 28 d, soreness drops its count when it equals the logged count. A cached row has no header window. // 208 (2026-09-10): `load.form_headline` reads "Form −32 · high risk" in every week — a recovery or taper week with form under −30 no longer gets "Recovery • …", and no "(TrainingPeaks)" is printed. A cached row still carries the old words. // 207 (2026-09-10): audit H-B10 — top-level `post_race_unofficial` (the race-day finish State used to find on the phone). A cached row has none and State prints no unofficial finish without it. // 206 (2026-09-10): audit item 17 — `load` gains form_headline, form_zones, total_7d, dominant, composition_7d; `weekly_state_v1.strength_logged_sets`. A cached row has none of them and the screens print nothing without them. // 205 (2026-09-08): empty-State subtitle names the plans.
...
// 177: §B5's CHANGE RULE REACHES THE CARD (2026-09-01, Round 3 addendum item 4, approved by Michael). `state_trends_v1.display.viadaWeek` gains `weekChange` — this rolling seven-day window's lifting buckets against the seven days before it, only the buckets that moved by MORE than 10% (`WEEK_CHANGE_FLAG_PCT`, `accessory-dosing/dose.ts`), resolved on the spine; the card prints the list. A stated `comparable` flag separates "no prior work to measure against" from "nothing moved". ⛔ COMPUTATION CHANGE: a cached row serves a `display.viadaWeek` with no `weekChange` and passes the `cachedVer >= COACH_PAYLOAD_VERSION` gate — the same trap v175 hit on `perLift` and v176 on `patternBandApplies`. Without this bump the field lands nowhere.
// 176: THE TEST-WEEK FLAG REACHES THE CARD (2026-09-01, FIXLIST 2c). `state_trends_v1.display.viadaWeek` gains `patternBandApplies` — false in a test week, because p084's heavy band opens at 90% while the p215 pretest tops out at 86.25%, so the heavy count is STRUCTURALLY ZERO for the whole week and does not resolve once the sessions are logged. The card suppresses the entire heavy/velocity row for that week rather than printing a zero that can never move. Resolved from `isTestWeek` in compute-snapshot — the composer's own constant, one definition, no week number compared to a literal anywhere. ⛔ A CACHED ROW WOULD OTHERWISE SERVE A `display` OBJECT WITH NO FLAG and pass the `cachedVer >= COACH_PAYLOAD_VERSION` gate, so the card would keep printing `0 heavy` with no error — the same trap v175 hit on `perLift` in this same object. Bump. // 175: THE LIFT CHARTS REACH THE CLIENT (2026-08-29). `display.strengthFitness.perLift` now carries a series for every lift with two or more weeks of logged sets — the line is each WEEK'S HEAVIEST SET, so a Viada speed day drops out by arithmetic instead of by an intent gate that (measured, on 178 rows) admitted nothing at all. `liftHistoryWeeks` widened the snapshot's own fetch to a year, so the series is no longer capped at the twelve weeks the reference-max window happened to share. ⛔ A CACHED ROW SERVES THE OLD `perLift` — verified live: the snapshot had 12 lifts with 4 series while the cached coach payload had 0 — and the charts render nothing without it. Bump. // 174:  174: THE ENDURANCE SPINE + SESSION-TYPE GROUPING (2026-08-28, work order items 2 and 3). trends.display.enduranceSpine is NEW and is the PRIMARY endurance read: every run and ride, grouped by session type, with NO plan required — no planned_id, no family tag, no block week map (Q-294: a run is yours whether a plan exists or not). namedSessions stays and becomes the OVERLAY that appears only when a block exists. Run efficiency now reads EVERY run: the 30-70 minute window is gone at BOTH ends (Q-295 closed — the floor dropped two of every three of this athlete's runs, the ceiling deleted the long run a marathon athlete is training for) and non-steady sessions are GROUPED rather than deleted. trends.display.runFitness.efficiency gains `groups` (easy / long / quality, each with its own count, direction and series) and its headline verdict is now fitted on the EASY group alone rather than pooled across every run. Spine points carry `fadeWithheld`: a drift number is not printed for a session that was not a steady effort (Viada's LSD inserts surges and pauses by prescription, so a fade read there fails an athlete for following the book) — the run still feeds the efficiency trend, only the figure is withheld. ⛔ ADDED FIELDS, so a cached row simply lacks them and the new cards render nothing — bump. // 173: THE ENDURANCE READ (2026-08-28) — trends.display.namedSessions replaces namedRunSession and carries ONE PER SPORT: the repeated near-threshold run and the repeated hard ride, each with per-session efficiency read AS STORED (run_facts.efficiency_index / ride_facts.efficiency_factor, which is TrainingPeaks' EF and is not re-derived), hr_drift_pct, and whether a key session falls inside 24h so p107's tighter 5% line applies instead of 10%. The ride also carries a REFERENCE series — dated FTP readings off fitness_baselines, which supersedes rather than overwrites; the run gets none because threshold pace is a single overwritten value and a line must not be fabricated from one number. ⛔ THE FIELD WAS RENAMED, so a cached payload serves the OLD shape and the endurance cards render nothing — bump. // 172: THE STRENGTH READ, PART 1 (2026-08-28) — weekly_state_v1.me_history_v1 carries what every heavy session of the current block was (meSessionOutcome's own clean/mid_band/setback walk, per pattern, in training order) plus the reps held at the current weight. It has been computed inside earnedMeSets on every restate since the ME ladder shipped and the write path kept only the set counts it implied — the one reading in the app that knows whether a heavy session was finished reached the database and stopped. Nothing is recomputed; the card maps outcome to a word at its own edge. Cached payloads carry no such field, so bump. // 171: THE FIVE WEEKLY NUMBERS REACH THE SCREEN (2026-08-28, work order item 1) — weekly_state_v1.week_ledger_v1 carries the current week's endurance minutes (easy / medium-hard / hard, p146 buckets 1-3) plus the week's work-set count and the per-muscle lines, read off plans.config.standing_plan.week_ledgers and forwarded verbatim. Both ledgers have been computed at build since 2026-08-27 and were read by NOTHING (compose.ts:2644 attached the endurance one to the composed week and it died there). Nothing is recomputed; the coach looks the week up and passes it through. Cached payloads carry no such field, so a stale row renders an empty card — bump so they re-source. // 170: PHASE WORD SPLIT (2026-08-24) — block.phase_word now prints 'test' for test-family weeks (phaseDisplayWord, strength-profiles.ts) while the rules still treat them as taper. Cached payloads carry the old word ("week 1 of 12 · taper" on State), so bump to force rebuild. // 169: SLICE 3b — THE e1RM RECORD OBEYS THE TRUSTED-REP CEILING (D-417, applied to one more field). The strength row's "best NNN lb" (`allTimeBestE1rm`) was built by an inline reduce in compute-snapshot whose query selected no `best_reps`, so it could not apply the ceiling the SERIES has applied since D-417 — Michael's deadlift best read 225 (the 105x35 set) above its own 120-150 trusted range. The reduce now lives beside the series builder (`buildAllTimeBestByLift`, state-trend/assemble.ts) reading the same `estimateIsTrusted` gate; `best_reps` added to the query. RECORD VALUES MOVE (deadlift 225 -> 140, squat 125 -> 100, etc.) and `isPr` can flip where trusted history is under 3 readings -> bump. Unknown reps fail open, matching the series. The high-rep set keeps its real home as a REP PR (lastAllOut/isRepRecord) — untouched. ⚠️ REQUIRES A SPINE RECOMPUTE (compute-snapshot). // 168: SLICE 3 / D-420 — THE WEEKLY STRENGTH DIRECTION VERDICT IS RETIRED. No commercial app computes a weekly per-lift strength direction, and on a the previous program wave it reads the within-cycle weight-wave as a trend (Michael's deadlift: 105x35 → 110x25 → 115x20, one cycle, printed "1 lift trending down" + "sliding -8.2%"). Killed at the spine source (`computeStrengthState`, state-trend/strength.ts): per-lift `direction` is always 'needs_data' and `pctChange` null, the aggregate is a no-claim, and `perfByDisc.strength` is now NULL so the discipline card + State headline carry no strength verdict. The weekly "N lifts trending down" headline (weekly.ts computeStrength) states the RECORD instead, and `strength.overall.trend` is 'insufficient_data'. What replaces it: the e1RM RECORD (allTimeBestE1rm, monotonic), REP PRs (new per-lift `lastAllOut` {weight,reps,isRepRecord} off slice 2's all-out capture), and the 12wk CHART — all three already existed. VALUES MOVE + a new field → bump. RIPPLES, stated: strength drops out of computeAssessment's available-signal count, the D-267 e1RM-declining adherence veto goes quiet, and the per-lift '· provisional' hedge disappears (it hedged a direction that no longer exists). D-419's readsEffortAs/all-out INFRASTRUCTURE survives — only the verdict it fed is gone. ⚠️ REQUIRES A SPINE RECOMPUTE (compute-snapshot). // 167: SLICE 2 — STRENGTH PROGRESS READS THE PROTOCOL'S OWN GAUGE. For a block that declares `readsEffortAs: 'amrap'` (the previous program, strength-profiles.ts), a WAVED MAIN LIFT's progress direction now reads the ALL-OUT SET (the previous program), not the working-set e1RM the PROGRAM itself waves 65/75/85 → re-based cycle. A lighter prescribed week was reading as "1 lift trending down". Computed on the spine (`computeStrengthState`, state-trend/strength.ts) off a new all-out series (`allOutSeriesByLift`) that compute-snapshot builds from the SAME 40-session capture the Performance screen uses; coach forwards `state_trends_v1.strength.per_lift[].direction` unchanged. VALUES MOVE: per-lift `direction`/`pctChange`, the aggregate strength trend, and the "N lifts trending down" line — cached payloads hold the old e1RM-gauge verdict until they re-source → bump. New per-lift fields `directionGauge` ('amrap'|'e1rm') + `directionBasis` carry the receipt. Assistance lifts, 'rir' protocols and 'none' are byte-identical. ⚠️ REQUIRES A SPINE RECOMPUTE (compute-snapshot) to take effect — the coach only forwards it. // 166: SLICE 1 — ONE OVERLOAD SOURCE. "Are you overloaded" is minted in exactly one place (`mintOverloadVerdict`, _shared/load-status-reconcile.ts) and every lane reads it: the load word, the BODY/readiness chip, the over-reach accent, the response headline, and training_state. It is plan-aware and fed ONLY by athlete-owned signals — RPE, measured body metrics (HR drift / cardiac efficiency / execution), and actual-vs-PLANNED load. It is blind to absolute ACWR (a ratio the plan itself produces), to prescribed load, and to strength e1RM (a the previous program wave dips it by design). Six lanes used to decide this independently, which is how one payload read `glance on_track` beside "Signs of overreaching — consider backing off", "a bit high", the "needs absorbing" accent and `training_state overstrained` on an ACWR-1.1 ON-PLAN build week. VALUES MOVE for any athlete inside their plan's load: load_status.status, readiness_state, assessment.label, headline.subtext and training_state.code can all drop an alarm → bump so cached rows re-source. New payload field `load.overload` carries the verdict + its basis receipt. // 165: D-417 — the strength e1RM series now excludes sets past the trusted rep ceiling (8 reps, 5 on deadlift), so records/trend/summary/sparkline read only real low-rep strength. The per-lift e1RM VALUES + series MOVE (Michael's deadlift 200→155, squat 125→105, etc.), computed in compute-snapshot's series build (`liftSeriesFromExerciseLog` gate); coach forwards them, so cached payloads hold the OLD inflated numbers until they re-source → bump. Math is untouched (D-339 honest per-set); this only decides what counts as a strength reading. // 164: D-416 — LOAD STAYS OBJECTIVE, READINESS SITS BESIDE IT (matches Garmin/TP/Strava in principle). A single soft signal — one +0.7 RPE reading — set raw readiness 'fatigued', and that RAW value reached the load reconciler (D-260 sole authority) AND the over-reach accent, printing "a bit high" and "needs absorbing before more load" on an ACWR 1.1 week (squarely optimal, 0.8–1.3). The load path now reads the REFINED readiness (`readinessForLoadVerdict`, _shared/response-model/loaded-legs.ts): EFFORT UP / LEGS LOADED / LEGS SORE downgrade to 'normal'; only systemic FATIGUED (elevated ACWR or ≥2 declining signals) or 'overreached' carry into the load verdict. The SAFETY floor (computeSafetyFloor) and the honest `readiness_state` payload stay on the RAW value — this is verdict-only. ALSO un-inverts the BODY RPE tone (`rpeFeelTone`, weekly.ts): "a bit harder" (0.5–1.0) → amber (warning), red (danger) reserved for "noticeably harder" (≥1.0) — the mild bucket used to wear the harsher color, so a +0.7 read as a heavy red flag. `load_status.status` + `visible_signals[].trend_tone` VALUES MOVE → bump so cached rows re-source, else the screen serves the old "a bit high" + red for 24h. ALSO the Adjust-tab `plan_adaptation_suggestions` (deload / add-recovery) read the same systemic-only readiness and skip any week the plan is already easing (recovery/taper/DELOAD — the planned-deload week used to fall through and could suggest a deload mid-deload). ⛔ COACH_CLIENT_MIN_PAYLOAD_VERSION does NOT move with this (raising it early blanked the State section on 2026-08-02). // 162: THE VERDICT READS THE ALL-OUT SET, AND A the previous program LIFT STOPS BEING JUDGED ON RIR (Q-254 slice 2). ⛔ COMPUTATION CHANGE — `per_lift[].verdict_label` and `.verdict_tone` MOVE, so cached rows must re-source or the screen keeps serving the old command for 24h. TWO HALVES. (2a) `target_rir` is now stamped only when `protocolUsesRir(profile)` — `strength_primary` declares `usesRir: false` / `readsEffortAs: 'amrap'` and this line ignored both, falling through to `defaultTargetRir` (2). Worse, `current_avg_rir` falls back to `reaction.avg_strength_rir_7d` when a lift logs no reserve of its own — a SESSION-WIDE average that includes the accessories, which DO log RIR. So a the previous program main lift was compared to a target its protocol says does not exist, using a number borrowed from face pulls, and printed "add weight" / "back off weight" with a TAPPABLE weight suggestion behind it. Fixed at the STAMP SEAM, not by making `getTargetRir` nullable — its own docblock forbids that ("~a dozen callers … the ones that forgot would silently read 0 — 'grind to failure' — the worst possible failure direction"). (2b) `computeLiftVerdict` now takes the all-out set (slice 1 put it on `per_lift`) and reads it through the EXISTING `verdictFrom95Set` (wendler-531.ts:454), which is book-grounded: 0 reps → reset, 1+ → advance ("95% x 1 or more", p23), a big set → advance_untrusted because withholding the advance would punish an athlete who is genuinely stronger. Labels are READ-ONLY status — 'top set met' / 'top set missed' — chosen so `computeSuggestedWeight` returns null for both, which closes the plan_adjustments tap path BY CONSTRUCTION. ⛔ NO WEIGHT MOVES ON DEPLOY: nothing writes a prescribed weight without a tap, the only writer is StrengthAdjustmentModal, and no label this slice can emit yields a suggestion (fixture: "NO LABEL SLICE 2 CAN EMIT WRITES A WEIGHT"). ⛔ AND THE AMRAP DOES NOT RECOMPUTE THE WORKING NUMBER — the previous program keeps the +5/+10 increment and treats reps as feedback (wendler-531.ts:206-208); letting it drive the bar is Q-223. ⚠️ The AMRAP outranks RIR but never the WEEK: recovery/taper/peak still win, so a stale top set cannot overwrite a deload. // 161: THE ALL-OUT SET REACHES STATE (Q-254 slice 1, Michael 2026-08-03: "we need to make state screen read amrap as the north star for stregnth focus right?"). `response_model.strength.per_lift[]` gains `all_out` — the last AMRAP top set per lift (weight x reps, the rep record against the last 40 strength sessions, the previous program's estimated max and its trust flag, and the DATE it was measured), read by `_shared/strength/all-out-set.ts`, which is the SAME function the Performance screen's ALL-OUT SET card has called since 2026-07-30. Not a second implementation: a second answer to "was that a rep record" is how two screens start disagreeing about one session. ⛔ ADDITIVE, NOT A COMPUTATION CHANGE — no verdict, weight suggestion or trend moves in this bump; `computeLiftVerdict` still reads RIR and that is deliberate (slice 2 changes what it reads, after this rendering is seen on a device). The bump exists because coach_cache is keyed on this version and without it the new field never lands, so the row renders exactly as it does today for 24h and the change looks like it did not ship — the trap the notes below have already recorded three times. ⛔ THE CLIENT FLOOR (`COACH_CLIENT_MIN_PAYLOAD_VERSION`) DOES NOT MOVE WITH THIS. It moves only after the server is verified serving 162; raising it early is what blanked the whole State performance section on 2026-08-02. ⚠️ THE WINDOW IS 40 STRENGTH SESSIONS, not the 7d/28d bands the rest of this file uses — a leader cycle and every deload week carry NO all-out set, so 28 days returns nothing on a lift that is measured and climbing. And there is NO fallback to `best_weight`/`best_reps`: that pair is the heaviest set and the most reps at that weight, so a heavy single after the AMRAP reads as 1 rep on a session that went well (Q-254 Gap 1, the trap `cycle-verdicts.ts` already refuses to walk into). // 160: THE BIKE DIRECTION IS GATED AGAINST ITS OWN SCATTER (Q-241, Michael 2026-08-01: "bike should have FTP gains and losses language if the data is there"). Bike was the ONLY discipline whose direction was not checked against its own noise — run durability (`run.ts:299`) and strength e1RM (`strength.ts:191`) both require the early->recent shift to clear ~1 within-window SD; bike cleared only the fixed +/-2.0% dead-band. Best-20-min power repeats to only about +/-5% ride to ride (heat, fuelling, sleep, position, pacing, 1-2% between power meters), so a band-clearing move was not on its own evidence of anything. `noiseGuardStdev: 1.0` now passed at all three bike direction sites: terrain-binned power and HR-at-power efficiency (`bike-fitness.ts`, the STATE row) and `computeBikeState` (`bike.ts`, the session-detail direction). ⛔ COMPUTATION CHANGE, NOT DISPLAY: ungated bike arrows become `holding` and cached verdicts must re-source -> bump. Percentages are untouched; only the verdict moves. GATE FIRST, LANGUAGE SECOND — the bike row stays wordless until this is seen on a device. SAME BUMP, second half: the bike direction now has a RIDE FLOOR (`STATE_TREND_WINDOWS.bikeDirectionMinRides` = 8, DERIVED not felt: a lab 20-min TT repeats at CV ~2.9%, the best-20 inside a real ride is looser, and the typical error of a mean falls with sqrt(n) — so at 2 rides per endpoint the comparison error (~3%) is WIDER than the entire +/-2.0% verdict band, and 4 per endpoint is the first point where a band-clearing move is at least the size of its own error; it lands on run's 8 independently). Below it the verdict is `withheld` and the row reads "too few to read" — bike previously stepped straight from silence (minSessions 4) to a confident arrow, with no state for "I can see some rides, not enough to call it"; run has had that state since 2026-07-16. Michael 2026-08-01: "we can just say that when someone drops a discipline, as long as the engine is right for when they pick back up" — and it is: the window is a rolling 56d, so a returning rider's old rides age out and the read re-forms. 6 is NOT a borrowed number (no commercial app publishes one) — it is run's endpoint logic at bike's lower, bin-split cadence; see the comment on the constant. Selection rule added with it: a QUALIFIED terrain bin outranks a FRESHER under-floor one, and a `withheld` power read no longer hides a usable efficiency read. `withheld` prints NO percent on the row (the percent would be the claim we just declined to make). // 159: THE FITNESS ANCHOR CARRIES ITS OWN NUMBER (Michael 2026-08-01: "every piece needs to know everything"). `FitnessAnchor` gains `value` + `metric` for run, bike and swim. It always HAD the number — `ActiveFitnessBaseline.value` is what the tick is placed from — but the object existed only to answer "where does the tick go", so for any row whose tick could not be placed (bike and swim: the anchor is FTP/pace against an efficiency band, a metric mismatch deferred at assemble.ts:588) the value was computed and dropped one line later. Nothing hid it; nothing asked for it. The bike row now reads "Measured against an estimated FTP of 212 W — not one you confirmed." ⛔ CARRIED, NOT RE-RESOLVED: this is the FTP the verdict was ACTUALLY computed against. A client-side resolveCurrentFtp would return a number that is PROBABLY the same, and "probably the same" is exactly what the FTP-fracture work existed to remove. Payload gains fitnessAnchors[].value/.metric -> bump. // 158: THE SORENESS PERSISTENCE LINE (D-354). When soreness sits above the athlete's OWN normal on 4 of the last 6 logged sessions, the BODY row carries one extra line stating that fact and offering the Adjust tab. It does not diagnose, name a cause, or act — soreness clustering around lifting days is an observation; "your lifting caused this" is a causal claim the same evidence bar that killed the interference verdict rejects. 4-of-6 is the monitoring standard already used by longitudinal-signals.ts against an absolute floor, whose own header says that floor should become baseline-relative once history exists; this is baseline-relative. ADVISORY ONLY — it can never mark the athlete down, because a rough week reported honestly must not cost fitness on screen or the honest reporting stops. Payload gains visible_signals[].soreness_flag -> bump. // 157: THE BODY HEART-RATE ROW IS DELETED (D-354, Michael 2026-08-01). It was a pure ROLLUP of run.efficiency + bike.efficiency — the same two verdicts the RUN and BIKE rows render beneath it — with no number of its own, because speed-per-heartbeat and watts-per-heartbeat cannot honestly be combined. It died in three steps, each removing something it could not support: it could not out-alarm the rows below (D-353), could not restate them (v156), and could not use words (the app-wide no-verdict-word rule). What remained was an arrow duplicating the two rows under it, pointing a direction its contributors split on (Q-238), stamped with a date that hid its stale half (Q-239). ⛔ THE DURABLE REASON IS THE INSTRUMENT: overreached athletes show submaximal HR going DOWN and HR recovery getting FASTER while RPE rises, so an HR-based fatigue read can be actively backwards in the case it is wanted; the subjective measures outperform it (BJSM review, cited in longitudinal-signals.ts). BODY reads REPORTED effort and soreness instead. Within-session fading is a RUN signal — sport-specific, confounded by heat and terrain — and already renders on the RUN row. ROW REMOVED -> bump, so cached rows stop serving it for 24h. // 156: THE BODY ROLLUP STOPS OUT-ALARMING ITS OWN EVIDENCE, AND STOPS RESTATING THE ROWS BENEATH IT (D-353, Michael 2026-08-01). Two changes on the 'Heart-rate response' row. (1) SEVERITY IS CAPPED AT THE CONTRIBUTORS. Its tone was a bare literal — declining -> 'warning' — pinned to what the RUN row looked like when it was written (the comment said so: "WARNING, NOT DANGER, the RUN row renders the same movement in amber"). The RUN row went neutral on 2026-08-01 and this literal did not, so one measurement carried two severities on one screen and the louder one was the row with no metric of its own. `capRollupTone` (_shared/state-trend/severity.ts) makes it structural: a rollup can never exceed the max severity of its contributors, so this row inherits run.efficiency + bike.efficiency and cannot out-alarm them however either is rendered later. (2) THE CONTRIBUTOR CLAUSE IS DROPPED. 'Running easing off (1d ago) - bike holding (17d ago)' was the RUN row and the BIKE row restated two inches above themselves; same redundancy killed in v119 and v131, and it only became redundancy when D-346 pointed this row at run.efficiency. The row now states its OWN verdict and its as-of date; the contributor list moves to the PROVENANCE (one tap), so the ages and the stale-half naming are demoted, not lost. Glass-box holds: the evidence is the two verdict rows rendered directly beneath, adjacent and tappable. COPY + TONE VALUE CHANGE -> bump, because coach_cache is keyed on this version and without it the corrected row never reaches a device for 24h. // 155: THE BLOCK CARD GAINS ITS PHASE WORD (Stage 1 of the 2026-08-01 State pass). `plan.block` has carried the plan's RAW phase name since v150 — 'Leader' / 'Anchor' / 'Deload' — which is the previous program's vocabulary and not something an athlete should ever read on a fitness row. It now also carries `phase_word`, the plain word ('base' / 'build' / 'peak' / 'taper' / 'recovery'), resolved by the card through `normalizePhaseKey` — the SAME table that already decides which effort rule governs the week (D-322), asked a second question rather than copied into a second table. The State fitness rows and the strength Performance surface render it verbatim; neither translates a phase name, so a non-the previous program plan renders correctly through the identical path and a plan that does not place the week renders the week number alone. ⛔ Bumped because coach_cache is keyed on this version: without it, cached rows serve a block card with no phase word for 24h and the rows below print "week 3 of 12" with the word silently missing — which reads exactly like a plan that has no phase, the one thing null is supposed to mean. // 154: ONE VOCABULARY ON THE BODY ROW (Michael, 2026-07-31 — "we need consistant language which this isnt", and on the section: "this whole section really is just how are you handling the training"). The heart-rate row minted its own headline ("drifting up — working harder to hold effort", RED) beside the RUN row's word for the same movement ("easing off", AMBER). Defensible while its run half was DECOUPLING; not defensible once that half became `run.efficiency` — same number, two words, two severities, reading as two findings. It now STATES ITS CONTRIBUTORS in the app's shared trend words ("Running easing off (today) · bike holding (16d ago)") and adds no verdict of its own; tone drops danger→warning to match the row beneath. ⚠️ v144 already had to rewrite this string because "easing" here meant the OPPOSITE of "easing off" below it — twice now from two vocabularies adjacent on one screen. Copy + tone change → bump. // 153: copy — the HR-response provenance printed "(0d ago)" once its run half started reading a pool whose newest run is today. Says "(today)", matching the shared recencyLabel. Server copy change → bump. // 152: THE HEART-RATE ROW READS THE CLEAN NUMBER (D-346). Its run half was `run.decoupling`, still gated on `isSteadyAerobic(workout_type)` — the field that reads `steady_state` on every run ever logged — so a 24.9% hill session sat in its pool and the BODY row said "drifting up — working harder to hold effort" in red on the same screen where the RUN row had already moved to a clean read. It now reads `run.efficiency` (speed per heartbeat, terrain-adjusted, heat-corrected), which answers the row's own question — "am I working harder for the same output" — more directly than decoupling, and unlike decoupling does not need a pristine steady effort it cannot identify. Falls back to decoupling when efficiency has no read. VERDICT VALUE CHANGES -> bump. // 151: FACTS, NOT INSTRUCTION (Michael, 2026-07-31 — "lets not give instruction or diagnosis, facts and at best estimates"). Two clauses deleted from the heart-rate response provenance: "A steady ride with power feeds the bike side" (an INSTRUCTION — the age is already stated one clause earlier) and "Keep the aerobic work this light and your base can slip" (an imperative AND a prediction, computed off running-under-target, which is the CROSS-TRAINING row's fact and is already stated there properly as "Running's at 10 of your 18-mile target — under what holds it"). One subject was told twice and the worse telling sat on a row that does not own volume, hung off the decoupling verdict that still counts hill sessions (D-346). ⛔ Bumped because this is a SERVER COPY CHANGE and coach_cache is keyed on this version — without the bump the corrected row never reaches a device, which is the exact trap the notes below have warned about three times and which I walked into anyway on first deploy. // 150: BLOCK IDENTITY (Q-230 / audit F3+F4+F9) — the payload now carries `plan.block`: which protocol, what the goal is, week-in-cycle, deload, whether this week is the 95% reading, and how the block reads effort (amrap/rir/none). Resolved ONCE by _shared/block-identity.ts, which reads BOTH dialects (`config.strength_protocol` and the strength-primary `config.source`) so a Strength Focus block stops resolving to a null protocol. Two value changes ride with it: the generic "estimated one-rep maxes have been sliding" claim is now suppressed on strength_primary (it prescribes 40->95% by design), and the cross-training CEILING clause no longer prescribes "easing the running" off a verdict the block designed (verdictTrusted). Also feeds strengthProtocol its missing weekInBlock/isDeloadWeek/workingPct — 3 of 6 fields were never passed, so its ceiling and deload clauses could never fire on ANY protocol. Value change -> bump. // 149: WEEK-NARRATIVE #2 (Michael, after the audit) — the "quietly disappearing" clause now fires ONLY for a `develop` discipline (one the athlete declared they're building). Reverses the old "state the fact even without a declared intent" choice that twice put a discipline with no stake on screen (parked bike, off-plan dip). Government of stakes: develop→flag, maintain→upkeep line, dropped/parked/undeclared→silent (matches posture.ts isConcern). Also deleted the dead MIN_SHARE_PCT/share() guard (never wired; the acwr floor subsumes it). Value change → bump. // 148: PARKED-DISCIPLINE fix, take 2 — v147 mapped 'out'→'dropped' but keyed it by the athlete's word ('bike') while disciplines arrive CANONICALIZED ('ride'), so the lookup still missed → still 'unknown' → still named. composeCoachWeekInsight's postureOf now resolves bike/ride/cycling as aliases before normalizing, so a parked bike keyed 'bike' matches the 'ride' discipline and stays silent. Bump to invalidate the v147 rows that still carried "Riding came in below its recent normal". // 147: PARKED DISCIPLINE NAMED (Michael on device) — "Riding came in below its recent normal" fired on a STRENGTH plan where bike posture is 'out' (parked). Cause: the coach's postureMap kept only develop/maintain/dropped, so 'out' fell through → bike posture 'unknown' → the "below its own recent normal" clause (which only skips 'maintain') reported it. Fix: map 'out' → 'dropped' at every posture-map build site AND normalize in composeCoachWeekInsight's postureOf; a parked discipline is now excluded (active filter drops 'dropped'), never mentioned — posture.ts's "an unasked question needs no answer". Value change → bump. // 146: CONTRADICTION FIX (Michael on device) — the week narrative said "Running came in heavier than planned" (this-week load over a small maintenance plan) two inches above the coach's-eye "Running's under what holds it" (28-day upkeep). Same discipline, opposite verdicts. coach-week-insights clause 3 now EXCLUDES maintain-posture disciplines from the plan-adherence over/under line — their story is the trailing upkeep target (D-297/D-130), not this-week plan ratio. Value change → bump so cached rows re-source. // 145: POSTURE DIAGNOSIS voice reset + surfaced (Michael — "you sound like a punitive mail-order bride"). The per-discipline posture sentence (posture.ts, on card.postureSentence) is rewritten fact-first/mechanism-first: the old "You said 3 a week. You've been doing about 1.6 a week. That's a trade, not a mistake." was second-person accusatory with a decimal rate. Now e.g. "Running's at about half the 3-a-week plan. Easy pace drifts slower at lower volume, and picks back up when the running does." — states the volume gap + mechanism + reversibility, names only the observable (never a physiology cause), no "you said", no scold/console. ALSO: this sentence now RENDERS — in the RUN efficiency ⓘ tap-down (StatePerformanceSection.RunFitnessRow), opt-in so it doesn't duplicate the week-execution trade line; the orphaned always-visible PostureLine (F10) is deleted. ALSO — week-narrative voice pass (coach-week-insights.ts, live on the State screen, Michael on device "this is terrible"): "Riding ran below its own recent normal" → "came in below its recent normal"; "Running ran heavier than planned — that load is carried forward, not forgotten" → "came in heavier than planned — it carries into next week's rolling load"; the no-plan down-week reads ("...how a down week is supposed to look" → "the shape of a down week") and the all-clear ("went in on top of it" → "on top") de-consoled/de-idiomed per docs/COPY-VOICE.md. Server copy change → bump so cached rows re-source. // 144: copy — BODY heart-rate response "improving" detail "settling — HR easing at the same effort" → "settling — lower HR at the same effort". "easing" collided with the efficiency verdict word "easing off" (StatePerformanceSection.tsx:20, a deliberate app-wide declining-trend word) — same word, OPPOSITE valence, adjacent on the State screen (Michael). The efficiency word is load-bearing/system-wide and stays; the local HR string (already led with "settling") drops the redundant echo and states the fact. Payload string change → bump so cached rows re-source. // 143: STRAIN IS MULTI-SPORT (D-318) — the D-317 load block reads TOTAL load correctly, but the RECONCILER (sole verdict authority, coach:3564) then re-escalated to 'high' off run-brain STRAIN signals: it counted "RIR declining" as strain (for a strength-primary plan that's the INTENT — pushing toward failure across a build, not overreaching) AND counted "HR drift declining" even when the absorption engine had already excluded drift ("no steady aerobic effort to corroborate"). Michael's real week (3 strength, 1 ride, 1 swim, runs on/under plan) read 3 declining signals → 'high' → "pull back". Now computeDecliningSignals drops RIR for strength-primary and gates HR drift on the absorption steady-aerobic gate (driftUsable); threaded into BOTH the escalation and the safety floor so they agree (D-264). His week → 1 signal (RPE) → 'productive', not 'high'. load_status VALUE changes → bump so cached v142 "pull back" rows re-source. // 142: LOAD IS MULTI-SPORT (D-317) — the base load_status was a RUN ENGINE ("Primary signal: run-only load vs run plan"; running_acwr>1.3 → 'high'), so a strength/multi-sport athlete on-plan with a bunched maintenance run read "pull back." Now computeTotalLoadStatus reads TOTAL load (all disciplines), phase-aware (build tolerates ≤1.6, easy tightens to ≤1.3), descriptive; goal-cost judgment stays in the coach's eye. load_status VALUE changes → bump so cached "pull back" rows re-source. // 141: THE LONG VIEW — run.efficiency gains a 12-week `series` (the SAME points the verdict reads, over an 84d window vs the verdict's 42d, each flagged `recent` for the in-window slice) so the State run row renders a sparkline: verdict = "is my current training working" (6wk), chart = "am I trending up over the block" (12wk). Chart's recent tail IS the verdict's data → they can't contradict. Fills as the athlete trains (new user sees "building · N of 12 weeks"), never a fabricated line. Charts OUTPUT (efficiency), not TP's LOAD (CTL/ATL/TSB). Payload gains run.efficiency.series → bump so cached rows re-source. // 140: cross_training_signal gains `discipline` so the client colors the sentence in the SUBJECT discipline's signature hue (run=gold etc.) instead of blue — blue was swim's own color, so a blue sentence about running collided with the discipline system (Michael on device). Warnings stay amber; neutral/positive reads take the discipline color. Payload gains cross_training_signal.discipline → bump so cached rows re-source. // 139: PROJECTED RACE TIMES on the RUN row (Michael — "estimate 5k/10k for varied runners" + "unlock by distance"). Goal-free VDOT projections off current fitness (5K/10K/half/marathon), reusing the shipped race-readiness engine (estimateVdotFromPace → getTargetTime) with NO goal race needed — the goal was only ever used to say "on track", never to compute the number. Longer distances UNLOCK as the long run grows (10k ~6mi, half ~10mi, marathon ~16mi; long-run distance estimated from the longest recent run's duration at easy pace) so a marathon estimate never shows off short runs. Computed in compute-snapshot (kept OFF the client-math fallback), attached to display.runFitness.projections; client renders unlocked times + dim locked rows. projectStandardRaces (race-readiness, 4 tests). Payload gains runFitness.projections → bump so cached rows re-source. // 138: pace line goes RAW by default + a GAP TOGGLE (Michael — field-standard, Strava/TrainingPeaks show raw pace and grade-adjust only under the hood). The RUN pace line now shows the RAW pace the watch recorded (derived from efficiency_index, not the GAP index); the grade-adjusted twin (recentGapPaceSecPerKm, from gap_efficiency_index) rides along so the client can offer a tap "GAP" toggle. Toggle suppressed when any recent run lacks GAP or the two paces differ <3 s/km. Payload gains run.efficiency.recentGapPaceSecPerKm → bump so cached rows re-source. // 137: PACE-AT-HR LINE on the RUN row (Michael — "another line for pace"). The efficiency index is a fitness signal but "1.55" means nothing to a human, so the row now shows the recent steady-run PACE + HR behind it ("pace ~8:50/mi at 138 bpm"). Derived from the SAME index the verdict reads (gap_efficiency_index ?? efficiency_index) and hr_avg, averaged over the last-2 in-window steady runs — so the pace line can't disagree with the number above it (recentEfficiencyPaceHr, run.ts; 4 tests). Payload gains run.efficiency.recentPaceSecPerKm + recentHrAvg → bump so cached rows re-source. // 136: PRECISE VERDICT WORDS (Michael — "all the words for every scenario has to be precise"). classifyTrend now carries `recentlyFlat` (the recent half of the window sits inside the holding band), so a moving verdict splits into two: sliding + recentlyFlat → "settled lower" (dropped, then levelled); sliding + still-moving → "easing off". "sliding" is retired as a display word; "easing off" is its non-alarming default everywhere (softer than Garmin's "Detraining"). Shared engine — every discipline's trend gains the flag; run efficiency renders it now (StatePerformanceSection verdictLabel). Payload gains run.efficiency.recentlyFlat → bump so cached rows re-source. // 135: RUN ROW LEADS WITH GRADE-ADJUSTED EFFICIENCY (Michael) — "faster at the same heart rate" (GAP-pace ÷ HR on steady runs) replaces the confusing durability DOT-lead. compute-snapshot computes gap_efficiency_index (canonical per-sample GAP from workout_analysis.overall.avg_gap_s_per_mi, fed into the ONE efficiency formula); the run efficiency read is grade-adjusted (raw pace only as the flat/treadmill fallback); durability (decoupling) demoted to a quiet secondary. Client renders efficiency verdict+arrow+%, no dot. // 134: REAL PR — the strength PR badge now fires only on a genuine new ALL-TIME high estimated 1RM (compute-snapshot queries all-history exercise_log for the per-lift best; assemble threads allTimeBestE1rm/allTimeCount into per_lift). Was best-of-6-weeks (fired on nearly every progressing lift, even stamped a lift reading "new"). Client: PR needs latest >= all-time best + >=3 all-history points; PR suppresses the bare "new" label. // 133: HR-response note TIGHTENED — dropped the method legend ("run=decoupling, bike=HR-at-power") + swim-exclusion line; the nudges carry it. 3 sentences not 7 lines. // 132: HR-response staleness NOTE is now an OPPORTUNITY nudge, not a mystery/scold — names the lever (a steady session refreshes the read), DOUBLE DUTY when the athlete is under their run target ("the running you have been under target on"), a "steady ride with power" prompt when the bike qualifier is old, and a "base can slip" consequence gated on aerobic work being GENUINELY low. Garmin says "unproductive"; this says "here is what a session gets you back". // 131: coach-s-eye consolidation — the week-accent TRADE + banner-carried branches REMOVED (the coach's eye owns the running-low / carried / fades story; the trade sentence "running came in at 0 of 3; cycling carried; it fades" was that story a SECOND time on a different window). RIR ungated. One running voice. // 130: THE COACH-S EYE — the Cross-training row is rebuilt from a "trade" narrative (rejected: bad tone, editorialized, shoehorned swim/ride) into a balance read relative to the GOAL. FLOOR (maintain under its declared target — a hard fact, absorbs the upkeep line which is now removed from the week-accent), CEILING (focus slipping/dipping while a supplement climbs — a correlation PROMPT + a lever + an ⓘ that says "you know your body; gains happen in recovery; find your ceiling organically", NEVER a causal verdict), ROOM (pushing a supplement, focus holding — the maximiser's green light). Gated on GOAL IMPACT; one flag or silent; flat fact-first copy. composeCoachEye (10 tests, personas). cross_training_signal gains `info`; client renders headline + ▸ detail + ⓘ popup. // 129: cross-training read is now GLANCE + OPEN — a frank HEADLINE (shown) + a DETAIL (on tap), and the fade language SWITCHES ON THE EASED DISCIPLINE per specificity science (run: "it fades" — use-it-or-lose-it; bike/swim: softer, retained better). User-agnostic (reads each athlete's own posture/verdicts/target/load; field-standard bands), verified across personas (strength-focus, marathoner, freeballer, thin data — 11 tests). cross_training_signal gains `detail`; client renders headline + a ▸ expand (StateTab BODY row). Payload change → bump. // 128: cross-training read WINDOW FIX — a maintain discipline UNDER its trailing upkeep target no longer reads as "pushed" off a 1-week acwr uptick (it contradicted the upkeep line right below: "you're pushing your running" above "running's under for 4 weeks", one number two windows, found on device). underTarget (trailing) overrides the weekly blip → the read now correctly says the TRADE ("building strength, running eased to make room"). Payload change → bump. // 127: HONEST CROSS-TRAINING READ — the Cross-training row now answers "is pushing one thing costing another you care about?" off OUTCOMES (spine strength e1RM + run/bike verdicts + per-discipline load), never HR. Fixes Garmin's "Unproductive": building strength while running eases reads as a TRADE ("that's the trade, not lost fitness"), not lost fitness. Fires only with a declared focus + a real trade/cost/room story; else falls back to the reassurance. composeCrossTrainingRead (_shared/insights/cross-training-read.ts, 7 tests). Payload change → bump so cached rows re-source. // 126: the strength↔endurance INTERFERENCE VERDICT is retired (computeCrossDomain). The "HR +Nbpm after lifting" / "Interference detected between disciplines" warning fired on a signal too weak to trust (effect < e1RM measurement error; HR-at-pace inverts under real overreaching) for a situation the PLAN already prevents (the 6h separation gate). SCIENCE doc: the app may comment on scheduling, never on whether interference occurred. cross_training_signal now only reassures ("No interference…" / "Handling load…") or stays silent; the alarm branch is gone. Payload change → bump so cached rows re-source. // 125: HR-response NO-SILENT-DROP (Michael's "is it lagging?" catch) — a run PRESENT but below the 8-run trend floor is excluded from the heart-rate response rollup, so the read leaned on an older bike and looked stale while fresh runs sat just under the bar. The provenance now NAMES it: "Your runs aren't in this yet — N of 8 steady runs needed to read a direction." (hrResponseExcludedRunNote, tested). Payload string change → bump so cached rows re-source. // 124: F17 — the "Cross-training" row's positive label now SCOPES to what it checked. It only inspects the five interference/strain signals (RPE, HR drift, strength fade, RIR, body-concern), never volume/adherence/upkeep — so "Handling combined load well" over-claimed a training all-clear and rendered green above a real run shortfall (the app arguing with itself). Now "No interference between disciplines" (the word the concurrent-gains branch two above already uses). Payload string change → bump so cached rows re-source. // 123: F15 — the upkeep accent's CARRIERS clause now reads the SAME 28-day trailing window as the shortfall (resolveAerobicCarriers over completedRolling), not this-week's session counts. On a Monday those counts are 0, so "Swimming and cycling carried the endurance load." silently dropped and the flagship sentence rendered as a bare shortfall. The FIX is in _shared/state-trend/week-accent.ts but the payload is cached for 24h keyed on THIS version — without a bump the corrected sentence never reaches a device (the D-281/Q-170/v118 trap the comments below keep warning about, walked into anyway). Bump so cached rows re-source. // 122: intent_summary is POSITION ONLY (plan name + week N of M). The ~130-line / ~25-string imperative tree is gone (v121), and the synopsis clause that briefly replaced it is gone too — buildLoadHeadline (src/lib/load-headline.ts, StateTab.tsx:1404) ALREADY composes the reconciled load verdict + readiness into "This week: Balanced load." on the very next line. Position is the one thing no other line carries. Bump so cached rows re-source. // 121: the TOP LINE is now POSITION + SYNOPSIS. Replaced a ~130-line nested tree keyed on FOUR crossed axes (load x cross-training composition x week intent x readiness) emitting ~25 hardcoded strings, nearly all IMPERATIVES ("protect recovery now", "Rest fully", "back off and recover") — the register D-155 bans and that nothing enforced here. Now: plan name + week N of M, then AT MOST ONE synopsis clause naming a verdict the rows below actually render, in their vocabulary. Silence when nothing rises. Also kills the live banned-copy ("body is ready, stay consistent", Q-194) and the possible "cross-training pushing load high" vs BODY row "handling combined load well" contradiction. Load is spoken about ONLY when the raw ACWR band and the reconciled verdict agree (Q-196). Bump so cached rows re-source. // 120: D-306 window fix — the paragraph was reading plan-vs-actual off the ROLLING 7-day window (acute7_by_type) while the plan lives on the CALENDAR week, so consecutive sentences described two different spans and both called it "the week" (Michael, on sight of the screen). Plan comparison now uses computeWtdLoadSummary (_shared/adherence-plan.ts) PER DISCIPLINE — the same shipped helper the rest of the week reads — which bounds planned load to sessions due ON OR BEFORE today. That RETIRES the hand-rolled Sunday gate from v118: partial weeks were already solved here (v100/v102, the Q-177 trap), so the clause now speaks every day instead of once a week. ACWR stays ROLLING on purpose — "drifting below its own normal" is a trailing question. Bump so cached rows re-source. // 119: D-306 follow-up, ON SIGHT OF THE REAL SCREEN. Dropped the mix sentence — it restated the LOAD bar rendered directly above it ("44% of your load, then strength 22%") which is the exact redundancy every rejected AI-narration feature was killed for. And the all-clear no longer says "every discipline": it only ever checked disciplines the PLAN asked for, and fired on a week where bike and swim carried real off-plan load. It now names the scope and surfaces the off-plan work. Bump so cached rows re-source. // 118: D-306 — the week narrative is DETERMINISTIC. `coach.narrative` is now composed by `_shared/insights/coach-week-insights.ts` (protocol-aware, focus-aware) instead of written by the LLM. A composed SILENCE is a real answer, so the legacy LLM fallback is gated on `deterministicNarrativeApplied` and can no longer back-fill an intentional quiet. Bump so cached rows re-source — otherwise coach_cache serves the old LLM prose for 24h and the change looks like it did not ship (the D-281/Q-170 trap). // 117: upkeep SLIP GATE (refines D-297) — the upkeep accent flips from compliance-only to a gated measured "aerobic base has started to slip" ONLY when hr_drift is measurably declining (never routine, never a prediction); holding/thin → unchanged. Bump so cached rows re-source. // 116: threshold-pace single-source (audit #6) — the baseline prose line now reads the ONE resolver (learned/measured wins over the wizard effort_paces it read first), formatted m:ss, so the coach speaks the SAME threshold pace race-projections predicts off. Bump so cached rows re-source the corrected line. // 115: UPKEEP accent (D-297) — a MAINTAIN discipline is measured against its OWN stored target (run = target_weekly_miles), in miles, on the trailing pattern, with a science-backed gentle read (docs/SCIENCE-upkeep-maintenance.md) — replacing the session-count "1 of 3 runs / speed fades" trade for maintain disciplines. Bump so cached rows re-source. // 114: bike anchor label gains its as-of date ("auto - FTP est - <date>") to match the run anchor grammar; cosmetic. Bump so cached rows re-source.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ⛔ THE BLOCK IDENTITY CARD — resolved ONCE, here, and read by everything below (Q-230).
    //
    // Every surface used to answer "what block is this?" for itself off `plans.config`, and they
    // disagreed: this function read `config.strength_protocol` and nothing else, so a strength-primary
    // block — which never wrote that key — resolved to a null protocol. Downstream, that null made
    // `protocolExpectsE1rmToDip` return false and left the generic "estimated one-rep maxes have been
    // sliding" claim un-suppressed on a block whose own prescription is why they dipped (audit F3).
    //
    // ⚠️ READ-ONLY. Nothing that consumes this card may change a prescribed weight or move a session —
    // it describes a plan that already exists (Constitution Law 4).
    const blockIdentity = resolveBlockIdentity({
      planId: activePlan?.id ?? null,
      planName: activePlan?.name ?? null,
      planConfig,
      durationWeeks: activePlan?.duration_weeks ?? null,
      // The goal the ACTIVE plan was built for — it owns what the athlete is chasing, and it is the
      // only place the non-race focus is recorded (Q-230 Part B).
      goal: (goalContext?.goals || []).find((g: any) => String(g?.id ?? '') === String(activePlan?.goal_id ?? '')) ?? null,
      onDateIso: asOfDate,
    });
    ...
    // ── Slice 1 (2026-08-12): the actual-vs-PLANNED load facts, computed ONCE, here ────────────────
    // These are the only load inputs allowed to escalate an overload verdict (see mintOverloadVerdict).
    // They are hoisted this early because the verdict has to exist BEFORE the weekly response model,
    // the readiness tree, and the load reconciler — all three used to decide "overloaded" on their own.
    // ⚠️ NOT `wtd_completion_ratio`: that ratio is clamped to [0,1] (_shared/adherence-plan.ts) so it
    // can never say "you did more than the plan asked" — the exact fact this needs.
    const loadVsPlanPct = (plannedWtdLoad > 0)
      ? Math.round(((actualWtdLoad - plannedWtdLoad) / plannedWtdLoad) * 100)
      : null;
    // Off-plan work added on top of the plan, as a % of the planned week.
    const unplannedLoadWtd = (Array.isArray(weekWorkouts) ? weekWorkouts : [])
      .filter((w: any) => String(w?.workout_status || '').toLowerCase() === 'completed' && !w?.planned_id)
      .reduce((sum: number, w: any) => sum + (Number(w?.workload_actual) || 0), 0);
    const unplannedSharePct = (plannedWtdLoad > 0)
      ? Math.round((unplannedLoadWtd / plannedWtdLoad) * 100)
      : null;
    ...
    /**
     * ⛔ THE NEXT ROW'S OWN LIST (2026-09-15, Stage 4 session 2). State's NEXT row does not show today
     * (Michael, 2026-09-01: *"I dont think we need today reflected on state screen or next"*) and shows
     * three. The phone filtered `keySessionsRemaining` itself against a date it built from its own clock.
     *
     * ⚠️ `keySessionsRemaining` IS NOT NARROWED. It is documented as "from as_of_date INCLUSIVE" and
     * `hasUpcomingLong` (:551) reads it to write race-week guidance — moving the exclusion into that
     * field would change race-week copy for a State-screen ruling. This is the second list, cut once.
     *
     * ⚠️ AND THE DATE IS ALREADY LOCAL. `asOfDate` is the date the phone sent (`payload.date`), or this
     * athlete's own timezone resolved here, so the cut lands on the same day the phone would have chosen.
     */
    const NEXT_SESSIONS_SHOWN = 3; // OURS — the State row's display bound. docs/STATE-SOURCES.md.
    const nextSessions: KeySessionItem[] = keySessionsRemaining
      .filter((x) => String(x?.date || '').slice(0, 10) > asOfDate)
      .slice(0, NEXT_SESSIONS_SHOWN);
    ...
    const runSessionTypes7d: NonNullable<CoachWeekContextResponseV1['run_session_types_7d']> = (Object.keys(runAgg) as RunSessionType[])
      ...
      .map((k) => {
        ...
        return {
          ...
          // D-372 (2026-09-04, Michael: "kill the line"): no verdict on a 7-day drift average. The number stays
          // (avg_decoupling_pct / avg_hr_drift_pct); the per-workout row already says what drift was. Drift reports and flags; it never diagnoses.
          efficiency_label: null,
          efficiency_tone: 'neutral' as const,
        };
      })
    ...
    // State's run row prints threshold heart rate from the same inputs Adjust's readout uses (`save-baselines/zones.ts`),
    // and the typed run threshold heart rate lives in `configured_hr_zones`, which ArcContext does not carry.
    const { data: hrZonesRow } = await supabase
      .from('user_baselines')
      .select('configured_hr_zones')
      .eq('user_id', userId)
      .maybeSingle();
    const configuredHrZones = (() => {
      const raw = hrZonesRow?.configured_hr_zones;
      if (raw && typeof raw === 'object') return raw as Record<string, unknown>;
      if (typeof raw === 'string') { try { const o = JSON.parse(raw); return o && typeof o === 'object' ? o : null; } catch { return null; } }
      return null;
    })();
    ...
    if (!shouldSuppressBaselineDriftSuggestions) {
      for (const p of driftPairs) {
        ...
        const cap = resolveStrengthCapacity({ key: p.lift, typed: perf, learnedStrength1rms: strength, locked: (arc as any)?.locked_baselines ?? null, asOf: today });
    ...
    // ── Fitness · Fatigue · Form — TrainingPeaks' Performance Management Chart, the LOAD read on State ──
    // (2026-09-04, Michael: one reference per metric — ACWR is off the screen.) The SAME workload_actual
    // column ACWR reads (D-264 single source), over the athlete's WHOLE history: TrainingPeaks seeds the
    // 42-day average at zero from the first workout, so an 84-day window (the fetch until today) under-
    // stated fitness for anyone with more history than that. Three columns, one query.
    ...
    // ⛔ THE CHART'S DAYS COME OFF THIS SAME CALL (2026-09-25, approved by Michael). `seriesDays` has the one walk
    // that makes today's three numbers also keep each day of the chart window (`series`), so the chart's last day
    // IS the card's reading — no second load series, no second average. FIELD — TrainingPeaks' Performance
    // Management Chart draws fitness, fatigue and form as lines over time; the window is the 12-week chart window
    // every State chart draws (`TREND_FIT_MAX_WEEKS`, TrainingPeaks' 90-day default), in days.
    const fitnessFatigueToday = computeFitnessFatigue(ffLoadRows, { asOfDate, seriesDays: TREND_FIT_MAX_WEEKS * 7 });
    // The same three numbers seven days ago, so the screen can print the week's change beside each
    // (intervals.icu's fitness tile shows the 7-day delta; TrainingPeaks' PMC shows it as the chart's slope).
    const weekAgoDate = (() => { const [y, m, d] = String(asOfDate).slice(0, 10).split('-').map(Number); const t = new Date(Date.UTC(y, m - 1, d) - 7 * 86_400_000); return `${t.getUTCFullYear()}-${String(t.getUTCMonth() + 1).padStart(2, '0')}-${String(t.getUTCDate()).padStart(2, '0')}`; })();
    const fitnessFatigueWeekAgo = computeFitnessFatigue(ffLoadRows, { asOfDate: weekAgoDate });
    // ⛔ THE THREE READINGS, FINISHED HERE (2026-09-15, Stage 4 session 2). The LOAD line printed the
    // server's 1-dp numbers through its own rounder, subtracted this week from last week itself, and
    // divided the averaging constants by 7 to write "6 wk" / "7 d". Four rules on the phone over numbers
    // that are already the server's. Each reading now arrives as the text the line prints.
    // ⚠️ THE WINDOWS ARE THE MODEL'S OWN CONSTANTS (`provenance.tau_*_days`, TrainingPeaks' 42 and 7);
    // fitness reads in weeks, fatigue in days, and form has no window — it is the gap between the two.
    // A run with no load prints no readings at all, exactly as before.
    const ffRound = (v: number | null | undefined): number | null =>
      (v == null || !Number.isFinite(v) ? null : Math.round(v));
    const ffDelta = (now: number | null | undefined, then: number | null | undefined): string | null => {
      const a = ffRound(now), b = ffRound(then);
      if (a == null || b == null) return null;
      // The change is the difference of the two PRINTED numbers, so the line agrees with what is on screen.
      const d = a - b;
      return d === 0 ? '±0' : d > 0 ? `+${d}` : `${d}`;
    };
    const ffSigned = (v: number | null | undefined): string | null => {
      const n = ffRound(v);
      if (n == null) return null;
      return `${n > 0 ? '+' : n < 0 ? '−' : ''}${Math.abs(n)}`;
    };
    const tauFitDays = fitnessFatigueToday.provenance?.tau_fitness_days ?? null;
    const tauFatDays = fitnessFatigueToday.provenance?.tau_fatigue_days ?? null;
    const fitnessFatigueDisplay = {
      fitness: {
        value: ffRound(fitnessFatigueToday.fitness) != null ? String(ffRound(fitnessFatigueToday.fitness)) : null,
        change: ffDelta(fitnessFatigueToday.fitness, fitnessFatigueWeekAgo.fitness),
        window: tauFitDays != null && Number.isFinite(tauFitDays) && tauFitDays > 0 ? `${Math.round(tauFitDays / 7)} wk` : null,
      },
      fatigue: {
        value: ffRound(fitnessFatigueToday.fatigue) != null ? String(ffRound(fitnessFatigueToday.fatigue)) : null,
        change: ffDelta(fitnessFatigueToday.fatigue, fitnessFatigueWeekAgo.fatigue),
        window: tauFatDays != null && Number.isFinite(tauFatDays) && tauFatDays > 0 ? `${Math.round(tauFatDays)} d` : null,
      },
      // Form carries its sign in the text; there is no window on a difference.
      form: {
        value: ffSigned(fitnessFatigueToday.form),
        change: ffDelta(fitnessFatigueToday.form, fitnessFatigueWeekAgo.form),
        window: null as string | null,
      },
    };
    // ⛔ THE CHART'S CAPTIONS: THE WINDOW'S ACTUAL FIRST AND LAST DAY (2026-09-25). TrainingPeaks' Performance
    // Management Chart draws the daily values with no fitted line, so the captions read the first and last day
    // through the SAME rounders as the readings above (`ffRound`, `ffSigned`) — the last number is the card's.
    // `draw_series` is the two points a line needs (`TREND_CHART_MIN_POINTS`), as `drawFtpLine` is for the bike row.
    const ffSeries = fitnessFatigueToday.series ?? [];
    const ffSeriesCaption = loadChartCaptions(ffSeries, {
      whole: (v) => { const n = ffRound(v); return n == null ? null : String(n); },
      signed: ffSigned,
    });
    const fitnessFatigue = {
      ...fitnessFatigueToday,
      week_ago: { fitness: fitnessFatigueWeekAgo.fitness, fatigue: fitnessFatigueWeekAgo.fatigue, form: fitnessFatigueWeekAgo.form },
      display: fitnessFatigueDisplay,
      series_caption: ffSeriesCaption,
      draw_series: ffSeries.length >= TREND_CHART_MIN_POINTS,
    };
    ...
    // strength-primary block, so this resolved a `durability` RIR profile for a the previous program block — the
    // Q-192 / D-322 failure, third instance, on a protocol whose own registry entry says
    // `usesRir: false`. Harmless while no RIR is collected (the targets are inert), and wrong.
    const strengthProfile = resolveProfile(blockIdentity.protocolId);
    ...
    try {
      // FIX B — READ = WRITE. compute-snapshot writes state_trends_v1 to the CURRENT calendar week
      // (mondayOfToday). This read used to take max(week_start), which a stray future/non-Monday snapshot
      // could shadow (a 07-14 row hid the 07-13 write carrying the fitness anchors). Bounding to
      // week_start <= this week's Monday excludes any future-dated row and reads the snapshot the compute
      // actually wrote — the invariant, fixed at the read RULE (not by skipping NULL-anchor rows).
      const { data: snapRows } = await supabase
        .from('athlete_snapshot')
        .select('interference, intensity_distribution, state_trends_v1')
        .eq('user_id', userId)
        .lte('week_start', mondayOfToday())
    ...
    // ── THE ALL-OUT SET, PER LIFT, FOR STATE (Q-254 slice 1, 2026-08-03) ─────────────────────────
    //
    // ⛔ THE APP COLLECTS THE ONE MEASUREMENT the previous program IS BUILT ON AND STATE REASONED FROM SOMETHING
    // ELSE. Michael, on his own screen: Deadlift read *"Working ~120 vs your 150 baseline"* while his
    // logged e1RM was already ~225 — the row compares a light the previous program working weight to a stale TYPED
    // baseline and concludes he is going light. The AMRAP top set is the measurement, and the
    // Performance screen has rendered it correctly since 2026-07-30. This feeds State the same read.
    //
    // ⛔ IT IS THE SAME FUNCTION PERFORMANCE CALLS — `_shared/strength/all-out-set.ts`. Not a copy.
    // A second implementation of "was that a rep record" is exactly how two screens start disagreeing
    // about one session, and this file already carries five private exercise matchers' worth of that
    // lesson (audit F5).
    //
    // ⚠️ THE WINDOW IS 40 STRENGTH SESSIONS, NOT THE 7d/28d BANDS THIS FILE OTHERWISE USES. It has to
    // be: a leader cycle and every deload week carry NO all-out set, so an athlete can easily go a
    // month without one and 28 days would return nothing on a lift that is measured and climbing.
    // Q-254's open question 1, ruled: carry the last measured value WITH ITS DATE.
    //
    // ⛔ AND THERE IS NO FALLBACK. If no all-out set exists in the window, this is empty and the row
    // says nothing extra. It must NEVER fall back to `best_weight`/`best_reps` — that pair is the
    // heaviest set and the most reps at that weight, so a heavy single after the AMRAP makes it read
    // 1 rep on a session that went well. `cycle-verdicts.ts` refuses to read it for this reason and
    // so does this. (Q-254 Gap 1.)
    const lastAllOutPerLift: LastAllOutByLift = await (async () => {
      try {
        const { data: strengthRows } = await supabase
          .from('workouts')
          .select('id,date,planned_id,strength_exercises')
          .eq('user_id', userId)
          // ⚠️ THE TYPE LIST AND THE ABSENT STATUS FILTER BOTH MATCH `workout-detail`'s rep-record
          // scan EXACTLY. A `workout_status` filter here would be a divergence, and a case-sensitive
          // one at that (every other read in this file lowercases before comparing) — State would
          // then compute a rep record off a smaller history than Performance did and the two screens
          // would disagree about the same set. `workouts` rows are logged sessions; an all-out set
          // with reps on it is performed work by definition.
        ...
        // The planned rows behind those sessions — the `set_plan` fallback needs them, and WITHOUT
        // them State would silently see fewer all-out sets than Performance does on the same
        // sessions (the flag only lands when the row was built from a planned session).
        ...
        // ⚠️ OLDEST FIRST. `lastAllOutByLift` builds the rep-record history as it walks, so a
        // reversed list would judge every set against the future and mint records that are not.
        const sessions = rows
          .slice()
          .sort((a: any, b: any) => String(a?.date || '').localeCompare(String(b?.date || '')))
          .map((r: any) => {
          ...
          .filter((s) => s.date.length === 10);
        return lastAllOutByLift(sessions);
      } catch { return {}; }
    })();
      ...
      try {
        ...
        return Object.entries(s1rms)
          ...
          .map(([key, v]: [string, any]) => {
            ...
            const cap = canon ? resolveStrengthCapacity({ key, typed: perf, learnedStrength1rms: s1rms, locked: (arc as any)?.locked_baselines ?? null, asOf: asOfDate }) : null;
            ...
            // ⛔ Q-254 SLICE 2a — A PROTOCOL THAT DOES NOT USE RIR GETS NO RIR TARGET.
            //
            // `strength_primary` declares `usesRir: false` / `readsEffortAs: 'amrap'`
            // (`strength-profiles.ts:161`), and this line ignored both: `getTargetRir` never
            // consults `usesRir` and falls through to `defaultTargetRir` — 2 — so a the previous program lift was
            // being judged against a reserve target its own protocol says does not exist.
            //
            // ⛔ AND THE RIR IT WAS COMPARED TO WAS OFTEN NOT EVEN ITS OWN. `current_avg_rir` below
            // falls back to `reaction.avg_strength_rir_7d` when a lift logs no RIR of its own — a
            // SESSION-WIDE average that includes the accessories, which do log it. So a main lift
            // with no reserve data inherited the accessory average, cleared the ±1.0 band, and
            // printed "add weight" or "back off weight" with a tappable weight suggestion behind it.
            // The comment further up this file called the targets "inert while no RIR is collected";
            // that premise stops holding the moment any movement in the session logs one.
            //
            // ⚠️ FIXED AT THE STAMP SEAM, NOT BY MAKING `getTargetRir` NULLABLE — and that is the
            // documented rule, not a preference. `protocolUsesRir`'s own docblock
            // (`strength-profiles.ts:458-469`) says making it nullable "would push a null check into
            // every one of [~a dozen callers], and the ones that forgot would silently read `0` —
            // 'grind to failure' — which is the worst possible failure direction for this."
            ...
            // Q-254 slice 1: the last all-out set for THIS lift, or null when the window holds none.
            // ⚠️ Null is honest and common — a leader cycle prescribes no AMRAP at all.
            last_all_out: lastAllOutPerLift[key] ?? null,
            };
          });
      } catch { return []; }
    })();
    ...
    const weeklyResponseModel: WeeklyResponseState = computeWeeklyResponse({
      ...
      unplannedShareOfPlannedPct: unplannedSharePct,
    ...
    // ── THE ONE OVERLOAD VERDICT (Slice 1, 2026-08-12) ────────────────────────────────────────────
    // Minted inside computeWeeklyResponse (the first point where the athlete's own signals and the
    // actual-vs-planned load both exist) and read from here on by: the assessment label, the week
    // headline, the readiness state, the load reconciler, the over-reach accent, and training_state.
    // ⛔ If you are about to write "if (acwr > X) → overloaded" anywhere below, that is the bug this
    // slice closed. Read `overloadVerdict`.
    const overloadVerdict = weeklyResponseModel.overload;

    // ⛔ THE BODY 'Heart-rate response' ROW IS DELETED (D-354, Michael 2026-08-01). DO NOT REBUILD IT.
    //
    // It was a pure ROLLUP: since D-346 its contributors were `run.efficiency` and `bike.efficiency` —
    // the same two verdicts the RUN and BIKE rows render directly beneath it. So it restated two rows
    // and added nothing they did not already say, with no number of its own (speed-per-heartbeat and
    // watts-per-heartbeat cannot honestly be combined into one figure).
    //
    // ⚠️ IT DIED IN THREE STEPS, AND EACH ONE REMOVED SOMETHING IT COULD NOT SUPPORT: it could not
    // out-alarm the rows below (D-353's severity cap), it could not restate them (the contributor
    // clause), and it could not use words (the app-wide no-verdict-word rule). What was left was an
    // arrow duplicating the direction of the two rows under it — pointing a direction its own two
    // contributors split on (Q-238), stamped with a date that hid its stale half (Q-239).
    //
    // ⛔ HEART RATE IS NOT THE BODY INSTRUMENT HERE, and that is the durable reason. Overreached
    // athletes show submaximal HR going DOWN and HR recovery getting FASTER while RPE rises
    ...
    let bodyTodayLine: string | null = null;
    {
      ...
      if (rpeRow) {
        // ⛔ EVERY NUMBER CARRIES ITS WINDOW, ONCE (Michael 2026-09-10). The section header carries the recent
        // window — the effort pass (`rpeStart`, asOf −6) and the soreness read, which is handed the SAME days
        // here rather than left on its own default — so the rows do not repeat it. "usual" is the 28-day norms
        // pass (`normStart`), a different window, so it says so beside its number. Both day counts are read
        // off the window starts themselves, never typed a second time.
        const bodyDays = (fromISO: string) =>
          Math.round((Date.parse(`${String(asOfDate).slice(0, 10)}T12:00:00Z`) - Date.parse(`${String(fromISO).slice(0, 10)}T12:00:00Z`)) / 86400000) + 1;
        const recentDays = bodyDays(rpeStart);
        const normDays = bodyDays(normStart);
        (weeklyResponseModel as any).body_window_label = `last ${recentDays} days`;
        const sore = resolveCurrentSoreness(sorenessEntries, { asOf: asOfDate, recentDays });
        // ⛔ THREE FACTS, THREE ROWS (2026-09-03, DESIGN_GUIDELINES "No prose in a value slot"). This
        // used to be one paragraph in the value slot — "About as hard as usual — you rated 5.1 of 10 avg
        // vs 4.7 typical. Soreness normal for you: 2.0 of 7. Logged on 8 sessions." — which wrapped to
        // five lines on a screen of one-line readouts. Effort, soreness and the logged count are now
        // separate signals; the client already draws one row per signal, so nothing there had to change
        // except the driver line's label match.
        //   effort     5.1 of 10 · usual 4.7 · about as hard as usual
        //   soreness   2.0 of 7 · normal for you
        //   logged     8 sessions            (carries the as-of date)
        const rpe = (weeklyResponseModel as any).endurance?.rpe as { current_avg?: number | null; baseline_avg?: number | null } | undefined;
        const cur = rpe?.current_avg ?? null;
        ...
        if (sore.level && sore.recent != null) {
          ...
          const thin = nEnt < 3;
          // The entry count prints only when it differs from the logged count below (the header's window
          ...
          extra.push({
            ...
            value_display: `${sore.logged} of 5 logged`,
            detail: 'needs 5 sessions to read a normal',
        ...
        if (rated != null) {
          extra.push({
            ...
            value_display: `${rated} session${rated === 1 ? '' : 's'}`,
            detail: '',
            samples: rated, samples_label: `${rated} session${rated === 1 ? '' : 's'}`,
            // The as-of date reads once, under the count, instead of floating under the paragraph.
            as_of_date: (rpeRow as any).as_of_date ?? null,
          });
          (rpeRow as any).as_of_date = null;
        }
        const at = weeklyResponseModel.visible_signals.indexOf(rpeRow);
        weeklyResponseModel.visible_signals.splice(at + 1, 0, ...extra);
        const soreRow = extra.find((x) => x.label === 'soreness') ?? rpeRow;

        // ── THE PERSISTENCE LINE (D-354) ────────────────────────────────────────────────────────
        // ⛔ IT STATES A FACT AND POINTS AT A DOOR. It does not diagnose, does not name a cause, and
        // does not act. Soreness clusters around lifting days is an OBSERVATION; "your lifting caused
        // this" is a causal claim the same evidence bar that killed the interference verdict would
        // reject. Asking rather than asserting is what makes it honest.
        //
        // ⚠️ 4 OF 6 IS THE MONITORING STANDARD, not a number picked here — `longitudinal-signals.ts`
        // uses the same shape against an absolute floor and its own header says that floor should
        // become baseline-relative once history exists. This is baseline-relative.
        //
        // ⚠️ IT CANNOT MARK THE ATHLETE DOWN. Advisory only, exactly like the existing soreness
    ...
    /**
     * ⛔ EACH PAST DAY'S OWN FORM AND BODY LINE (2026-09-17, Michael: yesterday's Today card showed a gap where form
     * and effort sit). TrainingPeaks' and Intervals.icu's calendars give every day its own fitness, fatigue and form;
     * a past day prints its own, never today's. Same rules as today's line, run as of each date:
     *   · form — `computeFitnessFatigue` as of the day, the zone word from `formZone` (the same two calls today uses).
     *   · the BODY line — effort is the average session rating of completed sessions in the 7 days ending that day;
     *     soreness joins on 3+ entries with its baseline from the 28 days ending that day (`resolveCurrentSoreness`,
     *     the call today's line makes, with the same windows).
     * OURS — 84 days (12 weeks) of past days: the State trends' window. Older days and future days get no entry,
     * and the card keeps their space empty.
     */
    const DAY_SERIES_DAYS = 84;
    const form_by_day: Record<string, { form: number; label: string | null; label_text: string | null }> = {};
    const body_line_by_day: Record<string, string> = {};
    try {
      for (let i = 1; i <= DAY_SERIES_DAYS; i++) {
        const d = addDaysISO(asOfDate, -i);
        const ff = computeFitnessFatigue(ffLoadRows, { asOfDate: d });
        if (ff.form != null && Number.isFinite(ff.form)) form_by_day[d] = { form: Math.round(ff.form), label: formZone(ff.form), label_text: formZoneText(ff.form) };
      }
      const { data: dayRows } = await supabase
        .from('workouts')
        .select('id,date,timestamp,type,workout_status,workout_metadata,rpe,session_rpe,strength_exercises,workout_analysis')
        .eq('user_id', userId)
        .gte('date', addDaysISO(asOfDate, -(DAY_SERIES_DAYS + 27)))
        .lt('date', asOfDate);
      const done = (Array.isArray(dayRows) ? dayRows : []).filter((w: any) => String(w?.workout_status || '').toLowerCase() === 'completed');
      const rated = done.map((w: any) => ({ date: String(w.date || '').slice(0, 10), rpe: sessionRpeFromWorkout(w as any) }))
        .filter((x) => x.rpe != null) as Array<{ date: string; rpe: number }>;
      const soreAll = done.map((w: any) => {
        const meta: any = parseJson((w as any)?.workout_metadata) || {};
        const s = Number(meta?.readiness?.soreness);
        return Number.isFinite(s)
          ? { workoutId: String(w?.id ?? ''), startTime: String(w?.timestamp || (w?.date + 'T12:00:00Z')), soreness: s, date: String(w.date || '').slice(0, 10) }
          : null;
      }).filter(Boolean) as Array<{ workoutId: string; startTime: string; soreness: number; date: string }>;
      for (let i = 1; i <= DAY_SERIES_DAYS; i++) {
        const d = addDaysISO(asOfDate, -i);
        const from7 = addDaysISO(d, -6);
        const inWeek = rated.filter((x) => x.date >= from7 && x.date <= d);
        if (!inWeek.length) continue;
        const effort = inWeek.reduce((a, b) => a + b.rpe, 0) / inWeek.length;
        const parts = [`effort ${effort.toFixed(1)} of 10`];
        const from28 = addDaysISO(d, -27);
        const sore = resolveCurrentSoreness(soreAll.filter((e) => e.date >= from28 && e.date <= d), { asOf: d, recentDays: 7 });
        if (sore.level && sore.recent != null && sore.recentCount >= 3) parts.push(`soreness ${sore.recent.toFixed(1)} of 7`);
        parts.push('last 7 days');
        body_line_by_day[d] = parts.join(' · ');
      }
    } catch (e) {
      console.warn('[coach] day series skipped:', e instanceof Error ? e.message : e);
    }
    // ⛔ STATE'S "This week" LINE (2026-09-18) — the plan week so far (week start to today), completed sessions, time
    // per sport (`_shared/week-time-line.ts`). It replaced the planned-vs-done session bars.
    const week_time_line = weekTimeLine(weekWorkouts);
    ...
    // Slice 1 (2026-08-12): the readiness tree moved to `_shared/response-model/readiness-state.ts` so
    // it can be fixtured (it was unreachable inside this @ts-nocheck file), and it now reads THE
    // overload verdict instead of minting its own from a soft signal count / absolute ACWR. That bare
    // count is the seam that kept "a bit high" + the over-reach accent alive after the assessment label
    // was gated. Branch order is unchanged — see that file's header for exactly what moved.
    const readinessState = computeReadinessState({
      verdictCode: v.code,
      assessmentLabel: weeklyResponseModel.assessment.label,
      signalsConcerning: weeklyResponseModel.assessment.signals_concerning,
      signalsAvailable: weeklyResponseModel.assessment.signals_available,
      acwr: metrics.acwr,
      isPlanTransitionPeriod,
      weekIntent,
    ...
    const fatigueRefinement: { label: string; loadedLegs: LoadedLegsDiagnosis | null } | null = (() => {
      ...
      const loadLabel = zoneWord === 'high risk' && metrics.form != null ? `form ${Math.round(metrics.form)} (${formZoneText(metrics.form)})` : ''; // empty = nothing to add; the zone's one name (2026-09-26)
    ...
    // ── LOAD STAYS OBJECTIVE, READINESS SITS BESIDE IT (D-416, 2026-08-11). ──────────────────────────
    // The load path reads readiness REFINED the same way the chip is (D-232), not the raw `fatigued`
    // catch-all — which over-fires on a single unattributed signal. A lone effort-up ('EFFORT UP') or a
    // localized leg-load ('LEGS LOADED'/'LEGS SORE') is NOT systemic fatigue and must not relabel the
    // objective load verdict (a 1.1 ACWR is optimal, not "a bit high"). Only a genuinely SYSTEMIC picture
    // — refined label 'FATIGUED' (elevated ACWR or ≥2 declining signals), or 'overreached' — carries into
    // the load verdict + the over-reach accent. Matches Garmin/TP/Strava in principle: the load number is
    // measured from the work; how-you-feel lives on its own rail and never rewrites the load label.
    // Consumers: reconcileLoadStatus (sole verdict authority, D-260) + overReachCandidate ONLY. The
    // SAFETY floor (computeSafetyFloor) and the honest readiness_state payload stay on the RAW value.
    // Rule lives in one place (readinessForLoadVerdict, _shared/response-model/loaded-legs.ts) so it is
    // pinned by a fixture and cannot drift from the chip's refinement.
    const readinessForLoad = readinessForLoadVerdict(
      readinessState,
      fatigueRefinement?.label ?? null,
    ) as typeof readinessState;
    ...
    // D-416: plan-aware gate — never prompt a deload/recovery in a week the PLAN is already easing.
    // recovery/taper were excluded; a planned DELOAD (weekIntent 'deload' or blockIdentity.cycle.isDeload)
    // used to fall through, so the Adjust tab could say "consider a deload" mid-deload. All three easing
    // shapes are now excluded together.
    const planAlreadyEasing = weekIntent === 'recovery' || weekIntent === 'taper' || weekIntent === 'deload'
      || blockIdentity.cycle?.isDeload === true;
    if (activePlan && !isPlanTransitionPeriod && !planAlreadyEasing) {
      ...
      if (readinessForLoad === 'overreached' || v.code === 'recover_overreaching') {
        addSuggestion(
          'deload',
      ...
      } else if (readinessForLoad === 'fatigued' || v.code === 'caution_ramping_fast') {
        // D-416: reads the systemic-only readiness (readinessForLoad) — a lone effort-up no longer
        // reaches this prompt. Body-confirmation stays as a second gate.
    ...
    try {
      ...
      // Slice 1: one computation of actual-vs-planned (hoisted above, feeds the overload mint too) —
      // this used to be a second, identical inline copy of the same arithmetic.
      const loadPct = loadVsPlanPct;
      ...
      // ── Reconcile load_status with body signals + plan context ─────────
      {
        ...
        const _meanDrift = _easyRunDrifts.length
          ...
          : null;
        const _driftSession = _meanDrift != null ? { intentEasy: true, hrDriftBpm: _meanDrift, anchorThin } : null;
        // D-318: the reconciler's HR-drift strain signal must obey the SAME steady-aerobic gate the
        // absorption engine uses. No valid steady session (driftUsable=false) ⇒ HR drift is not counted
        // as strain — otherwise the loose all-runs cardiac trend flags "HR drift declining" on a week the
        // absorption engine already excluded. Threaded into BOTH the safety floor and the reconciler so
        // the two verdict paths read one de-run-brained signal set (D-264). planPrimary drops RIR-as-strain
        // for a strength-primary plan (declining RIR = pushing toward failure = intent, not overreaching).
        const _driftUsable = _driftSession != null && steadyGate(_driftSession).describe;
        const _decliningOpts = { planPrimary, driftUsable: _driftUsable };
        // D-318: for a strength-primary plan the RIR-direction 'strength' trend is training INTENT
        // (pushing toward failure across a build), not muscular strain — so it must not corroborate
        // absorption strain either (consistent with computeDecliningSignals dropping RIR). Without this,
        // bodyEscalate = effort.elevated && ledger.elevated stays true off the very signal we just
        // de-run-brained, corroborated_strain stays true, and the two-key cap never softens the verdict.
        // e1RM-declining remains the genuine strength-strain signal via computePrimaryAdherence (D-267).
        const _ledgerSig = trendSig(snapshotBody.weekly_trends.strength);
        const _ledgerForAbsorption = planPrimary === 'strength'
          ? { ..._ledgerSig, elevated: false, strong: false }
          : _ledgerSig;
        const absorption = assessAbsorption({
          effort: trendSig(snapshotBody.weekly_trends.effort_perception),
          ledger: _ledgerForAbsorption,
          driftSession: _driftSession,
          typicalSteadyDriftBpm: null, // v1 cold-start: historical gate-passing baseline deferred (correct for a thin anchor anyway)
          safetyFloor: computeSafetyFloor(snapshotBody.weekly_trends, readinessState, _decliningOpts),
        });
        ...
        const reconciled = reconcileLoadStatus(
          ...
          readinessForLoad, // D-416: refined readiness (systemic-only) — a lone effort-up never relabels load
          {
            weekIntent,
            weekIndex,
          ...
          overloadVerdict, // Slice 1: THE verdict — no lane escalates unless this says overloaded
        );
      ...
      const partialSnapshot: AthleteSnapshot = {
        version: 1,
      ...
      // Early artifact detection — when every completed run hit its planned duration/distance the
      // run-load delta is an intensity-factor artifact; the load bar patch below reads this flag.
      ...
      athleteSnapshot = partialSnapshot;
      ...
      // Replaced `coaching.narrative` (the last output-LLM). The `coaching` field itself, the model
      // headline and next-session guidance no client ever read, came out with the no-AI work order
      // (2026-09-07); the snapshot is now sections 1-4 only.
      //
      ...
      try {
        ...
        for (const [k, v] of Object.entries(rawPosture)) {
          // The stored/sanitized parked value is 'out' (posture.ts); the week composer's vocabulary is
          // 'dropped'. Map 'out' → 'dropped' so a PARKED discipline is excluded, not silently demoted to
          // 'unknown' and then reported. Bug: bike posture='out' → unknown → "Riding came in below its
          // recent normal" on a strength plan bike has nothing to do with (Michael on device 2026-07-24).
        ...
        const deterministicWeek = composeCoachWeekInsight({
          ...
          partialWeek: false,
          // ⛔ FED FROM THE CARD, AND FED IN FULL. This block used to carry 3 of its 6 fields — no
          // `weekInBlock`, no `isDeloadWeek`, no `workingPct` — so the 5×5 ceiling clause could never
          // fire and the deload suppression never fired, on ANY protocol. A built reader starved of
          // its inputs (audit F3).
          strengthProtocol: {
            protocolId: blockIdentity.protocolId,
            weekInBlock: blockIdentity.weekIndex,
            isDeloadWeek: blockIdentity.cycle?.isDeload ?? null,
            // The reader wants a whole percent (it compares against an 85 ceiling); the card carries
            // the fraction the composer wrote the sets from.
            workingPct: typeof blockIdentity.topSetPct === 'number' ? Math.round(blockIdentity.topSetPct * 100) : null,
    ...
    // The week narrative has exactly one author: the deterministic composer above (D-306). The legacy
    // model fallback that used to sit here was deleted with the no-AI work order (2026-09-07); when the
    // snapshot path throws, week_narrative stays null and the card shows no sentence.
    void deterministicNarrativeApplied;
    ...
    const weekExecutionV1: NonNullable<CoachWeekContextResponseV1['weekly_state_v1']>['week_execution_v1'] = (() => {
      ...
      // ANCHOR DESCENT (2026-07-17): the rolling run anchor eased because its source aged out — carried on
      // the spine (run_anchor_descent) so we don't infer it. The CREDIT clause is GATED: aerobic load
      // actually carried by other disciplines THIS window AND HR-response not degrading (efficiency proxy).
      const descentSignal = (latestSnapshot?.state_trends_v1 as any)?.run_anchor_descent ?? null;
      ...
      try {
        ...
        for (const [rawDisc, p] of Object.entries(posture)) {
          ...
          // THE SLIP GATE (refines D-297): the aerobic base is MEASURABLY declining (hr_drift worsening with
          // sufficient data — the same signal the interference check reads). Only then does the upkeep line
          // flip from compliance-only to the tipping-point flag. Holding/thin data → false → D-297 unchanged.
          const hd: any = (weeklyResponseModel as any)?.endurance?.hr_drift;
          const baseSlipping = !!(hd?.sufficient && hd?.trend === 'declining');
          upkeepCandidates.push(upkeepCandidate({ discipline: disc as any, actualPerWeek: perWeek, targetPerWeek: targetMiles, unit: 'mile', weeksUnder, aerobicCarriers: carriers, baseSlipping }));
        }
      } catch { /* upkeep read is non-fatal */ }

      // TRADE + UPKEEP REMOVED from the accent (2026-07-21) — the COACH'S EYE row now owns the whole
      // cross-discipline running-low story (the FLOOR flag: "at 6 of your 18-mile target — under what
      // holds it", + the specificity in its detail). Keeping the trade sentence here ("running came in
      // at 0 of 3; cycling carried the load; it fades") was the SAME story a second time, on a different
      // window (Michael, on device). What stays is what the coach's eye does NOT cover: over-reach
      ...
      const accent: WeekAccent | null = composeWeekAccent([
        ...
        bannerCandidate(banner?.line, banner?.branch === 'carried' ? null : banner?.branch),
      ]);
    ...
    const weekly_state_v1: NonNullable<CoachWeekContextResponseV1['weekly_state_v1']> = {
      ...
      week: {
        ...
        intent_summary: (() => {
          ...
          // ⛔ NO SYNOPSIS HERE — the line DIRECTLY BELOW already is one. `buildLoadHeadline`
          // (src/lib/load-headline.ts, rendered at StateTab.tsx:1404) composes the reconciled load
          // verdict + readiness into "This week: Balanced load." A synopsis here restates it three
          // pixels away — the same redundancy that got the mix sentence deleted from the paragraph
          // (D-306 amendment). Position is the ONE thing no other line on the screen carries.
          //
          // A first pass DID emit "Load balanced, readiness fresh." here. It duplicated the line below,
          // and it also guarded against a contradiction that cannot happen: the LOAD row renders
          // statusVolumeLabel(load_status.status) — the RECONCILED verdict (LoadBar.tsx:71), never the
          // raw ACWR band. The raw `label` at :5464 has no client consumer found. See Q-196.
          const synopsis: string | null = null;
      ...
      /**
       * ⛔⛔ THE FIVE WEEKLY NUMBERS, FORWARDED — READ, NEVER COMPUTED (Law-4).
       *
       * The block stored all twelve weeks at build (`buildStandingPlanRow`) and
       * `rematerialize-standing-block` refreshes them when earned sets rewrite the remainder. This
       * looks up the week the athlete is in and passes it through untouched.
       *
       * ⚠️ `planActiveNow` GATES IT for the same reason `week.index` is null before the start
       * (D-232): a plan that has not begun has no current week, and printing week 1's numbers on a
       * screen the athlete is reading a fortnight early states a fact about a week they are not in.
       * ⚠️ ABSENT, NEVER ZEROED. No standing block, an older block with no stored ledger, or a week
       * the map does not carry → the field is null and the client renders nothing. A zeroed bar
       * would say the week holds no work.
       */
      week_ledger_v1: (() => {
        if (!planActiveNow) return null;
        const wIdx = Number(weekIndex);
        if (!Number.isFinite(wIdx) || wIdx < 1) return null;
        const ledgers = (activePlan?.config?.standing_plan as any)?.week_ledgers;
        if (!ledgers || typeof ledgers !== 'object') return null;
        const led = ledgers[String(wIdx)];
        return led && typeof led === 'object' ? led : null;
      })(),
      /**
       * ⛔⛔ WHAT EVERY HEAVY SESSION OF THIS BLOCK WAS — forwarded, not judged.
       *
       * `meSessionOutcome` has run inside `earnedMeSets` on every restate since the ME ladder
       * shipped; the walk it makes is now stored on the row and this passes it through. The word a
       * card prints is a mapping of `outcome` at the display edge — the coach does not name it, and
       * the ladder's own thresholds are not a display concern.
       *
       * ⚠️ EMPTY ON A FRESH BLOCK, and that is the honest state rather than a broken one: no heavy
       * session has been logged, so there is no word. A card renders nothing.
       * ⚠️ NOT GATED ON `planActiveNow`. Unlike the week ledger this is not a claim about the week
       * the athlete is in — it is what has already been trained, and it reads the same before a
       * block's start date as after it.
       */
      me_history_v1: (() => {
        const sp = activePlan?.config?.standing_plan as any;
        const hist = sp?.me_history;
        if (!hist || typeof hist !== 'object') return null;
        return {
          history: hist,
          last_reps: (sp?.me_last_reps && typeof sp.me_last_reps === 'object') ? sp.me_last_reps : null,
          // ⚠️ The weight those reps were performed at — one reading with `last_reps`, never sourced
          // separately. See the write site in `rematerialize-standing-block`.
          at_weight: (sp?.me_at_weight && typeof sp.me_at_weight === 'object') ? sp.me_at_weight : null,
        };
      })(),
      guards: {
      ...
      load: {
        ...
        form_by_day, // each of the last 84 days' own form + zone word, for the Today card on a past day (v214)
        week_time_line, // "strength 2h 10m · run 2h 30m · bike 3h 40m" — the plan week so far, State's load card (v216)
        body_line_by_day, // each of the last 84 days' own BODY line, same rules as today's (v214)
        label: formZone(fitnessFatigue.form), // Friel's Form zone word — the KEY (colour, checks); the ACWR bands are gone
        label_text: formZoneText(fitnessFatigue.form), // the zone's one on-screen name ("building"), approved 2026-09-26 — State and Today print this
        form_key: formKeyText({ fitness: fitnessFatigue.display.fitness.value, fatigue: fitnessFatigue.display.fatigue.value, form: fitnessFatigue.display.form.value }), // the ⓘ's words, approved 2026-09-26
        ...
        fitness_fatigue: fitnessFatigue, // TrainingPeaks PMC (fitness / fatigue / form) — THE load read on State since 2026-09-04
      ...
      strength_logged_sets: buildStrengthLoggedSets(
        ...
        // The athlete's unit — a set is stored in pounds and the lines come back already converted
        // (2026-09-15, §8.0 #7 for this screen). The list printed "lb" on every account.
        String((arc.performance_numbers as { units?: unknown } | null)?.units ?? 'imperial') === 'metric',
      ),
      trends: {
        fitness_direction: fitnessDirection,
        /**
         * ⛔ THE FTP STATE PRINTS IS THE ONE THE APP RUNS ON (2026-09-15, TRUTH-MAP §8.0 #22). The bike
         * row took its number from `display.fitnessAnchors.bike`, which compute-snapshot builds from
         * `appliedLearnedFtp` — accepted, else the estimate, with NO tier for a typed FTP. So an athlete
         * on "your number" read the learned watts on State while Adjust, Baselines, the power zones and
         * every prescribed ride used the typed ones. The ANCHOR is untouched: it is the measured reading
         * and its label says "FTP estimate", which is true. This is the number in use, resolved once by
         * `resolveCurrentFtp` — the same call the other three screens make — and the row prints it.
         */
        applied_ftp: resolveCurrentFtp({
          learned_fitness: learnedFitness,
          performance_numbers: arc.performance_numbers,
        } as any)?.value ?? null,
        // The word beside it — where that number came from, by the rule Adjust's readout uses (2026-09-17, v212).
        applied_ftp_word: ftpSourceWord({
          learned_fitness: learnedFitness,
          performance_numbers: arc.performance_numbers,
        } as any),
        /**
         * ⛔ THE RUN'S HEADLINE IS THE THRESHOLD THE APP RUNS ON (2026-09-18, v215) — the bike row's shape. The pace
         * is `resolveCurrentRunThresholdPace`, the one the plan (materialize-plan) and Adjust's readout read, printed
         * in the athlete's unit; the word is Adjust's (`runThresholdSourceWord`); the heart rate is
         * `resolveCurrentLthr` for the run over the same three inputs Adjust passes. The easy target is the plan's
         * easy range off that same threshold (Friel run Zone 2, `resolveCurrentRunEasyPace`), so an accepted
         * threshold moves all four together. The phone prints them.
         */
        ...(() => {
          const runBaselines = { learned_fitness: learnedFitness, performance_numbers: arc.performance_numbers } as any;
          const metric = !isImperial;
          const thr = resolveCurrentRunThresholdPace(runBaselines);
          const easy = resolveCurrentRunEasyPace(runBaselines);
          const lthr = resolveCurrentLthr(
            { learned_fitness: learnedFitness, performance_numbers: arc.performance_numbers, configured_hr_zones: configuredHrZones } as never,
            { sport: 'run' },
          ).bpm;
          const lo = formatRunPace(easy.range_lo_sec_per_mi, metric);
          const hi = formatRunPace(easy.range_hi_sec_per_mi, metric);
          return {
            applied_run_threshold: formatRunPace(thr.sec_per_mi, metric),
            applied_run_threshold_word: thr.sec_per_mi != null ? runThresholdSourceWord(runBaselines) : null,
            applied_run_threshold_hr: lthr != null ? Math.round(lthr) : null,
            run_easy_target: lo && hi ? `${lo.replace(/\/(mi|km)$/, '')}–${hi}` : null,
          };
        })(),
    ...
    // ⛔ AUDIT H-B10 (2026-09-10): after race day and before a result is saved, the finish off the race-day
    // workout — `pickRaceDayWorkout`, the pick complete-race saves. State found its own run here before.
    let post_race_unofficial: CoachWeekContextResponseV1['post_race_unofficial'] = null;
    try {
      // The readiness State reads: usable when it carries a finish display (`src/lib/coach-payload.ts`).
      const rrUsable = raceReadiness && typeof raceReadiness.predicted_finish_display === 'string' ? raceReadiness : null;
      const candidate = postRaceCandidate({
        asOfDate,
        activePlanId: activePlan?.id ?? null,
        activePlans: allActivePlans.map((p) => ({ plan_id: String(p.id), race_date: p.config?.race_date ?? null })),
        goals: goalContext.goals,
        primaryEventId: goalContext.primary_event?.id ?? null,
        // The projection State reads: usable with a goal id and an anchor time (`src/lib/coach-payload.ts`).
        projectionGoalId: raceFinishProjectionV1 && Number(raceFinishProjectionV1.anchor_seconds) > 0 ? raceFinishProjectionV1.goal_id ?? null : null,
        readinessGoalId: (rrUsable as { goal?: { id?: string | null } } | null)?.goal?.id ?? null,
        lastCompletedRaceGoalId: last_completed_race?.goal_id ?? null,
      });
      if (candidate) {
        const { data: raceDayRows, error: raceDayErr } = await supabaseService
          .from('workouts')
          .select('id, date, type, workout_status, moving_time, elapsed_time, duration, computed, name')
          .eq('user_id', userId)
          .eq('date', candidate.race_date)
          .eq('workout_status', 'completed');
        if (raceDayErr) throw raceDayErr;
        post_race_unofficial = buildPostRaceUnofficial(
          candidate,
          Array.isArray(raceDayRows) ? raceDayRows : [],
          rrUsable ? { seconds: Number(rrUsable.predicted_finish_time_seconds), display: rrUsable.predicted_finish_display } : null,
        );
      }
    } catch (prE: any) {
      console.warn('[coach] post_race_unofficial failed (non-fatal):', prE?.message ?? prE);
    }
    ...
    const response: CoachWeekContextResponseV1 = {
      ...
      plan: {
        ...
        week_start_dow: weekStartDow,
        athlete_context_for_week: athleteContextStr || null,
        // ⛔ WHAT THIS BLOCK IS — the fact State was never told (Q-230, audit F9). Every plan-blind
        // read on that screen was downstream of its absence: the payload carried a plan id, a name and
        // a week number, and nothing about what the plan was FOR or where in its own rhythm the week
        // sat. One card, resolved once (see `blockIdentity` above), rendered — never re-derived.
        // ⚠️ Nulls here are honest: they mean the plan did not say, and a surface must render nothing
        // rather than fill the gap.
        block: {
          protocol_id: blockIdentity.protocolId,
          protocol_known: blockIdentity.protocolKnown,
          // 'amrap' | 'rir' | 'none' — what this block reads to judge a session. A screen asking for
          // reps-in-reserve on a protocol that reads an all-out set is asking for nothing.
          effort_read: blockIdentity.effortRead,
          ...
          phase_word: blockIdentity.phaseWord,
          cycle_kind: blockIdentity.cycle?.kind ?? null,
          week_in_cycle: blockIdentity.cycle?.weekInCycle ?? null,
          is_deload_week: blockIdentity.cycle?.isDeload ?? null,
          has_all_out_set: blockIdentity.hasAllOutSet,
          // ⚠️ NOT the same as `has_all_out_set`: every anchor week ends on an open set, but only the
          // 95% one is the reading that moves the working number.
          is_measurement_week: blockIdentity.isMeasurementWeek,
          top_set_pct: blockIdentity.topSetPct,
        },
      ...
      week: {
        ...
        // State's NEXT row, cut here — after today, at most three (2026-09-15).

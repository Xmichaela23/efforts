...
import { composeCrossTrainingRead } from '../_shared/insights/cross-training-read.ts';
          ...
          //    a declared FOCUS and a real trade/cost/room story; else falls through to the reassurance.
          {
            ...
            if (stv) {
              ...
              const ctStates = [
                ...
                { discipline: 'run', posture: ctPosture.run || 'unknown', verdict: stv.run?.decoupling?.verdict || 'needs_data', acwr: runningAcwr ?? null, underTarget: runUnderTarget },
                { discipline: 'bike', posture: ctPosture.ride || 'unknown', verdict: bikeV, acwr: cyclingAcwr ?? null },
              ];
              const ctRead = composeCrossTrainingRead(ctStates as any);
              if (ctRead) return { label: ctRead.headline, tone: ctRead.tone, detail: ctRead.detail };
            }
          }

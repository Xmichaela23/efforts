  ...
  try {
    ...
    // can't contradict the PERFORMANCE run/bike reads.
    {
      ...
      if (hrResp.verdict !== 'needs_data' && hrResp.contributors.length > 0) {
        ...
        // ⛔ ONE VOCABULARY, AND NO SECOND VERDICT FOR THE SAME NUMBER (2026-07-31).
        //
        // Michael: *"we need consistant language which this isnt"* — and, on the section as a whole,
        // *"this whole section really is just how are you handling the training."*
        //
        // This row used to mint its own headline ("drifting up — working harder to hold effort", in
        // RED) beside the RUN row's word for the same movement ("easing off", in AMBER). That was
        // defensible while the run half was DECOUPLING — a different metric deserves different words.
        // It stopped being defensible the moment the run half became `run.efficiency`: the two rows
        // then described the identical number with different words and different severity, and red
        // beside amber reads as two findings when there is one.
        //
        // ⚠️ THE COLLISION IS ON RECORD. Payload v144 already had to rewrite this string because
        // "easing" here meant the OPPOSITE of "easing off" on the row beneath it. Two vocabularies
        // adjacent on one screen have now caused the same class of problem twice.
        //
        // So the row STATES ITS CONTRIBUTORS in the app's shared trend words and stops. It is a
        // rollup; naming what rolled up is the honest thing it can add, and it cannot contradict the
        // rows it is summarising because it uses their words.
        const SHARED = (v: string): string => v === 'improving' ? 'improving' : v === 'sliding' ? 'easing off' : 'holding';
        const discLabel = (d: string): string => d === 'run' ? 'running' : d === 'bike' ? 'bike' : d;
        const partVerdict = SHARED;
        // ⚠️ "(0d ago)" is not English. The shared `recencyLabel` already says "today" at zero; this
        // local formatter did not, and it surfaced the moment the row started reading a pool whose
        // newest run is today rather than yesterday.
        const partAge = (a: number | null): string => a == null ? '' : a <= 0 ? ' (today)' : ` (${a}d ago)`;
        ...
        // NO SILENT DROP + OPPORTUNITY nudge (Michael, 2026-07-21): a stale/absent contributor is named
        // with the LEVER (a steady session refreshes it), never a scold (Garmin's "unproductive"). Run
        // note does double-duty when the athlete is under their declared run target; the bike side names
        // "a steady ride with power" when its last qualifier is old; the "base can slip" consequence is
        // gated on aerobic work being GENUINELY low (run under target AND no fresh bike holding it).
        const runUpkeepHr = (() => {
          try {
            ...
            if (!(Number.isFinite(tgtMi) && tgtMi > 0)) return false;
        ...
        // ⛔ THE CONTRIBUTOR CLAUSE IS GONE (D-353, 2026-08-01). It read "Running easing off (1d ago) ·
        // bike holding (17d ago)" — which is the RUN row and the BIKE row, rendered again, two inches
        // above themselves. Same redundancy killed in v119 (restating the LOAD bar) and v131 (the
        // running story told twice). It was defensible while this row had its own metric (decoupling);
        // once D-346 pointed it at `run.efficiency`, it became a second telling of one fact.
        //
        // ⚠️ GLASS-BOX IS NOT WEAKENED. The evidence is the two verdict rows rendered DIRECTLY BENEATH
        // this one — on screen, adjacent, tappable. Glass-box requires evidence be REACHABLE, not
        // restated. Citation by adjacency is sufficient when the cited rows are immediately below.
        //
        // The ages the clause carried are not lost: `parts` still feeds the PROVENANCE (the tap-down),
        // so "bike holding (17d ago)" is one tap away and the stale half is still named.
        const lead = parts.join(' · ');
        const detail = dir === 'improving'
          ? 'Settling — lower heart rate at the same effort'
          : dir === 'declining'
            ? 'Easing off'
            : 'Holding';
        // The contributor list moves HERE — demoted from the glance to the receipt, not deleted.
        ...
        weeklyResponseModel.visible_signals.unshift({
          ...
          // ⛔ SEVERITY IS CAPPED AT THE EVIDENCE (D-353). This line used to be a bare literal —
          // `declining ? 'warning' : ...` — pinned to what the RUN row happened to look like at the
          // time ("WARNING, NOT DANGER — the RUN row renders the same movement in amber"). That row
          // changed and this line did not, so a rollup was rendering a severity its own contributors
          // do not carry. `capRollupTone` makes that structurally impossible: this row can never
          // out-alarm `run.efficiency` + `bike.efficiency`, whatever either is rendered as later.
          trend_tone: capRollupTone(
            dir === 'improving' ? 'positive' : dir === 'declining' ? 'warning' : 'neutral',
            hrResp.contributors.map((c) => c.verdict),
          ),
          detail,
          provenance,

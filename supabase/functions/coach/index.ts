...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fitnessFatigueToday = computeFitnessFatigue(ffLoadRows, { asOfDate });
    ...
    const fitnessFatigueDisplay = {
      ...
      fatigue: {
        ...
        /** The athlete's own middle half of 12 weeks of daily fatigue, "44–60" — set with the day series below (v216). */
        usual: null as string | null,
      },
    ...
    // ⛔ FATIGUE'S "usual" (2026-09-18, approved by Michael) — the athlete's own middle half of their daily fatigue
    // over the same 12 weeks State shows (today and the 83 days before it), by the rule the Performance screen's
    // Workload "usual" uses (`_shared/middle-half.ts`). Each day's fatigue is `computeFitnessFatigue` as of that day,
    // the call this loop already makes for form. Below the rule's minimum there is no range.
    const fatigueDays: number[] = [];
    if (fitnessFatigueToday.fatigue != null) fatigueDays.push(fitnessFatigueToday.fatigue);
    try {
      for (let i = 1; i <= DAY_SERIES_DAYS; i++) {
        ...
        if (i < DAY_SERIES_DAYS && ff.fatigue != null) fatigueDays.push(ff.fatigue);
      }
    ...
    {
      const band = middleHalf(fatigueDays);
      fitnessFatigueDisplay.fatigue.usual = band
        ? (band.low === band.high ? `${band.low}` : `${band.low}–${band.high}`)
        : null;
    }

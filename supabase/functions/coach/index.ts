  ...
  try {
    ...
    // can't contradict the PERFORMANCE run/bike reads.
    {
      ...
      if (hrResp.verdict !== 'needs_data' && hrResp.contributors.length > 0) {
        ...
        // The DETAIL is the contributor statement — the row's whole content. The provenance keeps the
        // ages and any exclusion note, so the glance is the fact and the tap-down is the receipt.
        const detail = lead ? lead.charAt(0).toUpperCase() + lead.slice(1) : 'holding';
        const provenance = `${excludedRun ? excludedRun : ''}${bikeOpp}${baseSlip}`.trim();
        ...
        weeklyResponseModel.visible_signals.unshift({
          ...
          // ⚠️ WARNING, NOT DANGER — the RUN row renders the same movement in amber. Red beside amber
          // for one number reads as two findings of different size.
          trend_tone: dir === 'improving' ? 'positive' : dir === 'declining' ? 'warning' : 'neutral',
          detail,
          provenance,

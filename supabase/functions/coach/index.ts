 ...
 *  `isMain531Lift` and returns an EMPTY label for anything else, so a hard-feeling accessory
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // strength-primary block, so this resolved a `durability` RIR profile for a 5/3/1 block — the
    ...
    //
    // ⛔ THE APP COLLECTS THE ONE MEASUREMENT 5/3/1 IS BUILT ON AND STATE REASONED FROM SOMETHING
    // ELSE. Michael, on his own screen: Deadlift read *"Working ~120 vs your 150 baseline"* while his
    // logged e1RM was already ~225 — the row compares a light 5/3/1 working weight to a stale TYPED
      ...
      try {
        ...
        return Object.entries(s1rms)
          ...
          .map(([key, v]: [string, any]) => {
            ...
            // consults `usesRir` and falls through to `defaultTargetRir` — 2 — so a 5/3/1 lift was

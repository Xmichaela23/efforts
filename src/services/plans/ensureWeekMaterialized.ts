...
export async function ensureWeekMaterialized(planId: string, weekNumber: number): Promise<{ inserted: number }>{
  ...
  for (const s0 of weekSessions) {
    ...
    const strengthFromTokens = (stepsPreset?: any[]): any[] | undefined => {
      ...
      // Client-side strength calculations removed - server handles all 1RM and accessory ratio calculations
      for (const tok of steps){
        ...
        if (m){
          ...
          // Server handles all strength calculations - no client-side fallback needed
  ...
  try {
    ...
    if (filtered.length) {
      const { error } = await supabase.from('planned_workouts').insert(filtered as any);

...
- **USE ALL BASELINE DATA:**
  ...
  // Generate plan using bulletproof PlanEngine
  const generatePlan = async () => {
    try {
      setGeneratingPlan(true);
      
      // Use the bulletproof PlanEngine instead of unreliable AI
      const planEngine = new PlanEngine(baselines, responses);
      const result = planEngine.generatePreviewPlan();
      
      console.log('Plan generated by PlanEngine:', result);
      
      // Create plan object with predictable structure
      const plan = {
        id: `plan-${Date.now()}`,
        name: result.name,
        description: result.description,
        focus: selectedFocus.join(', '),
        plan: result, // Use the bulletproof plan structure
        fullPlan: { plan: result }
      };

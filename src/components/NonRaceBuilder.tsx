...
// ⛔ `WeekStrip` MOVED TO ITS OWN FILE (2026-09-21) — "Can't train this day" shows the same strip. `./WeekStrip`.
...
// ⛔ THE STEP FLOW AND ITS KEYS LIVE IN `src/lib/wizard-steps.ts` (2026-08-30) — a file with no
// React in it, so a test can CALL the router instead of reading this one's source text. See that
...
export default function NonRaceBuilder({ onClose, entry: initialEntry, onPlanSeason }: { onClose?: () => void; entry?: EntryCardId; onPlanSeason?: () => void } = {}) {
  ...
  // ⛔ The easy pace the plan payload carries. The run step's weekly-miles placeholder that also read it
  // was on a step no route reached, and came out with it (2026-09-16, Stage 7 session 1).

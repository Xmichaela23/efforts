...
export default function AssociatePlannedDialog({ workout, open, onClose, onAssociated }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md p-5 space-y-4 border border-white/20">
        <div className="text-lg font-semibold text-gray-800/80">Associate with planned…</div>
        <div className="text-sm text-gray-500">{String(workout?.name || '')} • {workout?.date}</div>
        {error && <div className="text-sm text-red-500 bg-red-50/50 rounded-lg px-3 py-2">{error}</div>}
        {loading ? (
                ...
                <button 
                  ...
                  className={`w-full text-left rounded-xl p-3 transition-all duration-200 ${
                    isSameDay 
                      ...
                      : 'bg-white/50 border border-gray-200/50 hover:bg-white/80 hover:border-gray-300/50'
                  }`}
                >

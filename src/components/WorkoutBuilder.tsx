...
export default function WorkoutBuilder({ onClose, initialType, existingWorkout }: WorkoutBuilderProps) {
  ...
  // FIXED: Bulletproof trash delete function
  const handleTrashClick = (e: React.MouseEvent) => {
    ...
    if (existingWorkout) {
      ...
      try {
        // FIXED: Get the workout ID properly
        const workoutId = existingWorkout.id;
        const workoutDate = existingWorkout.date;
        
        // Remove from main workouts array
        const savedWorkouts = JSON.parse(localStorage.getItem('workouts') || '[]');
        const updatedWorkouts = savedWorkouts.filter((w: any) => w.id !== workoutId);
        localStorage.setItem('workouts', JSON.stringify(updatedWorkouts));
        
        // Remove from date-specific storage
        const dateKey = `workout_${workoutDate}`;
        localStorage.removeItem(dateKey); // FIXED: Just remove the entire key
        
        // FIXED: Clear any form state completely
        setFormData({
          name: '',
          type: 'run',
          date: getLocalDateString(),
        ...
        // FIXED: Force immediate refresh
        setTimeout(() => {
          window.dispatchEvent(new Event('storage'));
          window.dispatchEvent(new Event('workoutDeleted'));
  ...
  // FIXED: Bulletproof save function
  const handleSave = (navigateAfterSave: boolean = false) => {
    try {
      // FIXED: Generate clean data
      const workoutTitle = formData.name.trim() || `${formData.type.charAt(0).toUpperCase() + formData.type.slice(1)} - ${new Date(formData.date).toLocaleDateString()}`;
      ...
      const workoutData = {
        ...
        name: workoutTitle,
        type: formData.type,
        date: formData.date, // FIXED: Use exact date from form
        description: finalDescription,
        userComments: formData.userComments,
        completedManually: formData.completedManually,
        ...
        comments: existingWorkout?.comments || []
      };
      
      console.log('Saving workout:', workoutData); // Debug log
      
      // FIXED: Save to main workouts array
      const savedWorkouts = JSON.parse(localStorage.getItem('workouts') || '[]');
      
      if (existingWorkout) {
        // Update existing
        const index = savedWorkouts.findIndex((w: any) => w.id === existingWorkout.id);
        if (index !== -1) {
          savedWorkouts[index] = workoutData;
        } else {
          savedWorkouts.push(workoutData); // Fallback if not found
        }
      } else {
        // Add new
      ...
      // FIXED: Save to date-specific storage (overwrite completely)
      const dateKey = `workout_${formData.date}`;
      const dateWorkouts = [workoutData]; // FIXED: Simple single workout per date
      localStorage.setItem(dateKey, JSON.stringify(dateWorkouts));
      
      console.log(`Saved to ${dateKey}:`, dateWorkouts); // Debug log
      
      // FIXED: Force immediate refresh
      setTimeout(() => {
        window.dispatchEvent(new Event('storage'));
        window.dispatchEvent(new Event('workoutAdded'));
        window.dispatchEvent(new Event('workoutDeleted')); // Clear any stale data
      }, 0);
      ...
      if (navigateAfterSave) {
        setTimeout(() => onClose(), 100); // Small delay for events to fire
      }
    } catch (error) {
    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      ...
      {showSaveOptions && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-black text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center gap-4">
          ...
          <span>{existingWorkout ? 'effort updated' : 'effort saved'}</span>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold">
          {existingWorkout ? 'edit effort' : (formData.name.trim() || 'new effort')}
        </h1>

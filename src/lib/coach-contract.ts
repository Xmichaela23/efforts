/**
 * Reads `COACH_PAYLOAD_VERSION` from `supabase/functions/_shared/coach-payload-version.ts` — one number.
 ...
 */
import { COACH_PAYLOAD_VERSION } from '@shared/coach-payload-version';
...
// ⛔ THE FLOOR IS THE SERVER'S OWN NUMBER (2026-09-10) — `_shared/coach-payload-version.ts`. It sat at
// 205 while the server moved to 209, and a fresh 208 saved copy kept old words on State. The D-373
// warning above was about a version check in the response path that dropped the whole payload; the
// response path no longer checks the version (`runPipeline` sets whatever `coach` returns), so a floor
// ahead of the deployed server costs a recompute on open, never a blank screen.
export const COACH_CLIENT_MIN_PAYLOAD_VERSION = COACH_PAYLOAD_VERSION; // history (was a hand-kept copy): 205 — 2026-09-07: athlete_snapshot.coaching removed (no-AI work order); older caches carry a model-written field. 202: // 2026-09-05: prior-day values on fitness_fatigue. 201: // 2026-09-05: week_ago on fitness_fatigue. 200: // 2026-09-04: the bike row draws FTP over time (`display.bikeFitness.ftpHistory`); a 199 row has no history and still carries the deleted `range` dot. compute-snapshot + coach deployed at 200 before this moved (CLI confirmed the deploy; the by-call check of the served version was not run this session). 199: // 2026-09-04: grader + raw-pace fallback. 198: // 2026-09-04: pools rebuilt after the regrade. 197: // 2026-09-04: run facts regraded. 196: // 2026-09-04: drift points re-gated on the measured steady test. 195: // 2026-09-04: LOAD reads TrainingPeaks' fitness / fatigue / form off the whole history; a 194 row's 84-day fitness is under-seeded. 194: // 2026-09-04: bike trend eligibility changed; a 193 row still marks 10 of 13 rides out. 193: // 2026-09-04: trend verdicts are Garmin's 28/28 rule (no withheld / recentlyFlat; `recent` = last 28 days); the headline averages the recent-flagged points, so a 192 row would average the wrong span. // 2026-09-04 (D-372): 7-day drift verdict labels removed server-side; a 191 row still carries them. 191: // 2026-09-04: `driftWholeSession` on spine points means an interval session only, not the mixed-effort hedge — the State drift trend draws every steady run; a 190 row still carries 3 usable run points out of 22, and the client reads coach_cache directly so the server bump alone does not reach the screen. 190: // 2026-09-03: spine points keyed by workout id, `countsTowardTrend`, 'power' drift basis, and the ride climb — the client reads coach_cache directly, so the server bump alone does not reach the screen. 189:  BODY is three signals (effort · soreness · logged); a 188 row still holds the paragraph, and the client reads coach_cache directly so the server bump alone does not reach the screen. 188: warm-up easy points + one drift read on the State card

...
⚠️ **UNVERIFIED, AND WORTH CHECKING BEFORE BUILDING EITHER:** on 2026-09-03 both arrow maps
in `StatePerformanceSection.tsx` (`VERDICT` line 44, `NUMERIC` line 75) carried `arr: ''` on
every entry, while the comment block above them describes `↑ / → / ↓` in detail. Either the
arrows draw from somewhere else, or the maps are starved and the comment is stale. **Settle
that before adding an arrow — this may be a wiring job, not a build job.** Second open
question: the aerobic series headline is a MEDIAN of the last five points, and a median is
not a direction. If the server emits no `improving` / `holding` / `sliding` verdict for that
group, there is nothing to point an arrow with and the work is server-side.
